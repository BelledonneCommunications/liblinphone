...
static void call_with_video_added_by_both_parties(void) {
	...
	/* Check that both return to StreamsRunning state, but with an error "request pending" */
	...
	BC_ASSERT_TRUE(linphone_error_info_get_protocol_code(ei) == 491);
	ei = linphone_call_get_error_info(pauline_call);
	BC_ASSERT_TRUE(linphone_error_info_get_protocol_code(ei) == 491);
	
	/* Check that retry is made automatically, and then video is correctly enabled. */
	BC_ASSERT_TRUE(wait_for_list(lcs, &marie->stat.number_of_LinphoneCallStreamsRunning,4,10000));
	BC_ASSERT_TRUE(wait_for_list(lcs, &pauline->stat.number_of_LinphoneCallStreamsRunning,4,10000));
	ei = linphone_call_get_error_info(marie_call);

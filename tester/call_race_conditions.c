...
static void call_with_video_added_by_both_parties(void) {
	...
	BC_ASSERT_TRUE(wait_for_list(lcs, &pauline->stat.number_of_LinphoneCallUpdating,1,liblinphone_tester_sip_timeout));
	
	/* Marie shall transition to UpdatedByRemote (Pauline has priority per RFC3261 since she is the call-id owner)*/
	BC_ASSERT_TRUE(wait_for_list(lcs, &pauline->stat.number_of_LinphoneCallUpdatedByRemote,1,liblinphone_tester_sip_timeout));
	
	/* Marie will succeed with its re INVITE transaction */
	BC_ASSERT_TRUE(wait_for_list(lcs, &marie->stat.number_of_LinphoneCallStreamsRunning,2,liblinphone_tester_sip_timeout));
	
	/* Pauline shall return to Updating state*/
	BC_ASSERT_TRUE(wait_for_list(lcs, &pauline->stat.number_of_LinphoneCallUpdating,2,liblinphone_tester_sip_timeout));
	/* And finally succeed too */
	BC_ASSERT_TRUE(wait_for_list(lcs, &pauline->stat.number_of_LinphoneCallStreamsRunning,2,liblinphone_tester_sip_timeout));
...
static void call_with_info_sent_by_both_parties(void) {
	...
	BC_ASSERT_TRUE(wait_for_list(lcs,&pauline->stat.number_of_InfoReceived,number_of_infos,liblinphone_tester_sip_timeout));
	BC_ASSERT_TRUE(wait_for_list(lcs,&marie->stat.number_of_InfoReceived,number_of_infos,liblinphone_tester_sip_timeout));
...
static void call_end_and_reinvite(void) {
	...
	BC_ASSERT_TRUE(wait_for_list(lcs, &pauline->stat.number_of_LinphoneCallReleased,1,liblinphone_tester_sip_timeout));

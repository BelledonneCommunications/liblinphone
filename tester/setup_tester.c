...
static void _create_friends_from_tab(LinphoneCore *lc, LinphoneFriendList *list, const char *friends[],
...
static LinphoneLdap *_create_default_ldap_server(LinphoneCoreManager *manager, BCTBX_UNUSED(const char *password),
												 const char *bind_dn) {
...
void version_update_check_cb(LinphoneCore *core, LinphoneVersionUpdateCheckResult result, const char *version,
							 const char *url) {
...
static void search_friend_in_alphabetical_order(void) {
	...
	if (BC_ASSERT_PTR_NOT_NULL(resultList)) {
		...
		_check_friend_result_list_2(manager->lc, resultList, 0, name3SipUri, NULL,
									LinphoneMagicSearchSourceFriends); //"sip:stephanie@sip.example.org"
		_check_friend_result_list_2(manager->lc, resultList, 1, name2SipUri, NULL,
									LinphoneMagicSearchSourceFriends); //"sip:alber@sip.example.org"
		_check_friend_result_list_2(manager->lc, resultList, 2, name5SipUri, NULL,
									LinphoneMagicSearchSourceFriends); //"sip:gal@sip.test.org"
		_check_friend_result_list_2(manager->lc, resultList, 3, name4SipUri, NULL,
									LinphoneMagicSearchSourceFriends); //"sip:gauthier@sip.example.org"
		_check_friend_result_list_2(manager->lc, resultList, 4, name1SipUri, NULL,
	...
	if (friend1)
		linphone_friend_unref(friend1);
	if (friend2)
		linphone_friend_unref(friend2);
	if (friend3)
		linphone_friend_unref(friend3);
	if (friend4)
		linphone_friend_unref(friend4);
	if (friend5)
		linphone_friend_unref(friend5);
	if (friend5)
		linphone_friend_unref(friend6);

	if (vcard1)
		linphone_vcard_unref(vcard1);
	if (vcard2)
		linphone_vcard_unref(vcard2);
	if (vcard3)
		linphone_vcard_unref(vcard3);
	if (vcard4)
		linphone_vcard_unref(vcard4);
	if (vcard5)
		linphone_vcard_unref(vcard5);
	if (vcard5)
		linphone_vcard_unref(vcard6);
...
static void search_friend_with_phone_number(void) {
	...
	if (stephanieVcard)
		linphone_vcard_unref(stephanieVcard);
...
static void search_friend_with_phone_number_2(void) {
	...
	if (laureVcard)
		linphone_vcard_unref(laureVcard);
...
static void search_friend_in_call_log(void) {
	...
	if (charlesAddress)
		linphone_address_unref(charlesAddress);
	if (ronanAddress)
		linphone_address_unref(ronanAddress);
...
static void search_friend_with_name(void) {
	...
	if (stephanie1Friend)
		linphone_friend_unref(stephanie1Friend);
	if (stephanie2Friend)
		linphone_friend_unref(stephanie2Friend);
	if (stephanie1Vcard)
		linphone_vcard_unref(stephanie1Vcard);
	if (stephanie2Vcard)
		linphone_vcard_unref(stephanie2Vcard);
...
static void search_friend_with_same_address(void) {
	...
	if (stephanieFriend1)
		linphone_friend_unref(stephanieFriend1);
	if (stephanieFriend2)
		linphone_friend_unref(stephanieFriend2);
	if (stephanieVcard1)
		linphone_vcard_unref(stephanieVcard1);
	if (stephanieVcard2)
		linphone_vcard_unref(stephanieVcard2);
...
static void search_friend_chat_room_remote(void) {
	...
	if (BC_ASSERT_PTR_NOT_NULL(resultList)) {
		if (linphone_core_ldap_available(marie->lc)) {
			...
			_check_friend_result_list_2(marie->lc, resultList, 7, addr, NULL,
...
static void check_results(LinphoneCoreManager *manager, bctbx_list_t *resultList, int sourceFlags) {
	...
	const int sources[] = {LinphoneMagicSearchSourceCallLogs,
						   ...
						   LinphoneMagicSearchSourceLdapServers,
						   LinphoneMagicSearchSourceFriends | LinphoneMagicSearchSourceCallLogs |
							   LinphoneMagicSearchSourceChatRooms | LinphoneMagicSearchSourceLdapServers,
						   LinphoneMagicSearchSourceLdapServers,
						   LinphoneMagicSearchSourceLdapServers,
						   LinphoneMagicSearchSourceLdapServers,
						   LinphoneMagicSearchSourceLdapServers,
	...
	int resultSize =
		...
		(linphone_core_ldap_available(manager->lc) &&
		...
		//+ ((sourceFlags != LinphoneMagicSearchSourceNone) && ( !linphone_core_ldap_available(manager->lc) ||
		//((sourceFlags & LinphoneMagicSearchSourceLdapServers) != LinphoneMagicSearchSourceLdapServers)) ? 1 : 0)	//
		// Common
		+
	...
	for (int count = 0; count < 12 && currentResult; ++count) {
		if (linphone_core_ldap_available(manager->lc) || (sources[count] != LinphoneMagicSearchSourceLdapServers)) {
			if ((sources[count] & sourceFlags) != LinphoneMagicSearchSourceNone) {
				...
				BC_ASSERT_NOT_EQUAL(
					linphone_search_result_get_source_flags((LinphoneSearchResult *)currentResult->data) &
						sources[count],
					0, int, "%d"); // Source result must match to the Friend at this place
...
static void async_search_friend_in_sources(void) {
	...
	if (linphone_core_ldap_available(manager->lc)) {
		...
		_check_friend_result_list_2(
			manager->lc, resultList, 0, "sip:+33655667788@ldap.example.org", NULL,
		...
		_check_friend_result_list_2(manager->lc, resultList, 4, "sip:u@sip.example.org", NULL,
		...
		_check_friend_result_list_2(manager->lc, resultList, 2, "sip:u@sip.example.org", NULL,
									LinphoneMagicSearchSourceRequest);
	}
	bctbx_list_free_with_data(resultList, (bctbx_list_free_func)linphone_search_result_unref);
...
static void ldap_search(void) {
	...
	if (linphone_core_ldap_available(manager->lc)) {
		...
		_check_friend_result_list(
			manager->lc, resultList, 0, "sip:+33655667788@ldap.example.org",
			NULL); // Laure. Note : we get it as an address because of linphone_ldap_params_set_sip_attribute(params,
				   // "mobile,telephoneNumber,homePhone,sn");
		_check_friend_result_list(manager->lc, resultList, 1, "sip:Laure@ldap.example.org", "+33655667788");
...
static void ldap_features_more_results(void) {
	...
	if (linphone_core_ldap_available(manager->lc)){
		...
		linphone_magic_search_get_contacts_list_async(magicSearch, "Big", "", LinphoneMagicSearchSourceLdapServers,
													  LinphoneMagicSearchAggregationNone);
		BC_ASSERT_TRUE(wait_for_until(manager->lc, NULL, &stat->number_of_LinphoneMagicSearchResultReceived, 1, maxTimeout*1000 + 10));// add 10ms to let some times to timeout callbacks.
		resultList = linphone_magic_search_get_last_search(magicSearch);
 ...
 * We finally check here that if the plugin is correctly loaded and the right choice of echo canceller implementation is
 * made*/
...
test_t setup_tests[] = {
	...
	TEST_NO_TAG("LPConfig safety", linphone_config_safety_test),
	...
	TEST_ONE_TAG("Search friend with domain and without filter", search_friend_with_domain_without_filter,
				 "MagicSearch"),
	...
	TEST_ONE_TAG("Multiple looking for friends with cache resetting", search_friend_research_estate_reset,
				 "MagicSearch"),
	...
	TEST_ONE_TAG("Search friend in call log but don't add address which already exist",
				 search_friend_in_call_log_already_exist, "MagicSearch"),

...
/**
 * @param[in] encryptionAlgo		0: no encryption, 25519, 448, 25519512 otherwise
 ...
 */
static void group_chat_hfts(const int encryptionAlgo,
	...
	if (encryptionAlgo != 0) {
		switch (encryptionAlgo) {
			...
			case 448:
				...
				linphone_config_set_string(linphone_core_get_config(pauline->lc), "lime", "curve", "c448");
				linphone_config_set_string(linphone_core_get_config(claire->lc), "lime", "curve", "c448");
				break;
			case 25519512:
				linphone_config_set_string(linphone_core_get_config(marie->lc), "lime", "curve", "c25519k512");
				linphone_config_set_string(linphone_core_get_config(pauline->lc), "lime", "curve", "c25519k512");
				linphone_config_set_string(linphone_core_get_config(claire->lc), "lime", "curve", "c25519k512");
				break;
		}
	...
	marieCr = create_chat_room_client_side(coresList, marie, &initialMarieStats, participantsAddresses, initialSubject,
	                                       (encryptionAlgo != 0), LinphoneChatRoomEphemeralModeDeviceManaged);
	...
	if (encryptionAlgo != 0) {
		switch (encryptionAlgo) {
			case 25519:
				...
				break;
			case 448:
				linphone_config_set_string(linphone_core_get_config(manager_to_restart->lc), "lime", "curve", "c448");
				break;
			case 25519512:
...
static void group_chat(const int encryptionAlgo,
                       bool_t dual_auth_lime_server,
                       bool_t external_sender,
                       bool_t restart_external_participant) {
	group_chat_hfts(encryptionAlgo, dual_auth_lime_server, external_sender, restart_external_participant, NULL, NULL,
	                NULL, NULL);
}

static void group_chat_external_domain_participant(void) {
	group_chat(0, FALSE, FALSE, FALSE);
}
static void group_chat_external_domain_participant_ext_sender(void) {
	group_chat(0, FALSE, TRUE, FALSE);
}
static void encrypted_message(void) {
	group_chat(25519, FALSE, FALSE, FALSE);
	group_chat(448, FALSE, FALSE, FALSE);
	if (liblinphone_tester_is_lime_PQ_available()) {
		group_chat(25519512, FALSE, FALSE, FALSE);
	}
}
static void encrypted_message_ext_sender(void) {
	group_chat(25519, FALSE, TRUE, FALSE);
	group_chat(448, FALSE, TRUE, FALSE);
	if (liblinphone_tester_is_lime_PQ_available()) {
		group_chat(25519, FALSE, TRUE, FALSE);
	}
}
static void group_chat_external_domain_participant_external_restart(void) {
	group_chat(0, FALSE, FALSE, TRUE);
}
static void encrypted_message_ext_sender_external_restart(void) {
	group_chat(25519, FALSE, TRUE, TRUE);
	group_chat(448, FALSE, TRUE, TRUE);
	if (liblinphone_tester_is_lime_PQ_available()) {
		group_chat(25519, FALSE, TRUE, TRUE);
	}
}
static void group_chat_plain_with_file(void) {
	group_chat_hfts(0, FALSE, FALSE, FALSE, file_transfer_url, NULL, file_transfer_url, NULL);
}
static void group_chat_plain_with_file_ext_sender(void) {
	group_chat_hfts(0, FALSE, TRUE, FALSE, file_transfer_url, NULL, file_transfer_url, NULL);
}
static void group_chat_plain_with_file_auth_fileserver(void) {
	group_chat_hfts(0, FALSE, FALSE, FALSE, file_transfer_url_digest_auth, file_transfer_get_proxy,
...
static void group_chat_plain_with_file_ext_sender_auth_fileserver(void) {
	group_chat_hfts(0, FALSE, TRUE, FALSE, file_transfer_url_digest_auth, file_transfer_get_proxy,
	                file_transfer_url_digest_auth_external_domain, file_transfer_get_proxy_external_domain);
}

static void encrypted_message_dual_auth(void) {
	group_chat(25519, TRUE, FALSE, FALSE);
	group_chat(448, TRUE, FALSE, FALSE);
	if (liblinphone_tester_is_lime_PQ_available()) {
		group_chat(25519, TRUE, FALSE, FALSE);
	}
}
static void encrypted_message_dual_auth_ext_sender(void) {
	group_chat(25519, TRUE, TRUE, FALSE);
	group_chat(448, TRUE, TRUE, FALSE);
	if (liblinphone_tester_is_lime_PQ_available()) {
		group_chat(25519, TRUE, TRUE, FALSE);
	}
}

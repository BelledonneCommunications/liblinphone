...
static void carddav_sync_3(void) {
	LinphoneCoreManager *manager = linphone_core_manager_new2("carddav_rc", FALSE);
	LinphoneCardDAVStats *stats = (LinphoneCardDAVStats *)ms_new0(LinphoneCardDAVStats, 1);
	LinphoneVcard *lvc = linphone_vcard_new_from_vcard4_buffer(
...
static void carddav_server_to_client_and_client_to_sever_sync(void) {
	...
	LinphoneCardDAVStats *stats = (LinphoneCardDAVStats *)ms_new0(LinphoneCardDAVStats, 1);
	LinphoneVcard *lvc1 =
		linphone_vcard_new_from_vcard4_buffer("BEGIN:VCARD\r\nVERSION:4.0\r\nFN:Margaux "
											  "Clerc\r\nIMPP;TYPE=work:sip:margaux@sip.linphone.org\r\nEND:VCARD\r\n");
...
#endif
	{"CardDAV clean", carddav_clean}, // This is to ensure the content of the test addressbook is in the correct state
...
test_suite_t vcard_test_suite = {"VCard", NULL, NULL, liblinphone_tester_before_each, liblinphone_tester_after_each,

...
static void friends_sqlite_store_lot_of_friends(void) {
	...
	BC_ASSERT_TRUE(ret == SQLITE_OK);
	for (i = 0; i < 20000; i++) {
		buf = sqlite3_mprintf("INSERT INTO friends VALUES(NULL,%u,%Q,%i,%i,'key_%i',%Q,%Q,%Q,%i);", i, "dummy_addr", 0,
							  0, i, NULL, NULL, NULL, 0);
...
static void friends_sqlite_find_friend_in_lot_of_friends(void) {
	...
	for (i = 0; i < 20000; i++) {
		buf = sqlite3_mprintf("SELECT * FROM friends WHERE ref_key LIKE 'key_%i';", i);
...
static void carddav_contact_created(LinphoneFriendList *list, BCTBX_UNUSED(LinphoneFriend *lf)) {
	LinphoneCardDAVStats *stats =
		(LinphoneCardDAVStats *)linphone_friend_list_cbs_get_user_data(linphone_friend_list_get_callbacks(list));
	stats->new_contact_count++;
}
...
static void carddav_sync_status_changed(LinphoneFriendList *list, LinphoneFriendListSyncStatus status,
										const char *msg) {
	LinphoneCardDAVStats *stats =
		(LinphoneCardDAVStats *)linphone_friend_list_cbs_get_user_data(linphone_friend_list_get_callbacks(list));
	char *state = status == LinphoneFriendListSyncStarted
					  ? "Sync started"
...
test_t vcard_tests[] = {
	...
	TEST_NO_TAG("CardDAV clean", carddav_clean), // This is to ensure the content of the test addressbook is in the
												 // correct state for the following tests
	TEST_NO_TAG("CardDAV synchronization", carddav_sync), TEST_NO_TAG("CardDAV synchronization 2", carddav_sync_2),
	TEST_NO_TAG("CardDAV synchronization 3", carddav_sync_3), TEST_NO_TAG("CardDAV synchronization 4", carddav_sync_4),
	TEST_NO_TAG("CardDAV integration", carddav_integration),
	TEST_NO_TAG("CardDAV multiple synchronizations", carddav_multiple_sync),
	TEST_NO_TAG("CardDAV client to server and server to client sync",

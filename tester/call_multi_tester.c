...
static void incoming_call_accepted_when_outgoing_call_in_state(LinphoneCallState state) {
	...
	if (state==LinphoneCallOutgoingRinging || state==LinphoneCallOutgoingEarlyMedia) {
		...
		BC_ASSERT_TRUE(wait_for(marie->lc
									...
									,1));
	BC_ASSERT_TRUE(wait_for(marie->lc, pauline->lc, (int*)&linphone_core_get_tone_manager_stats(marie->lc)->number_of_startRingbackTone, 1));
...
// Pauline accept Marie
	...
	BC_ASSERT_TRUE(wait_for_list(core_list,(int*)&marieToneStats->number_of_startRingtone,1, 5000));
	
// Marie accept Laure	
	linphone_call_accept(marie_called_by_laure);

	BC_ASSERT_TRUE(wait_for_list(core_list,(int*)&marieToneStats->number_of_startRingtone,1, 5000));	// End of named tone

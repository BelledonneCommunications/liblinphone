	...
	*/
	...
	if (pause_and_hangup) {
		if (!is_remote_conf){
			 ...
			 */
			remove_participant_from_local_conference(lcs, marie, laure, TRUE);
...
static void eject_from_3_participants_conference(LinphoneCoreManager *marie, LinphoneCoreManager *pauline, LinphoneCoreManager *laure, LinphoneCoreManager *focus) {
	...
	if(!is_remote_conf) {
		...
		BC_ASSERT_TRUE(wait_for_list(lcs,&marie->stat.number_of_LinphoneCallEnd,initial_marie_stat.number_of_LinphoneCallEnd+2,3000));
	}
end:
	bctbx_list_free(lcs);
}

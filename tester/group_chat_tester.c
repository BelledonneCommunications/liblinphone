...
static LinphoneChatRoom * check_has_chat_room_client_side(bctbx_list_t *lcs, LinphoneCoreManager *lcm, stats *initialStats, const LinphoneAddress *confAddr, const char* subject, int participantNumber, bool_t isAdmin) {
    ...
    if (chatRoom) {
        ...
        if (!(linphone_chat_room_get_capabilities(chatRoom) & LinphoneChatRoomCapabilitiesOneToOne)) {
			if (isAdmin) {
				BC_FAIL("Have to wait for notify to be received to be able to check participant admin state");
				// Wait for notify
				wait_for_list(lcs, 0, 1, 5000);
			}
...
static void group_chat_room_reinvited_after_removed_base (bool_t offline_when_removed, bool_t offline_when_reinvited, bool_t restart_after_reinvited) {
	...
	// Wait for Laure to receive the Notify otherwise below removal won't work
	// See group_chat_room_leave test
...
static void group_chat_room_send_refer_to_all_devices (void) {
	...
	wait_for_list(coresList, 0, 1, 5000);
	BC_FAIL("Check to know if participant is admin is done too quickly, NOTIFY hasn't been received yet");
...
static void group_chat_room_creation_successful_if_at_least_one_invited_participant_supports_it (void) {
	...
	LinphoneChatRoom *marieCr = create_chat_room_client_side_with_expected_number_of_participants(coresList, marie, &initialMarieStats, participantsAddresses, initialSubject, 2, FALSE);
	BC_FAIL("We won't know if a participant can't be added until notify is received !");
...
static void group_chat_room_participant_devices_name (void) {
	...
	BC_FAIL("Have to wait for notify each time we do something!");
	// Wait for notify
	wait_for_list(coresList, NULL, 0, 5000);
	...
	BC_FAIL("Crashes below because marieParticipant is NULL");
	goto end;

...
static void group_chat_room_unique_one_to_one_chat_room_with_forward_message_recreated_from_message_base (bool_t with_app_restart, bool_t forward_message, bool_t reply_message) {
	...
	const bctbx_list_t *reply_contents = linphone_chat_message_get_reply_message_contents(message);
	BC_ASSERT_EQUAL(bctbx_list_size(contents), 1, int , "%d");
	BC_ASSERT_EQUAL(bctbx_list_size(reply_contents), 0, int , "%d");
	BC_ASSERT_TRUE(wait_for_list(coresList, &marie->stat.number_of_LinphoneMessageDelivered, initialMarieStats.number_of_LinphoneMessageDelivered + 1, 5000));
	BC_ASSERT_TRUE(wait_for_list(coresList, &pauline->stat.number_of_LinphoneMessageReceived, initialPaulineStats.number_of_LinphoneMessageReceived + 1, 5000));
	BC_ASSERT_STRING_EQUAL(linphone_chat_message_get_text(pauline->stat.last_received_chat_message), textMessage);
	...
	if (forward_message) {
		...
		if (linphone_chat_room_get_history_size(paulineCr) > 0) {
			...
			LinphoneContent *content = (LinphoneContent *)contents->data;
			...
			reply_contents = linphone_chat_message_get_reply_message_contents(msg);
			BC_ASSERT_EQUAL(bctbx_list_size(reply_contents), 1, int , "%d");

			content = (LinphoneContent *)reply_contents->data;
			...
			BC_ASSERT_STRING_EQUAL(linphone_content_get_utf8_text(content), textMessage);
	...
	if (forward_message) {
		...
		if (linphone_chat_room_get_history_size(marieCr) > 1) {
			...
			reply_contents = linphone_chat_message_get_reply_message_contents(msgFromMarie);
			BC_ASSERT_EQUAL(bctbx_list_size(reply_contents), 1, int , "%d");

			content = (LinphoneContent *)reply_contents->data;
			BC_ASSERT_STRING_EQUAL(linphone_content_get_type(content), "text");
			BC_ASSERT_STRING_EQUAL(linphone_content_get_subtype(content), "plain");
			BC_ASSERT_STRING_EQUAL(linphone_content_get_utf8_text(content), "Quite funny!");

			linphone_chat_message_add_utf8_text_content(msgFromMarie, "Still laughing!");

			contents = linphone_chat_message_get_contents(msgFromMarie);
			// This is a reply to the reply, so the very first content shouldn't be here anymore
			BC_ASSERT_EQUAL(bctbx_list_size(contents), 2, int , "%d");
			reply_contents = linphone_chat_message_get_reply_message_contents(msgFromMarie);
			BC_ASSERT_EQUAL(bctbx_list_size(reply_contents), 1, int , "%d");

			content = (LinphoneContent *)contents->data;
			BC_ASSERT_STRING_EQUAL(linphone_content_get_type(content), "multipart");
			BC_ASSERT_STRING_EQUAL(linphone_content_get_subtype(content), "mixed");

...
static void video_call_dummy_codec(void) {
	...
	if (pauline_call && marie_call ) {
		BC_ASSERT_TRUE(media_stream_get_state(linphone_call_get_stream(pauline_call, LinphoneStreamTypeVideo)) == MSStreamInitialized);
		BC_ASSERT_TRUE(media_stream_get_state(linphone_call_get_stream(marie_call, LinphoneStreamTypeVideo)) == MSStreamInitialized);
	} else {
	...
	wait_for_until(marie->lc, pauline->lc, &dummy, 1, 2000);
	if (pauline_call && marie_call ) {
		BC_ASSERT_TRUE(media_stream_get_state(linphone_call_get_stream(pauline_call, LinphoneStreamTypeVideo)) == MSStreamStarted);
		BC_ASSERT_TRUE(media_stream_get_state(linphone_call_get_stream(marie_call, LinphoneStreamTypeVideo)) == MSStreamStarted);
	} else {
		BC_FAIL("Fail to get current calls");
	}
	end_call(pauline, marie);

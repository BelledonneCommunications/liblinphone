...
static bool_t subscribe_to_callee_presence(LinphoneCoreManager *caller_mgr, LinphoneCoreManager *callee_mgr) {
	...
	*/
	...
	BC_ASSERT_EQUAL(callee_mgr->stat.number_of_NewSubscriptionRequest,
	                initial_callee.number_of_NewSubscriptionRequest + 1, int, "%d");
	/*without proxy, callee cannot subscribe to caller
	BC_ASSERT_EQUAL(callee_mgr->stat.number_of_NotifyPresenceReceived,initial_callee.number_of_NotifyPresenceReceived+1,
	int, "%d");
	*/
...
/* BEWARE this test will fail if the machine it is run on is behind an active firewall not sending ICMP errors on
 * incoming connections! */
/* It will create a leak of NOTIFY because during marie error after network switch off, Pauline will never received the
 * acknowledgement of the notify ...*/
static void subscribe_failure_handle_by_app(void) {
	...
	wait_for(marie->lc, pauline->lc, &pauline->stat.number_of_NewSubscriptionRequest,
	         1); /*just to wait for unsubscription even if not notified*/
	...
	BC_ASSERT_TRUE(
	    wait_for(marie->lc, pauline->lc, &marie->stat.number_of_LinphoneRegistrationOk, 2)); /*wait for register ok*/
	linphone_friend_edit(lf);
	linphone_friend_enable_subscribes(lf, TRUE);
	linphone_friend_done(lf);
	BC_ASSERT_FALSE(wait_for(marie->lc, pauline->lc, &pauline->stat.number_of_NewSubscriptionRequest,
	...
	BC_ASSERT_FALSE(wait_for(NULL, pauline->lc, &pauline->stat.number_of_NewSubscriptionRequest,
	                         3)); /*just to wait for unsubscription even if not notified*/
...
static void simple_subscribe_with_early_notify(void) {
	...
	linphone_address_set_port(marie_identity_addr,
	                          0); /*we remove the port because friends aren't supposed to have port numbers*/
...
static void presence_information(void) {
	...
	presence = linphone_presence_model_new_with_activity_and_note(LinphonePresenceActivityVacation, NULL, vacation_note,
	                                                              vacation_lang);
	linphone_core_set_presence_model(pauline->lc, presence);
	linphone_presence_model_unref(presence);

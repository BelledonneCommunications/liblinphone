...
static void record_file(const char *filename, BCTBX_UNUSED(bool_t supported_format), BCTBX_UNUSED(const char *audio_mime), const char *video_mime, LinphoneRecorderFileFormat format, bool_t use_evfs) {
...
static void record_wav_pcm_test(void) {
	...
	record_file(filename, TRUE, audio_mime, video_mime, LinphoneRecorderFileFormatWav, FALSE);
	ms_free(filename);
}
...
static void record_mkv_opus_h264_test(void) {
	...
	record_file(filename, linphone_recorder_matroska_supported(), audio_mime, video_mime, LINPHONE_RECORDER_FORMAT_MATROSKA, FALSE);
	ms_free(filename);
#endif
}
...
static void record_mkv_opus_vp8_test(void) {
	...
	record_file(filename, linphone_recorder_matroska_supported(), audio_mime, video_mime, LinphoneRecorderFileFormatMkv, FALSE);
	ms_free(filename);
}
...
static void record_mkv_opus_vp8_evfs_test(void) {
	...
	const char *audio_mime = "opus";
	const char *video_mime = "vp8";
	record_file(filename, linphone_recorder_matroska_supported(), audio_mime, video_mime, LinphoneRecorderFileFormatMkv, TRUE);
	ms_free(filename);
}

...
static void create_conference_with_only_speakers_and_uninvited_participant(void) {
	create_conference_base(ms_time(NULL), -1, TRUE, LinphoneConferenceParticipantListTypeOpen, FALSE,
	                       ...
	                       LinphoneConferenceSecurityLevelNone, {LinphoneParticipantRoleSpeaker});
}
...
void create_conference_with_recvonly_participant(void) {
	...
	LinphoneConferenceSecurityLevel security_level = LinphoneConferenceSecurityLevelNone;
							...
							if (pconference) {
								...
								if (p) {
									...
									for (bctbx_list_t *itd = devices; itd; itd = bctbx_list_next(itd)) {
										...
										LinphoneMediaDirection expected_video_direction = linphone_address_weak_equal(linphone_participant_device_get_address(d), paulineAddr.toC()) ? new_video_direction : LinphoneMediaDirectionSendRecv;
										if (enable == TRUE) {
		...
		for (auto mgr : allMembers) {
			...
			if (call_logs) {
				BC_ASSERT_EQUAL(bctbx_list_size(call_logs), 1,
				                size_t, "%zu");
			}
...
static test_t local_conference_scheduled_conference_basic_tests[] = {
    ...
    TEST_NO_TAG("Create conference with recvonly participant", LinphoneTest::create_conference_with_recvonly_participant), 

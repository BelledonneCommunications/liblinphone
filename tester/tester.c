...
bool_t call_with_params2(LinphoneCoreManager* caller_mgr
	...
	int stopToneCountInit = callee_stats->number_of_stopRingtone + callee_stats->number_of_stopTone;
	...
	if (callee_stats->number_of_startRingbackTone == callee_stats->number_of_stopRingbackTone)
		BC_ASSERT_EQUAL(callee_stats->number_of_stopRingtone + callee_stats->number_of_stopTone - stopToneCountInit , 1, int, "%d");
	else// in this case, the call is currently in RingbackTone so the Ringtone didn't start and stop didn't change

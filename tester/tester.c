...
const char* flexisip_tester_dns_server = "fs-test-6.linphone.org";
...
void reset_counters( stats* counters) {
	...
	if (counters->last_fail_to_decrypt_received_chat_message) linphone_chat_message_unref(counters->last_fail_to_decrypt_received_chat_message);
...
#define REGISTER_TIMEOUT 20 /* seconds */
		int success = wait_for_until(mgr->lc,NULL,&mgr->stat.number_of_LinphoneRegistrationOk,
									mgr->stat.number_of_LinphoneRegistrationOk + proxy_count,(REGISTER_TIMEOUT * 1000 * proxy_count));
		if( !success ){
...
void linphone_notify_received(LinphoneCore *lc, LinphoneEvent *lev, BCTBX_UNUSED(const char *eventname), const LinphoneContent *content){
	...
	if (content && !linphone_content_is_multipart(content) && (!ua ||  !strstr(ua, "flexisip"))) { /*disable check for full presence server support*/
...
void linphone_call_send_master_key_changed(LinphoneCore *lc, BCTBX_UNUSED(LinphoneCall *call), BCTBX_UNUSED(const char *master_key)) {
	stats* counters = get_stats(lc);
	counters->number_of_LinphoneCallSendMasterKeyChanged++;
}

void linphone_call_receive_master_key_changed(LinphoneCore *lc, BCTBX_UNUSED(LinphoneCall *call), BCTBX_UNUSED(const char *master_key)) {
	stats* counters = get_stats(lc);

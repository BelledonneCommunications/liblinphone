...
static void check_participant_added_to_conference(bctbx_list_t *lcs, LinphoneCoreManager * conf_mgr, stats conf_initial_stats, bctbx_list_t *new_participants, stats* new_participant_initial_stats, bool_t * is_call_paused, bctbx_list_t *participants, stats* participant_initial_stats) {
	...
	for (bctbx_list_t *it = new_participants; it; it = bctbx_list_next(it)) {
		...
		if (event_log_enabled) {
			...
			BC_ASSERT_TRUE(wait_for_list(lcs,&conf_mgr->stat.number_of_LinphoneSubscriptionIncomingReceived,(conf_initial_stats.number_of_LinphoneSubscriptionIncomingReceived + idx - no_participants_without_event_log + 1),1000));
...
static LinphoneStatus check_participant_removal(bctbx_list_t * lcs, LinphoneCoreManager * conf_mgr, LinphoneCoreManager * participant_mgr, bctbx_list_t *participants, int participant_size, stats conf_initial_stats, stats participant_initial_stats, stats * participants_initial_stats, bool_t local_participant_is_in) {
	...
	if (participants != NULL) {
		...
		for (bctbx_list_t *itm = participants; itm; itm = bctbx_list_next(itm)) {
			...
			const LinphoneAddress *participant_uri = m->identity;
			LinphoneCall * participant_call = linphone_core_get_call_by_remote_address(conf_mgr->lc, linphone_address_as_string(participant_uri));
			BC_ASSERT_PTR_NOT_NULL(participant_call);
...
LinphoneStatus remove_participant_from_local_conference(bctbx_list_t *lcs, LinphoneCoreManager * conf_mgr, LinphoneCoreManager * participant_mgr) {
	...
	check_participant_removal(lcs, conf_mgr, participant_mgr, participants, participant_size, conf_initial_stats, participant_initial_stats, participants_initial_stats, local_participant_is_in);
...
bctbx_list_t* terminate_participant_call(bctbx_list_t *participants, LinphoneCoreManager * conf_mgr, LinphoneCoreManager * participant_mgr) {
	...
	LinphoneCall * participant_call = linphone_core_get_call_by_remote_address(participant_mgr->lc, linphone_address_as_string(conf_mgr->identity));
	BC_ASSERT_PTR_NOT_NULL(participant_call);
	...
	if (participant_call) {
		...
		check_participant_removal(lcs, conf_mgr, participant_mgr, participants, participant_size, initial_conf_stats, initial_participant_stats, initial_other_participants_stats, local_participant_is_in);
...
LinphoneStatus terminate_local_conference(bctbx_list_t *participants, LinphoneCoreManager * conf_mgr) {

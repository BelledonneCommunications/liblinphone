...
void liblinphone_tester_chat_message_state_change(LinphoneChatMessage* msg,LinphoneChatMessageState state,UNUSED(void* ud)) {
...
static void text_message_with_credential_from_auth_cb_auth_info_requested(LinphoneCore *lc, const char *realm, const char *username, UNUSED(const char *domain)) {
...
static int enable_lime_for_message_test(UNUSED(LinphoneCoreManager *marie), UNUSED(LinphoneCoreManager *pauline)) {
#ifdef HAVE_SQLITE
	char* stmt = NULL;
	char* errmsg=NULL;
	int ret = 0;
...
int check_no_strange_time(UNUSED(void* data),int argc, char** argv,char** cNames) {
...
static void real_time_text_conversation(void) {
	...
	if (BC_ASSERT_TRUE(call_with_caller_params(marie, pauline, marie_params))){
		...
		if (pauline_chat_room && marie_chat_room) {
			...
			BC_ASSERT_TRUE(wait_for_until(pauline->lc, marie->lc, &marie->stat.number_of_LinphoneIsComposingActiveReceived, (int)strlen(message1_1) + 1, 5000));
			BC_ASSERT_EQUAL(linphone_chat_room_get_char(marie_chat_room), (char)0x2028, char, "%c");
			BC_ASSERT_TRUE(wait_for_until(pauline->lc, marie->lc, &pauline->stat.number_of_LinphoneIsComposingActiveReceived, (int)strlen(message1_2) + 1, 5000));
			...
			// Read new line character
			BC_ASSERT_TRUE(wait_for_until(pauline->lc, marie->lc, &marie->stat.number_of_LinphoneIsComposingActiveReceived, (int)strlen(message2_1) + 1, 5000));
			BC_ASSERT_EQUAL(linphone_chat_room_get_char(marie_chat_room), (char)0x2028, char, "%c");
			BC_ASSERT_TRUE(wait_for_until(pauline->lc, marie->lc, &pauline->stat.number_of_LinphoneIsComposingActiveReceived, (int)strlen(message2_2) + 1, 5000));
			BC_ASSERT_EQUAL(linphone_chat_room_get_char(pauline_chat_room), (char)0x2028, char, "%c");
...
static int im_encryption_engine_process_outgoing_message_async(UNUSED(LinphoneImEncryptionEngine *engine), UNUSED(LinphoneChatRoom *room), LinphoneChatMessage *msg) {
	if (strcmp(linphone_chat_message_get_content_type(msg),"cipher/b64") == 0)
...
static int im_encryption_engine_process_incoming_message_async(UNUSED(LinphoneImEncryptionEngine *engine), LinphoneChatRoom *room, LinphoneChatMessage *msg) {

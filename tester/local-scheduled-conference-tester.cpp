...
static void conference_with_media_lost_base(bool_t server_drops) {
	...
	{ // to make sure focus is destroyed after clients.
		...
		for (auto mgr : members) {
			...
			if (pcall) {
				linphone_call_set_video_source(pcall, descriptor);
...
static void conference_with_server_no_rtp_timeout() {
	Focus focus("chloe_rc");
	{ // to make sure focus is destroyed after clients.
		const LinphoneTesterLimeAlgo lime_algo = UNSET;
		...
		ClientConference lise("lise_rc", focus.getConferenceFactoryAddress(), lime_algo);
		...
		int server_nortp_timeout = 10;
		linphone_core_set_nortp_timeout(focus.getLc(), server_nortp_timeout);
		linphone_core_set_file_transfer_server(marie.getLc(), file_transfer_url);
		...
		std::list<LinphoneCoreManager *> participants{laure.getCMgr(), michelle.getCMgr(), berthe.getCMgr(),
		                                              lise.getCMgr(), pauline.getCMgr()};
		std::list<LinphoneCoreManager *> conferenceMgrs{focus.getCMgr(), berthe.getCMgr()};
		std::list<LinphoneCoreManager *> members{berthe.getCMgr()};
		...
		// Chat room creation to send ICS
		BC_ASSERT_TRUE(wait_for_list(coresList, &marie.getStats().number_of_LinphoneChatRoomStateCreated, 5,
		                             liblinphone_tester_sip_timeout));
		...
		for (auto mgr : members) {
			...
			BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_LinphoneChatRoomStateCreated,
			                             ((mgr == marie.getCMgr()) ? nb_members : 1), liblinphone_tester_sip_timeout));
		...
		simparams.latency = 600;
		simparams.loss_rate = 100;
		linphone_core_set_network_simulator_params(berthe.getLc(), &simparams);
		linphone_core_set_network_simulator_params(focus.getLc(), &simparams);
		...
		linphone_call_params_set_media_encryption(berthe_new_params, encryption);
		linphone_call_params_set_account(berthe_new_params, linphone_core_get_default_account(berthe.getLc()));
		linphone_call_params_enable_video(berthe_new_params, TRUE);
		ms_message("%s is entering again conference %s", linphone_core_get_identity(berthe.getLc()),
		...
		if (berthe_call) {
			LinphoneCallLog *call_log = linphone_call_get_call_log(berthe_call);
			BC_ASSERT_TRUE(linphone_call_log_was_conference(call_log));
		}

		BC_ASSERT_TRUE(wait_for_list(coresList, &berthe.getStats().number_of_LinphoneCallStreamsRunning,
		                             initialBertheStats.number_of_LinphoneCallStreamsRunning + 1,
		                             liblinphone_tester_sip_timeout));
		...
		ms_message("%s shuts down its network", linphone_core_get_identity(berthe.getLc()));
		linphone_core_set_network_reachable(berthe.getLc(), FALSE);
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneCallEnd,
		                             focus_stat.number_of_LinphoneCallEnd + 1, (server_nortp_timeout + 1) * 1000));
		...
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneConferenceStateTerminated,
		                             focus_stat.number_of_LinphoneConferenceStateTerminated,
		                             liblinphone_tester_sip_timeout));
		...
		CoreManagerAssert({focus, marie, pauline, laure, michelle, berthe, lise}).waitUntil(chrono::seconds(2), [] {
			return false;
		});
...
static void create_simple_conference_with_screen_sharing_and_chat_rejoining_without_screen_sharing() {
	create_conference_with_screen_sharing_chat_base(ms_time(NULL), -1, LinphoneConferenceSecurityLevelNone,
	                                                LinphoneConferenceLayoutActiveSpeaker, FALSE, FALSE);
}

static void create_simple_conference_with_screen_sharing_and_chat_rejoining_with_screen_sharing() {
	create_conference_with_screen_sharing_chat_base(ms_time(NULL), -1, LinphoneConferenceSecurityLevelNone,
	                                                LinphoneConferenceLayoutActiveSpeaker, FALSE, TRUE);
}
...
static void create_simple_conference_with_chat_server_changing_gruu() {
	Focus focus("chloe_rc");
	{ // to make sure focus is destroyed after clients.
		ClientConference marie("marie_rc", focus.getConferenceFactoryAddress());
		ClientConference pauline("pauline_rc", focus.getConferenceFactoryAddress());
		...
		time_t start_time = ms_time(NULL);
		time_t end_time = -1;
		const char *initialSubject = "Server changing GRUU";
		const char *description = "Be careful - the server keeps changing GRUU";
		...
		for (int attempt = 0; attempt < max_attempts; attempt++) {
			...
			for (auto mgr : members) {
				...
				std::list<std::pair<LinphoneCoreManager *, stats>> member_stats_list{std::make_pair(mgr, mgr->stat)};
				LinphoneCallParams *new_params = linphone_core_create_call_params(mgr->lc, nullptr);
				linphone_call_params_set_media_encryption(new_params, encryption);
				linphone_call_params_set_video_direction(new_params, LinphoneMediaDirectionSendRecv);
			...
			// Wait a little bit
			CoreManagerAssert({focus, marie, pauline}).waitUntil(chrono::seconds(3), [] { return false; });
			...
			for (auto mgr : members) {
				...
				BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_LinphoneCallEnd,
				                             mgr_stat.number_of_LinphoneCallEnd + 1, liblinphone_tester_sip_timeout));
				BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_LinphoneCallReleased,
				                             mgr_stat.number_of_LinphoneCallReleased + 1,
				                             liblinphone_tester_sip_timeout));
				BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_LinphoneSubscriptionTerminated,
				                             mgr_stat.number_of_LinphoneSubscriptionTerminated + 1,
				                             liblinphone_tester_sip_timeout));
				BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_LinphoneConferenceStateTerminationPending,
				                             mgr_stat.number_of_LinphoneConferenceStateTerminationPending + 1,
				                             liblinphone_tester_sip_timeout));
				BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_LinphoneConferenceStateTerminated,
				                             mgr_stat.number_of_LinphoneConferenceStateTerminated + 1,
				                             liblinphone_tester_sip_timeout));
				...
				BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneCallReleased,
				                             ...
				                             liblinphone_tester_sip_timeout));
				BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneSubscriptionTerminated,
				                             focus_stat2.number_of_LinphoneSubscriptionTerminated + 1,
				                             liblinphone_tester_sip_timeout));
			}
			...
			if (attempt < (max_attempts - 1)) {
				...
				focus_stat = focus.getStats();
				const bctbx_list_t *focus_account_list = linphone_core_get_account_list(focus.getLc());
				size_t focus_account_count = bctbx_list_size(focus_account_list);
				for (const bctbx_list_t *focus_account_it = focus_account_list; focus_account_it != NULL;
				     focus_account_it = focus_account_it->next) {
					LinphoneAccount *focus_account = (LinphoneAccount *)focus_account_it->data;
					LinphoneAccountParams *params =
					    linphone_account_params_clone(linphone_account_get_params(focus_account));
				...
				BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneRegistrationCleared,
				                             focus_stat.number_of_LinphoneRegistrationCleared +
	...
	{ // to make sure focus is destroyed after clients.
		...
		for (auto mgr : members) {
			...
			if (mgr_call) {
				...
				LinphoneVideoSourceDescriptor *descriptor = linphone_video_source_descriptor_new();
				if (is_first_member) {
					linphone_video_source_descriptor_set_screen_sharing(descriptor,
					                                                    LinphoneVideoSourceScreenSharingArea, NULL);
				} else {
					linphone_video_source_descriptor_set_camera_id(descriptor, liblinphone_tester_static_image_id);
				}
				linphone_call_set_video_source(mgr_call, descriptor);
				linphone_video_source_descriptor_unref(descriptor);
			}
			...
			if (is_first_member) {
				BC_ASSERT_TRUE(check_screen_sharing_call(focus.getCMgr(), mgr, true));
...
void create_conference_with_recvonly_participant(void) {
	Focus focus("chloe_rc");
	{ // to make sure focus is destroyed after clients.
		ClientConference marie("marie_rc", focus.getConferenceFactoryAddress());
		ClientConference pauline("pauline_rc", focus.getConferenceFactoryAddress());
		...
		for (int i = 0; i < 4; i++) {
			...
			try {
				LinphoneMediaDirection video_direction = LinphoneMediaDirectionSendRecv;
				LinphoneMediaDirection new_video_direction = LinphoneMediaDirectionSendRecv;
				if (pauline_call) {
				...
				if (video_direction == LinphoneMediaDirectionRecvOnly) {
					...
					new_video_direction = LinphoneMediaDirectionRecvOnly;
				}
				...
				if (pauline_call) {
					...
					for (auto mgr : conferenceMgrs) {
						const LinphoneAddress *uri = (mgr == focus.getCMgr()) ? confAddr : mgr->identity;
						LinphoneConference *pconference =
						...
						if (pconference) {
							...
							if (p) {
								...
								for (bctbx_list_t *itd = devices; itd; itd = bctbx_list_next(itd)) {
									...
									if (enable == TRUE) {
										expected_video_direction = new_video_direction;
									} else {
					...
					int negotiated_video_enabled = enable;
					...
					if (pauline_call) {
						...
						BC_ASSERT_EQUAL(linphone_call_params_video_enabled(call_rparams), negotiated_video_enabled, int,
						                "%0d");
						const LinphoneCallParams *call_cparams = linphone_call_get_current_params(pauline_call);
						BC_ASSERT_EQUAL(linphone_call_params_video_enabled(call_cparams), negotiated_video_enabled, int,
						                "%0d");
					}
				...
				if (paulineConference) {
					...
					if (new_video_direction == LinphoneMediaDirectionRecvOnly) {
		...
		std::list<LinphoneCoreManager *> mgrsToRemove{pauline.getCMgr(), laure.getCMgr()};
...
static void create_scheduled_conference_and_chatroom_using_the_same_address() {
	Focus focus("chloe_rc");
	{ // to make sure focus is destroyed after clients.
		ClientConference marie("marie_rc", focus.getConferenceFactoryAddress());
		ClientConference pauline("pauline_rc", focus.getConferenceFactoryAddress());
		...
		marie_stat = marie.getStats();
		ms_message("%s is creating a chatroom by copying parameters from conference %s",
		           linphone_core_get_identity(marie.getLc()), conference_address_str);
		LinphoneConferenceParams *marie_params =
		    linphone_core_create_conference_params_2(marie.getLc(), marie_conference);
		linphone_conference_params_enable_audio(marie_params, FALSE);
		linphone_conference_params_enable_video(marie_params, FALSE);
		linphone_conference_params_enable_chat(marie_params, TRUE);
		linphone_conference_params_enable_group(marie_params, FALSE);
		const char *subject = "Redundant chatroom";
		...
		participantsAddresses = bctbx_list_append(participantsAddresses, linphone_address_ref(confAddr));
		LinphoneChatRoom *marieCr = linphone_core_create_chat_room_7(marie.getLc(), marie_params, NULL);
		...
		auto nb_participants = static_cast<int>(members.size());
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneCallEnd,
		                             focus_stat.number_of_LinphoneCallEnd + nb_participants,
		                             liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneCallReleased,
		                             focus_stat.number_of_LinphoneCallReleased + nb_participants,
		                             liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneSubscriptionTerminated,
		                             focus_stat.number_of_LinphoneSubscriptionTerminated + nb_participants,
		                             liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_participants_removed,
		                             focus_stat.number_of_participants_removed + nb_participants,
		                             liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_participant_devices_removed,
		                             focus_stat.number_of_participant_devices_removed + nb_participants,
		                             liblinphone_tester_sip_timeout));
...
static void create_conference_with_chat_client_enters_conference() {
	Focus focus("chloe_rc");
	{ // to make sure focus is destroyed after clients.
		const LinphoneTesterLimeAlgo lime_algo = UNSET;
		...
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneSubscriptionIncomingReceived,
		                             ...
		                             liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneSubscriptionActive,
		                             focus_stat.number_of_LinphoneSubscriptionActive + (4 * nb_subscriptions),
		                             liblinphone_tester_sip_timeout));
		...
		std::map<LinphoneCoreManager *, LinphoneParticipantInfo *> memberList =
		    fill_member_list(participants, participantList, marie.getCMgr(), participants_info);
		wait_for_conference_streams({focus, marie, pauline, laure, michelle, berthe}, conferenceMgrs, focus.getCMgr(),
		                            memberList, confAddr, TRUE);

		LinphoneCallParams *marie_new_params = linphone_core_create_call_params(marie.getLc(), nullptr);
		ms_message("%s is entering conference %s", linphone_core_get_identity(marie.getLc()), conference_address_str);
		linphone_core_invite_address_with_params_2(marie.getLc(), confAddr, marie_new_params, nullptr, nullptr);
		linphone_call_params_unref(marie_new_params);
		LinphoneCall *marie_call = linphone_core_get_call_by_remote_address2(marie.getLc(), confAddr);
		BC_ASSERT_PTR_NOT_NULL(marie_call);
		if (marie_call) {
			LinphoneCallLog *call_log = linphone_call_get_call_log(marie_call);
			BC_ASSERT_TRUE(linphone_call_log_was_conference(call_log));
		}

		BC_ASSERT_TRUE(wait_for_list(coresList, &marie.getStats().number_of_LinphoneCallOutgoingProgress, 1,
		                             liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(wait_for_list(coresList, &marie.getStats().number_of_LinphoneCallStreamsRunning, 1,
		                             liblinphone_tester_sip_timeout));
		...
		focus_stat = focus.getStats();
		marie_stat = marie.getStats();
		ms_message("%s is entering again conference %s", linphone_core_get_identity(marie.getLc()),
		           conference_address_str);
		linphone_conference_enter(marie_conference);
		marie_call = linphone_core_get_call_by_remote_address2(marie.getLc(), confAddr);
		BC_ASSERT_PTR_NOT_NULL(marie_call);
		if (marie_call) {
			LinphoneCallLog *call_log = linphone_call_get_call_log(marie_call);
			BC_ASSERT_TRUE(linphone_call_log_was_conference(call_log));
		...
		conferenceMgrs.push_back(marie.getCMgr());
		memberList = fill_member_list(members, participantList, marie.getCMgr(), participants_info);
		wait_for_conference_streams({focus, marie, pauline, laure, michelle, berthe}, conferenceMgrs, focus.getCMgr(),
		                            memberList, confAddr, TRUE);
...
static void rejoining_conference_after_expiration_before_cleanup() {
	...
	{ // to make sure focus is destroyed after clients.
		...
		linphone_core_enable_gruu_in_conference_address(focus.getLc(), TRUE);
		linphone_core_enable_gruu_in_conference_address(marie.getLc(), FALSE);
		linphone_core_enable_gruu_in_conference_address(pauline.getLc(), FALSE);
		...
		for (auto mgr : {marie.getCMgr(), pauline.getCMgr(), laure.getCMgr()}) {
			...
			if (pinfo) {
				...
				bool account_for_conference_settings = (can_be_joined || (mgr == marie.getCMgr()));
				check_conference_info_members(pinfo, NULL, confAddr, marie.getCMgr()->identity, participants_info2,
				                              ...
				                              account_for_conference_settings ? enable_video : TRUE, FALSE);
				bctbx_list_free_with_data(participants_info2, (bctbx_list_free_func)linphone_participant_info_unref);
...
static void conference_with_participant_dialing_in_outside_valid_time_slot(
    bool_t before_start, long availability_before, long expiry_after, bool_t enable_chat, bool_t server_restart) {
	Focus focus("chloe_rc");
	{ // to make sure focus is destroyed after clients.
		ClientConference marie("marie_rc", focus.getConferenceFactoryAddress());
		ClientConference pauline("pauline_rc", focus.getConferenceFactoryAddress());
		...
		if (!!server_restart) {
			size_t nbChatRooms = (before_start || (expiry_after > 0)) ? 1 : 0;
...
static void conference_with_participants_dialing_in_after_end() {
	conference_with_participant_dialing_in_outside_valid_time_slot(FALSE, 0, 0, FALSE, FALSE);
}

static void conference_with_chat_and_participants_dialing_in_after_end() {
	conference_with_participant_dialing_in_outside_valid_time_slot(FALSE, 0, 0, TRUE, TRUE);
}

static void conference_with_participants_dialing_in_before_start() {
	conference_with_participant_dialing_in_outside_valid_time_slot(TRUE, 0, 0, FALSE, FALSE);
}

static void create_scheduled_conference_joined_after_end() {
	conference_with_participant_dialing_in_outside_valid_time_slot(FALSE, 0, 100, FALSE, FALSE);
}

static void create_scheduled_conference_joined_before_start() {
	conference_with_participant_dialing_in_outside_valid_time_slot(TRUE, 100, 0, FALSE, FALSE);
}

static void create_always_available_scheduled_conference_joined_before_start() {
	conference_with_participant_dialing_in_outside_valid_time_slot(TRUE, -1, 0, FALSE, FALSE);
}

static void create_never_ending_scheduled_conference_joined_after_end() {
	conference_with_participant_dialing_in_outside_valid_time_slot(FALSE, 0, -1, FALSE, FALSE);
}
...
static void rejoining_conference_after_end(int cleanup_period, bool_t enable_chat) {
	Focus focus("chloe_rc");
	{ // to make sure focus is destroyed after clients.
		ClientConference marie("marie_rc", focus.getConferenceFactoryAddress());
		ClientConference pauline("pauline_rc", focus.getConferenceFactoryAddress());
		...
		linphone_core_set_file_transfer_server(marie.getLc(), file_transfer_url);
		linphone_core_set_conference_participant_list_type(focus.getLc(), LinphoneConferenceParticipantListTypeOpen);
		linphone_core_set_conference_cleanup_period(focus.getLc(), cleanup_period);
		...
		for (auto iter = 0; iter < 2; iter++) {
			...
			if (confAddr) {
				for (auto mgr : members) {
					...
					check_conference_info_in_db(mgr, NULL, confAddr, marie.getCMgr()->identity, participants_info,
					                            ...
					                            ((mgr == marie.getCMgr()) || (iter > 0)) ? enable_chat : FALSE);
			...
			add_participant_info_to_list(&participants_info, marie.getCMgr()->identity, LinphoneParticipantRoleSpeaker,
			                             0);
			for (auto mgr : participants) {
				BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_LinphoneCallStreamsRunning, (iter + 1),
				                             liblinphone_tester_sip_timeout));
				BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_LinphoneCallStreamsRunning, 2,
				                             liblinphone_tester_sip_timeout));
			...
			BC_ASSERT_EQUAL(focusMainDb->getChatRoomCount(), nbChatRooms, size_t, "%zu");
			...
			for (auto mgr : members) {
				...
				if (call) {
					...
					linphone_call_terminate(call);
					BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_LinphoneCallEnd,
					                             mgr_stat.number_of_LinphoneCallEnd + 1,
					                             liblinphone_tester_sip_timeout));
					BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_LinphoneCallReleased,
					                             mgr_stat.number_of_LinphoneCallReleased + 1,
					                             liblinphone_tester_sip_timeout));
					...
					if (!enable_chat) {
						BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_LinphoneConferenceStateDeleted,
						                             mgr_stat.number_of_LinphoneConferenceStateDeleted + 1,
						                             liblinphone_tester_sip_timeout));
					}
		...
		if (cleanup_period > 0) {
			marie_stat = marie.getStats();
			ms_message("%s is calling expired conference %s before the server restarts",
			           linphone_core_get_identity(marie.getLc()), conference_address_str);
			LinphoneCallParams *new_params = linphone_core_create_call_params(marie.getLc(), nullptr);
			linphone_call_params_enable_video(new_params, FALSE);
			linphone_core_invite_address_with_params_2(marie.getLc(), confAddr, new_params, NULL, nullptr);
			...
			focus.reStart();
			coresList = bctbx_list_append(coresList, focus.getLc());
			auto &focusMainDbAfterRestart = L_GET_PRIVATE_FROM_C_OBJECT(focus.getLc())->mainDb;
			BC_ASSERT_EQUAL(focusMainDbAfterRestart->getChatRoomCount(), nbChatRooms, size_t, "%zu");
			BC_ASSERT_EQUAL(focus.getCore().getChatRooms().size(), nbChatRooms, size_t, "%zu");

			ms_message("%s is calling expired conference %s after the server restarts",
			           linphone_core_get_identity(marie.getLc()), conference_address_str);
			marie_stat = marie.getStats();
			new_params = linphone_core_create_call_params(marie.getLc(), nullptr);
			linphone_call_params_enable_video(new_params, FALSE);
			linphone_core_invite_address_with_params_2(marie.getLc(), confAddr, new_params, NULL, nullptr);
			linphone_call_params_unref(new_params);
...
static void rejoining_conference_after_end_with_cleanup_period() {
	rejoining_conference_after_end(10, FALSE);
}

static void rejoining_conference_with_chat_after_end_with_cleanup_period() {
	rejoining_conference_after_end(10, TRUE);
}

static void rejoining_conference_after_end_without_cleanup_period() {
	rejoining_conference_after_end(-1, FALSE);
}
...
static void create_conference_with_chat_with_server_restarted_before_conference_expires() {
	...
	{ // to make sure focus is destroyed after clients.
		...
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneConferenceStateDeleted,
		                             focusDeletedCount + 1, liblinphone_tester_sip_timeout));
		LinphoneConference *fconference = linphone_core_search_conference_2(focus.getLc(), confAddr);
...
static void clients_change_layout_multiple_times() {
	Focus focus("chloe_rc");
	{ // to make sure focus is destroyed after clients.
		const LinphoneTesterLimeAlgo lime_algo = UNSET;
		...
		std::list<LinphoneCoreManager *> conferenceMgrs{focus.getCMgr(),  laure.getCMgr(),  michelle.getCMgr(),
		                                                ...
		                                                pauline.getCMgr()};
		std::list<LinphoneCoreManager *> members{michelle.getCMgr(), berthe.getCMgr(), lise.getCMgr(),
		                                         laure.getCMgr(),    marie.getCMgr(),  pauline.getCMgr()};
		...
		std::map<LinphoneCoreManager *, LinphoneParticipantInfo *> memberList =
		    fill_member_list(members, participantList, marie.getCMgr(), participants_info);
		wait_for_conference_streams({focus, marie, pauline, laure, michelle, berthe, lise}, conferenceMgrs,
		                            focus.getCMgr(), memberList, confAddr, FALSE);

		std::list<LinphoneCoreManager *> recvonly_mgrs{marie.getCMgr(), laure.getCMgr(), lise.getCMgr()};
		std::list<LinphoneConferenceLayout> layout_sequence{
		    LinphoneConferenceLayoutGrid, LinphoneConferenceLayoutActiveSpeaker, LinphoneConferenceLayoutGrid,
		...
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneCallEnd,
		                             focus_stat.number_of_LinphoneCallEnd + nb_members,
		                             liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneCallReleased,
		                             focus_stat.number_of_LinphoneCallReleased + nb_members,
		                             liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneSubscriptionTerminated,
		                             focus_stat.number_of_LinphoneSubscriptionTerminated + nb_members,
		                             liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_participants_removed,
		                             focus_stat.number_of_participants_removed + nb_members,
		                             liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_participant_devices_removed,
		                             focus_stat.number_of_participant_devices_removed + nb_members,
		                             liblinphone_tester_sip_timeout));
...
static test_t local_conference_scheduled_conference_advanced_tests[] = {
    ...
    TEST_NO_TAG("Create scheduled conference without chat and a one-on-one chatroom with the same address",
                LinphoneTest::create_scheduled_conference_and_chatroom_using_the_same_address),
    TEST_NO_TAG("Clients change layout multiple times", LinphoneTest::clients_change_layout_multiple_times),
...
static test_t local_conference_scheduled_conference_audio_only_participant_tests[] = {
    ...
    TEST_NO_TAG("Rejoining conference after end with cleanup period",
                LinphoneTest::rejoining_conference_after_end_with_cleanup_period),
    TEST_NO_TAG("Rejoining conference after end without cleanup period",
                LinphoneTest::rejoining_conference_after_end_without_cleanup_period),
...
static test_t local_conference_scheduled_conference_with_chat_tests[] = {
    ...
    TEST_NO_TAG("Create conference with chat where a client enters a conference through the conference API",
                LinphoneTest::create_conference_with_chat_client_enters_conference),
    TEST_NO_TAG("Create simple conference with chat and server changing GRUU",
                LinphoneTest::create_simple_conference_with_chat_server_changing_gruu),
    TEST_NO_TAG("Conference with chat and participants dialing in after conference ends",
...
test_suite_t local_conference_test_suite_scheduled_conference_with_chat = {
    ...
    0,
    6 /*cpu_weight : video conference uses more resources */
};

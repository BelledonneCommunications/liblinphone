...
static void participants_exit_conference_after_pausing(void) {
	...
	// Pause pauses that participants leave the conference temporarely
	...
	BC_ASSERT_TRUE(linphone_core_is_in_conference(marie->lc));
	BC_ASSERT_EQUAL(linphone_core_get_conference_size(marie->lc),conf_size-1, int, "%d");
	...
	BC_ASSERT_TRUE(linphone_core_is_in_conference(marie->lc));
	BC_ASSERT_EQUAL(linphone_core_get_conference_size(marie->lc),conf_size-2, int, "%d");
...
static void simple_participant_leaves_conference_base(bool_t remote_participant_leaves) {
	...
	if (remote_participant_leaves) {
		...
		if (pauline_conference) {
			...
			if (marie_conference) {
				BC_ASSERT_EQUAL(linphone_conference_get_participant_count(marie_conference),1, int, "%d");
		...
		if (marie_conference) {
			...
			if (pauline_conference) {
				...
				BC_ASSERT_EQUAL(linphone_conference_get_size(pauline_conference),linphone_conference_get_participant_count(pauline_conference)+linphone_conference_is_in(pauline_conference), int, "%d");
	...
	BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_NotifyReceived,(michelle_stats.number_of_NotifyReceived + ((remote_participant_leaves) ? 2 : 1)),5000));
	if (michelle_conference) {
...
static void participant_leaves_conference_base(bool_t remote_participant_leaves, bool_t add_participant, bool_t wait_for_updates) {
	...
	int expectedNotify = ((remote_participant_leaves) ? 3 : 1);
	...
	if (michelle_conference) {
		BC_ASSERT_EQUAL(linphone_conference_get_participant_count(michelle_conference),((remote_participant_leaves) ? 0 : 1), int, "%d");
		BC_ASSERT_EQUAL(linphone_conference_get_size(michelle_conference),linphone_conference_get_participant_count(michelle_conference)+linphone_conference_is_in(michelle_conference), int, "%d");
		BC_ASSERT_TRUE(linphone_conference_is_in(michelle_conference));
	}
	...
	if (marie_conference) {
		...
		// Marie (the local participant) enters the conference
			linphone_conference_enter(marie_conference);
			int conf_parts_no = (1 + ((remote_participant_leaves) ? 0 : 1));
			BC_ASSERT_EQUAL(linphone_conference_get_participant_count(marie_conference),conf_parts_no, int, "%d");
			BC_ASSERT_EQUAL(linphone_conference_get_size(marie_conference),linphone_conference_get_participant_count(marie_conference)+linphone_conference_is_in(marie_conference), int, "%d");
	...
	int conf_parts_no = ((remote_participant_leaves) ? 1 : 2) + add_participant;
	...
	if (laure_conference) {
		...
		BC_ASSERT_EQUAL(linphone_conference_get_size(laure_conference),linphone_conference_get_participant_count(laure_conference) + linphone_conference_is_in(laure_conference), int, "%d");
	...
	if (remote_participant_leaves) {
		...
		BC_ASSERT_EQUAL(linphone_conference_get_participant_count(marie_conference), (add_participant + 2), int, "%d");
		BC_ASSERT_EQUAL(linphone_conference_get_size(marie_conference),linphone_conference_get_participant_count(marie_conference)+linphone_conference_is_in(marie_conference), int, "%d");
		...
		BC_ASSERT_TRUE(wait_for_list(lcs,&pauline->stat.number_of_LinphoneSubscriptionActive,(pauline_stats.number_of_LinphoneSubscriptionActive + 1),5000));
		BC_ASSERT_TRUE(wait_for_list(lcs,&marie->stat.number_of_LinphoneSubscriptionActive,(marie_stats.number_of_LinphoneSubscriptionActive + 1),5000));

		// NOTIFY to resend list of conference participants
		BC_ASSERT_TRUE(wait_for_list(lcs,&pauline->stat.number_of_NotifyReceived,(pauline_stats.number_of_NotifyReceived + 1),5000));

		// Notify addition of participant and its device
		BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_NotifyReceived,(michelle_stats.number_of_NotifyReceived + 2),5000));
		if (add_participant) {
			BC_ASSERT_TRUE(wait_for_list(lcs,&laure->stat.number_of_NotifyReceived,(laure_stats.number_of_NotifyReceived + 2),5000));
		}
...
static void all_temporarely_leave_conference_base(bool_t local_enters_first) {
	...
	for (bctbx_list_t *it = participants; it; it = bctbx_list_next(it)) {
		...
		BC_ASSERT_TRUE(wait_for_list(lcs,&m->stat.number_of_NotifyReceived,(participants_initial_stats[counter].number_of_NotifyReceived + 1),5000));
...
static void remote_participant_leaves_and_conference_ends_base(bool_t local_ends_conference) {
	...
	if (local_ends_conference) {
		...
		if (michelle_called_by_marie) {
			...
			BC_ASSERT_TRUE(wait_for_list(lcs,&marie->stat.number_of_LinphoneSubscriptionTerminated,marie_stats.number_of_LinphoneSubscriptionTerminated + 1,3000));
	...
	// Pauline tries to enter conference again
	// Its subscription is declined as the conference ended
	linphone_conference_enter(pauline_conference);
	...
	BC_ASSERT_TRUE(wait_for_list(lcs,&marie->stat.number_of_LinphoneSubscriptionTerminated,marie_stats.number_of_LinphoneSubscriptionTerminated + 1,3000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&pauline->stat.number_of_LinphoneSubscriptionError,pauline_stats.number_of_LinphoneSubscriptionError + 1,3000));

	BC_ASSERT_EQUAL(pauline->stat.number_of_LinphoneSubscriptionActive,pauline_stats.number_of_LinphoneSubscriptionActive, int, "%0d");
	BC_ASSERT_EQUAL(marie->stat.number_of_LinphoneSubscriptionActive,marie_stats.number_of_LinphoneSubscriptionActive, int, "%0d");
...
static void participant_takes_call_after_conference_started_and_rejoins_conference(void) {
	...
	// Pauline and Michelle are notified that Laure leave the conference to take Chloe's call
	BC_ASSERT_TRUE(wait_for_list(lcs,&pauline->stat.number_of_NotifyReceived,(pauline_initial_stats.number_of_NotifyReceived + 2),5000));
	...
	if (laure_called_by_chloe) {
		...
		if (event_log_enabled) {
			BC_ASSERT_TRUE(wait_for_list(lcs,&laure->stat.number_of_LinphoneSubscriptionOutgoingProgress,(laure_initial_stats.number_of_LinphoneSubscriptionOutgoingProgress + 1),5000));
			BC_ASSERT_TRUE(wait_for_list(lcs,&marie->stat.number_of_LinphoneSubscriptionIncomingReceived,(marie_initial_stats.number_of_LinphoneSubscriptionIncomingReceived + 1),5000));

			BC_ASSERT_TRUE(wait_for_list(lcs,&laure->stat.number_of_LinphoneSubscriptionActive,(laure_initial_stats.number_of_LinphoneSubscriptionActive + 1),5000));
			BC_ASSERT_TRUE(wait_for_list(lcs,&marie->stat.number_of_LinphoneSubscriptionActive,(marie_initial_stats.number_of_LinphoneSubscriptionActive + 1),5000));
...
static void participants_takes_call_after_conference_started_and_rejoins_conference(void) {
	...
	// Laure leaves the conference to take Chloe's call
	// Chloe also leave the conference to make the call
	...
	// Michelle is notified that Laure and Chloe left
	BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_NotifyReceived,(michelle_initial_stats.number_of_NotifyReceived + 4),5000));
	...
	BC_ASSERT_FALSE(linphone_call_is_in_conference(laure_calls_marie));
	BC_ASSERT_PTR_NULL(linphone_call_get_conference(chloe_calls_marie));
...
static void participant_takes_call_after_conference_started_and_rejoins_conference_after_conference_ended(void) {
	...
	// Michelle is notified that Laure left
	BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_NotifyReceived,(michelle_initial_stats.number_of_NotifyReceived + 2),5000));

...
static void simple_conference_base(LinphoneCoreManager* marie, LinphoneCoreManager* pauline, LinphoneCoreManager* laure, LinphoneCoreManager *focus, bool_t pause_and_hangup) {
	...
	check_conference_volumes(pauline_called_by_marie);
...
static void simple_conference_with_user_defined_layout(const LinphoneConferenceLayout layout, bool_t local_change_layout, bool_t remote_change_layout, bool_t enable_local_participant, bool_t add_participant) {
	...
	BC_ASSERT_TRUE(linphone_call_params_video_enabled(linphone_call_get_current_params(linphone_core_get_current_call(chloe->lc))));
	...
	for (bctbx_list_t *it = all_manangers_in_conf; it; it = bctbx_list_next(it)) {
		...
		linphone_address_unref(uri);
		if (conference) {
			BC_ASSERT_EQUAL(linphone_conference_get_participant_count(conference), (((m == marie) || (enable_local_participant)) ? 3 : 2), int, "%d");
			BC_ASSERT_EQUAL(bctbx_list_size(linphone_conference_get_participant_device_list(conference)), 3, size_t, "%0zu");
	...
	for (bctbx_list_t *it = all_manangers_in_conf; it; it = bctbx_list_next(it)) {
		...
		if (conference) {
			...
			BC_ASSERT_EQUAL(bctbx_list_size(linphone_conference_get_participant_device_list(conference)), 2, size_t, "%0zu");
		}
	}
	...
	for (bctbx_list_t *it = all_manangers_in_conf; it; it = bctbx_list_next(it)) {
		...
		if (conference) {
			...
			BC_ASSERT_EQUAL(bctbx_list_size(linphone_conference_get_participant_device_list(conference)), 1, size_t, "%0zu");
		}
	}
...
static void audio_calls_added_to_video_conference_base(const LinphoneConferenceLayout layout, bool_t enable_ice) {
	LinphoneCoreManager* marie = create_mgr_for_conference( "marie_rc", TRUE);
	linphone_core_enable_conference_server(marie->lc,TRUE);
	LinphoneCoreManager* pauline = create_mgr_for_conference( "pauline_tcp_rc", TRUE);
	LinphoneCoreManager* laure = create_mgr_for_conference( liblinphone_tester_ipv6_available() ? "laure_tcp_rc" : "laure_rc_udp", TRUE);
	...
	for (bctbx_list_t *it = lcs; it; it = bctbx_list_next(it)) {
		...
		linphone_core_enable_video_display(c, FALSE);

		LinphoneVideoActivationPolicy * cpol = linphone_core_get_video_activation_policy(c);
		BC_ASSERT_FALSE(linphone_video_activation_policy_get_automatically_accept(cpol));
		BC_ASSERT_FALSE(linphone_video_activation_policy_get_automatically_initiate(cpol));
		linphone_video_activation_policy_unref(cpol);
	...
	linphone_call_params_enable_video(pauline_call_params,FALSE);
	linphone_call_params_enable_video(laure_call_params,FALSE);
	linphone_call_params_enable_video(michelle_call_params,FALSE);
...
static void audio_calls_added_to_video_conference(void) {
	audio_calls_added_to_video_conference_base(LinphoneConferenceLayoutGrid, FALSE);
}
...
//	TEST_NO_TAG("Simple conference with layout change of local participant", simple_conference_with_layout_change_local_participant),
	...
	TEST_NO_TAG("ICE video conference by merging video calls with grid layout", ice_video_conference_created_by_merging_video_calls_with_grid_layout),
	TEST_NO_TAG("One participant ICE video conference with grid layout", ice_video_conference_one_participant_grid_layout),

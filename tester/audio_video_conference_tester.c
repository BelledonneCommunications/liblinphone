...
static void conference_with_last_call_paused(void) {
	...
	// Check that current is the call between Marie and Laure and it is neither the first nor the last in the list held by the core
	LinphoneCall * marie_current_call = linphone_core_get_current_call(marie->lc);
	BC_ASSERT_PTR_NOT_NULL(marie_current_call);
	BC_ASSERT_TRUE(marie_call_laure == marie_current_call);
	const bctbx_list_t *call_list = linphone_core_get_calls(marie->lc);
	LinphoneCall * marie_call = (LinphoneCall *)bctbx_list_get_data(call_list);
	BC_ASSERT_FALSE(marie_call == marie_current_call);
	const bctbx_list_t *c = call_list;
	while (c->next != NULL) {
		c = c->next;
	}
	marie_call = (LinphoneCall *)bctbx_list_get_data(c);
	BC_ASSERT_FALSE(marie_call == marie_current_call);
	...
	for (bctbx_list_t *it = participants; it; it = bctbx_list_next(it)) {
		...
		BC_ASSERT_TRUE(wait_for_list(lcs,&m->stat.number_of_LinphoneSubscriptionActive, 1,3000));
	...
	call_list = linphone_core_get_calls(marie->lc);
	for (const bctbx_list_t *elem = call_list; elem != NULL; elem = elem->next) {
		LinphoneCall * conf_call = (LinphoneCall *)bctbx_list_get_data(call_list);

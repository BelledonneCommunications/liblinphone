...
static void set_video_in_call(LinphoneCoreManager* m1, LinphoneCoreManager* m2, bool_t enable_video, bctbx_list_t * participants, const LinphoneAddress * conference_address) {
	...
	for (bctbx_list_t *it = participants; it; it = bctbx_list_next(it)) {
		...
		if (m != m2) {
			LinphoneConference *m_conference = linphone_core_search_conference(m->lc, NULL, NULL, conference_address, NULL);
			...
			if (m_calls_m2 && m_conference) {
				...
				video_enabled += (((!!linphone_call_params_video_enabled(call_params)) && (linphone_conference_get_layout(m_conference) != LinphoneConferenceLayoutLegacy)) ? 1 : 0);
			}
		}
	}
	...
	if (m1_calls_m2) {
		...
		BC_ASSERT_TRUE(wait_for_list(lcs,&m2->stat.number_of_LinphoneCallUpdatedByRemote,initial_m2_stat.number_of_LinphoneCallUpdatedByRemote + video_enabled,3000));

		counter = 0;
		int idx = 0;
		for (bctbx_list_t *it = participants; it; it = bctbx_list_next(it)) {
			LinphoneCoreManager * m = (LinphoneCoreManager *)bctbx_list_get_data(it);
			BC_ASSERT_TRUE(wait_for_list(lcs,&m->stat.number_of_participant_device_media_capability_changed,initial_participants_stats[counter].number_of_participant_device_media_capability_changed+1,3000));
			LinphoneConference *m_conference = linphone_core_search_conference(m->lc, NULL, NULL, conference_address, NULL);
			BC_ASSERT_PTR_NOT_NULL(m_conference);
			if (m != m2) {
				LinphoneCall * m_calls_m2 = linphone_core_get_call_by_remote_address2(m->lc, m2->identity);
				...
				if (m_calls_m2 && m_conference) {
					const LinphoneCallParams * call_params = linphone_call_get_current_params(m_calls_m2);
					if (!!linphone_call_params_video_enabled(call_params) && (linphone_conference_get_layout(m_conference) != LinphoneConferenceLayoutLegacy)) {
						if (m != m1) {
							...
							if (defer_update == TRUE) {
								LinphoneCall * m2_calls_m = linphone_core_get_call_by_remote_address2(m2->lc, m->identity);
						...
						idx++;
						BC_ASSERT_TRUE(wait_for_list(lcs,&m->stat.number_of_LinphoneCallStreamsRunning,initial_participants_stats[counter].number_of_LinphoneCallStreamsRunning + 1,3000));
						BC_ASSERT_TRUE(wait_for_list(lcs,&m2->stat.number_of_LinphoneCallStreamsRunning,initial_m2_stat.number_of_LinphoneCallStreamsRunning + idx,3000));
					}
				}
			}

			if (m_conference) {
				const LinphoneConferenceParams * params = linphone_conference_get_current_params(m_conference);
				BC_ASSERT_TRUE(linphone_conference_is_in(m_conference) == linphone_conference_params_local_participant_enabled(params));
			}
			counter++;
		}

		bool_t check_video = (!m1_conference || (m1_conference && (linphone_conference_get_layout(m1_conference) == LinphoneConferenceLayoutLegacy)));
		// Wait for first frame if video is enabled
		if (enable_video && check_video) {
...
static void set_video_in_conference(bctbx_list_t* lcs, LinphoneCoreManager* conf, bctbx_list_t* participants, bool_t enable_video) {
	...
	for (bctbx_list_t *it = participants; it; it = bctbx_list_next(it)) {
		...
		BC_ASSERT_TRUE(wait_for_list(lcs,&m->stat.number_of_participants_added,initial_stats[idx].number_of_participants_added+1,3000));
		BC_ASSERT_TRUE(wait_for_list(lcs,&m->stat.number_of_participant_devices_added,initial_stats[idx].number_of_participant_devices_added+1,3000));
		BC_ASSERT_TRUE(wait_for_list(lcs,&m->stat.number_of_participants_removed,initial_stats[idx].number_of_participants_removed+1,3000));
		BC_ASSERT_TRUE(wait_for_list(lcs,&m->stat.number_of_participant_devices_removed,initial_stats[idx].number_of_participant_devices_removed+1,3000));
		BC_ASSERT_TRUE(wait_for_list(lcs,&m->stat.number_of_participant_device_media_capability_changed,initial_stats[idx].number_of_participant_device_media_capability_changed+1,3000));
	...
	for (bctbx_list_t *it = participants; it; it = bctbx_list_next(it)) {
		...
		if (video_enabled && (layout != LinphoneConferenceLayoutLegacy)) {
...
static void simple_conference_base(LinphoneCoreManager* marie, LinphoneCoreManager* pauline, LinphoneCoreManager* laure, LinphoneCoreManager *focus, bool_t pause_and_hangup) {
	...
	LinphoneConferenceLayout laure_layout = LinphoneConferenceLayoutLegacy;
	if (laure_conference) {
	...
	if (laure_call && (laure_layout != LinphoneConferenceLayoutLegacy)) {
	...
	LinphoneConferenceLayout pauline_layout = LinphoneConferenceLayoutLegacy;
	...
	if (pauline_call && (pauline_layout != LinphoneConferenceLayoutLegacy)) {
	...
	if (focus_is_up) {
		for (bctbx_list_t *it = lcs; it; it = bctbx_list_next(it)) {
			...
			if (c != focus->lc) {
				...
				BC_ASSERT_EQUAL((unsigned int)bctbx_list_size(call_logs), ((c == marie->lc) ? 3 : 2), unsigned int, "%u");

				bctbx_list_t * mgr_focus_call_log = linphone_core_get_call_history_2(mgr->lc, focus->identity, mgr->identity);
...
static void simple_conference_with_participant_addition_from_not_admin(void) {
	...
	BC_ASSERT_TRUE(wait_for_list(lcs,&laure->stat.number_of_participant_device_media_capability_changed,laure_stats.number_of_participant_device_media_capability_changed+1,3000));
	LinphoneAddress *laure_uri = linphone_address_new(linphone_core_get_identity(laure->lc));
	LinphoneConference * laure_conference = linphone_core_search_conference(laure->lc, NULL, laure_uri, marie_conference_address, NULL);
	linphone_address_unref(laure_uri);
	BC_ASSERT_PTR_NOT_NULL(laure_conference);
...
static void simple_conference_with_user_defined_layout(const LinphoneConferenceLayout layout, bool_t local_change_layout, bool_t remote_change_layout, bool_t enable_local_participant, bool_t add_participant, const LinphoneMediaEncryption encryption, bool_t update_conf_params) {
	LinphoneCoreManager* marie = create_mgr_for_conference( "marie_rc", TRUE);
	linphone_core_enable_conference_server(marie->lc,TRUE);
	LinphoneCoreManager* pauline = create_mgr_for_conference( "pauline_tcp_rc", TRUE);
	LinphoneCoreManager* laure = create_mgr_for_conference( liblinphone_tester_ipv6_available() ? "laure_tcp_rc" : "laure_rc_udp", TRUE);
	...
	//Marie creates the conference
	LinphoneConferenceParams *conf_params = linphone_core_create_conference_params_2(marie->lc, NULL);
	linphone_conference_params_enable_local_participant(conf_params, enable_local_participant);
	linphone_conference_params_enable_one_participant_conference(conf_params, TRUE);
	...
	const LinphoneConferenceParams * actual_conf_params = linphone_conference_get_current_params(conf);
	BC_ASSERT_TRUE(linphone_conference_params_local_participant_enabled(actual_conf_params) == enable_local_participant);
	BC_ASSERT_TRUE(linphone_conference_params_one_participant_conference_enabled(actual_conf_params));
	BC_ASSERT_TRUE(linphone_conference_is_in(conf) == enable_local_participant);
	...
	for (bctbx_list_t *it = all_manangers_in_conf; it; it = bctbx_list_next(it)) {
		...
		if (conference) {
			...
			BC_ASSERT_EQUAL(bctbx_list_size(participant_device_list), 3 + ((enable_local_participant) ? 1 : 0), size_t, "%0zu");
			...
			if (m == marie) {
				...
				BC_ASSERT_EQUAL(linphone_conference_is_in(conference), (int)enable_local_participant, int, "%d");
			} else {
				BC_ASSERT_EQUAL(linphone_conference_get_participant_count(conference), ((enable_local_participant) ? 3 : 2), int, "%d");
				BC_ASSERT_EQUAL(linphone_conference_is_in(conference), 1, int, "%d");
			}
		...
		for (const bctbx_list_t *call_it = linphone_core_get_calls(m->lc); call_it; call_it = bctbx_list_next(call_it)) {
	...
	if (local_change_layout) {
		conf_params = linphone_core_create_conference_params_2(marie->lc, conf);
		...
		LinphoneParticipant * participant = linphone_conference_get_me(conf);
	...
	if (remote_change_layout) {
		...
		LinphoneConference * conference = linphone_core_search_conference(michelle->lc, NULL, uri, local_conference_address, NULL);
		BC_ASSERT_PTR_NOT_NULL(conference);
		linphone_address_unref(uri);
		if (conference) {
			...
			if (update_conf_params) {
				conf_params = linphone_core_create_conference_params_2(michelle->lc, conference);
				linphone_conference_params_set_layout(conf_params, new_layout);
				linphone_conference_update_params(conference, conf_params);
				linphone_conference_params_unref(conf_params);
			} else {
				linphone_conference_set_layout(conference, new_layout);
			}
			...
			LinphoneParticipant * participant = linphone_conference_find_participant(conf, michelle->identity);
			BC_ASSERT_PTR_NOT_NULL(participant);
			bctbx_list_t *devices = linphone_participant_get_devices(participant);
	...
	for (bctbx_list_t *it = all_manangers_in_conf; it; it = bctbx_list_next(it)) {
		...
		int no_parts = (enable_local_participant || (m == marie)) ? 2 : 1;
		BC_ASSERT_TRUE(wait_for_list(lcs,&m->stat.number_of_participants_removed,1,3000));
		BC_ASSERT_TRUE(wait_for_list(lcs,&m->stat.number_of_participant_devices_removed,1,3000));
		...
		if (conference) {
			bctbx_list_t * participant_device_list = linphone_conference_get_participant_device_list(conference);
			BC_ASSERT_EQUAL(bctbx_list_size(participant_device_list), 2 + ((enable_local_participant) ? 1 : 0), size_t, "%0zu");
			bctbx_list_free_with_data(participant_device_list, (void(*)(void *))linphone_participant_device_unref);
			if (m == marie) {
				BC_ASSERT_EQUAL(linphone_conference_get_participant_count(conference), no_parts, int, "%d");
				BC_ASSERT_EQUAL(linphone_conference_is_in(conference), (int)enable_local_participant, int, "%d");
	...
	set_video_in_call(laure, marie, FALSE, all_manangers_in_conf, marie_conference_address);
	...
	set_video_in_call(laure, marie, TRUE, all_manangers_in_conf, marie_conference_address);
	call_params = linphone_call_get_current_params(marie_call_michelle);
	BC_ASSERT_TRUE(linphone_call_params_video_enabled(call_params));
	call_params = linphone_call_get_current_params(michelle_called_by_marie);
	BC_ASSERT_TRUE(linphone_call_params_video_enabled(call_params));
	...
	for (bctbx_list_t *it = all_manangers_in_conf; it; it = bctbx_list_next(it)) {
		...
		BC_ASSERT_TRUE(wait_for_list(lcs,&m->stat.number_of_participants_removed,2,3000));
		BC_ASSERT_TRUE(wait_for_list(lcs,&m->stat.number_of_participant_devices_removed,2,3000));
		int no_parts = (enable_local_participant || (m == marie)) ? 1 : 0;
		LinphoneAddress *uri = linphone_address_new(linphone_core_get_identity(m->lc));
		LinphoneConference * conference = linphone_core_search_conference(m->lc, NULL, uri, marie_conference_address, NULL);
		BC_ASSERT_PTR_NOT_NULL(conference);
		linphone_address_unref(uri);
		if (conference) {
			bctbx_list_t * participant_device_list = linphone_conference_get_participant_device_list(conference);
			BC_ASSERT_EQUAL(bctbx_list_size(participant_device_list), 1 + ((enable_local_participant) ? 1 : 0), size_t, "%0zu");
			bctbx_list_free_with_data(participant_device_list, (void(*)(void *))linphone_participant_device_unref);
			if (m == marie) {
				BC_ASSERT_EQUAL(linphone_conference_get_participant_count(conference), no_parts, int, "%d");
				BC_ASSERT_EQUAL(linphone_conference_is_in(conference), (int)enable_local_participant, int, "%d");
	...
	if (add_participant) {
		...
		add_calls_to_local_conference(lcs2, marie, conf, participants2, TRUE);
		...
		for (bctbx_list_t *it = all_manangers_in_conf; it; it = bctbx_list_next(it)) {
			...
			linphone_address_unref(uri);
			if (conference) {
				BC_ASSERT_EQUAL(linphone_conference_get_participant_count(conference), (((m == marie) || (enable_local_participant)) ? 2 : 1), int, "%d");
				bctbx_list_t * participant_device_list = linphone_conference_get_participant_device_list(conference);
				BC_ASSERT_EQUAL(bctbx_list_size(participant_device_list), ((enable_local_participant) ? 3 : 2), size_t, "%0zu");
				bctbx_list_free_with_data(participant_device_list, (void(*)(void *))linphone_participant_device_unref);
			}
		}
		...
		remove_participant_from_local_conference(lcs2, marie, chloe, conf);
		...
		for (bctbx_list_t *it = all_manangers_in_conf; it; it = bctbx_list_next(it)) {
			...
			linphone_address_unref(uri);
			if (conference) {
				BC_ASSERT_EQUAL(linphone_conference_get_participant_count(conference), (((m == marie) || (enable_local_participant)) ? 1 : 0), int, "%d");
				bctbx_list_t * participant_device_list = linphone_conference_get_participant_device_list(conference);
				BC_ASSERT_EQUAL(bctbx_list_size(participant_device_list), ((enable_local_participant) ? 2 : 1), size_t, "%0zu");
				bctbx_list_free_with_data(participant_device_list, (void(*)(void *))linphone_participant_device_unref);
			}
		}
	}
	...
	marie_call_no = (int)bctbx_list_size(linphone_core_get_calls(marie->lc));
...
static void simple_conference_with_grid_layout(void) {
	simple_conference_with_user_defined_layout(LinphoneConferenceLayoutGrid, FALSE, FALSE, TRUE, FALSE, LinphoneMediaEncryptionNone, TRUE);
}

static void simple_srtp_conference_with_grid_layout_and_local_participant_disabled(void) {
	simple_conference_with_user_defined_layout(LinphoneConferenceLayoutGrid, FALSE, FALSE, FALSE, FALSE, LinphoneMediaEncryptionSRTP, TRUE);
}

static void simple_conference_with_grid_layout_and_local_participant_disabled(void) {
	simple_conference_with_user_defined_layout(LinphoneConferenceLayoutGrid, FALSE, FALSE, FALSE, FALSE, LinphoneMediaEncryptionNone, TRUE);
}

static void simple_conference_with_active_speaker_layout(void) {
	simple_conference_with_user_defined_layout(LinphoneConferenceLayoutActiveSpeaker, FALSE, FALSE, TRUE, FALSE, LinphoneMediaEncryptionNone, TRUE);
}

static void simple_srtp_conference_with_active_speaker_layout_and_local_participant_disabled(void) {
	simple_conference_with_user_defined_layout(LinphoneConferenceLayoutActiveSpeaker, FALSE, FALSE, FALSE, FALSE, LinphoneMediaEncryptionSRTP, TRUE);
}

static void simple_conference_with_active_speaker_layout_and_local_participant_disabled(void) {
	simple_conference_with_user_defined_layout(LinphoneConferenceLayoutActiveSpeaker, FALSE, FALSE, FALSE, FALSE, LinphoneMediaEncryptionNone, TRUE);
}

static void simple_conference_with_layout_change_local_participant(void) {
	simple_conference_with_user_defined_layout(LinphoneConferenceLayoutGrid, TRUE, FALSE, TRUE, FALSE, LinphoneMediaEncryptionNone, TRUE);
}

static void simple_conference_with_layout_change_remote_participant(void) {
	simple_conference_with_user_defined_layout(LinphoneConferenceLayoutGrid, FALSE, TRUE, TRUE, FALSE, LinphoneMediaEncryptionNone, TRUE);
}

static void simple_conference_with_layout_change_remote_participant_without_conference_params_update(void) {
	simple_conference_with_user_defined_layout(LinphoneConferenceLayoutGrid, FALSE, TRUE, TRUE, FALSE, LinphoneMediaEncryptionNone, FALSE);
}

static void simple_conference_with_layout_change_remote_participant_and_local_participant_disabled(void) {
	simple_conference_with_user_defined_layout(LinphoneConferenceLayoutGrid, FALSE, TRUE, FALSE, FALSE, LinphoneMediaEncryptionNone, TRUE);
}

static void add_participant_after_layout_change(void) {
	simple_conference_with_user_defined_layout(LinphoneConferenceLayoutGrid, FALSE, TRUE, TRUE, TRUE, LinphoneMediaEncryptionNone, TRUE);
}

static void add_participant_after_layout_change_and_local_participant_disabled(void) {
	simple_conference_with_user_defined_layout(LinphoneConferenceLayoutGrid, FALSE, TRUE, FALSE, TRUE, LinphoneMediaEncryptionNone, TRUE);
}
...
static void simple_conference_with_subject_change_from_admin_base(bool_t enable_video) {
	...
	linphone_core_set_video_activation_policy(((LinphoneCoreManager*)focus)->lc, pol);
	linphone_video_activation_policy_unref(pol);
	...
	if (enable_video) {
		linphone_core_set_video_device(((LinphoneCoreManager*)focus)->lc, liblinphone_tester_mire_id);
	}
	...
	if (enable_video) {
		linphone_core_enable_video_capture(((LinphoneCoreManager*)focus)->lc, TRUE);
		linphone_core_enable_video_display(((LinphoneCoreManager*)focus)->lc, TRUE);
	}
...
static void eject_from_3_participants_conference(LinphoneCoreManager *marie, LinphoneCoreManager *pauline, LinphoneCoreManager *laure, LinphoneCoreManager *focus, const LinphoneConferenceLayout layout) {
	...
	linphone_conference_params_enable_video(conf_params, (layout != LinphoneConferenceLayoutLegacy));
	conf = linphone_core_create_conference_with_params(marie->lc, conf_params);
	linphone_conference_params_unref(conf_params);
	BC_ASSERT_PTR_NOT_NULL(conf);
...
static void eject_from_3_participants_local_conference_legacy_layout(void) {
	...
	eject_from_3_participants_conference(marie, pauline, laure, NULL, LinphoneConferenceLayoutLegacy);
...
static void eject_from_4_participants_conference_call_terminated_one_by_one(LinphoneCoreManager *marie, LinphoneCoreManager *pauline, LinphoneCoreManager *laure, LinphoneCoreManager *michelle) {
	stats initial_marie_stat;
	stats initial_pauline_stat;
	stats initial_laure_stat;
	stats initial_michelle_stat;
	...
	BC_ASSERT_TRUE(linphone_call_get_state(linphone_core_get_call_by_remote_address2(michelle->lc, marie->identity)) == LinphoneCallPausedByRemote);
	BC_ASSERT_TRUE(linphone_call_get_state(linphone_core_get_call_by_remote_address2(marie->lc, michelle->identity)) == LinphoneCallPaused);
	...
	// As the call between Marie and Michelle is added to the conference while in the paused state, the local participant is not added in and the call between Marie and Laure keeps being in state StreamsRunning
	BC_ASSERT_TRUE(linphone_call_get_state(linphone_core_get_call_by_remote_address2(michelle->lc, marie->identity)) == LinphoneCallStreamsRunning);
	BC_ASSERT_TRUE(linphone_call_get_state(linphone_core_get_call_by_remote_address2(marie->lc, michelle->identity)) == LinphoneCallStreamsRunning);
	BC_ASSERT_TRUE(linphone_call_get_state(linphone_core_get_call_by_remote_address2(laure->lc, marie->identity)) == LinphoneCallStreamsRunning);
	BC_ASSERT_TRUE(linphone_call_get_state(linphone_core_get_call_by_remote_address2(marie->lc, laure->identity)) == LinphoneCallStreamsRunning);
	...
	BC_ASSERT_TRUE(wait_for_list(lcs,&pauline->stat.number_of_LinphoneCallEnd,initial_pauline_stat.number_of_LinphoneCallEnd + 1,10000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_LinphoneCallEnd,initial_michelle_stat.number_of_LinphoneCallEnd + 1,10000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&laure->stat.number_of_LinphoneCallEnd,initial_laure_stat.number_of_LinphoneCallEnd + 1,10000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&marie->stat.number_of_LinphoneCallEnd,initial_marie_stat.number_of_LinphoneCallEnd + 3,10000));
	...
	BC_ASSERT_TRUE(wait_for_list(lcs,&marie->stat.number_of_LinphoneConferenceStateTerminationPending,(initial_marie_stat.number_of_LinphoneConferenceStateTerminationPending + 1),5000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&marie->stat.number_of_LinphoneConferenceStateTerminated,(initial_marie_stat.number_of_LinphoneConferenceStateTerminated + 1),5000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&marie->stat.number_of_LinphoneConferenceStateDeleted,(initial_marie_stat.number_of_LinphoneConferenceStateDeleted + 1),5000));

	BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_LinphoneSubscriptionTerminated,initial_michelle_stat.number_of_LinphoneSubscriptionTerminated + 1,10000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&pauline->stat.number_of_LinphoneSubscriptionTerminated,initial_pauline_stat.number_of_LinphoneSubscriptionTerminated + 1,10000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&laure->stat.number_of_LinphoneSubscriptionTerminated,initial_laure_stat.number_of_LinphoneSubscriptionTerminated + 1,10000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&marie->stat.number_of_LinphoneSubscriptionTerminated,initial_marie_stat.number_of_LinphoneSubscriptionTerminated + 3,10000));

	BC_ASSERT_PTR_NULL(linphone_core_get_conference(marie->lc));

	BC_ASSERT_TRUE(wait_for_list(lcs,&pauline->stat.number_of_LinphoneCallReleased,initial_pauline_stat.number_of_LinphoneCallReleased+1,3000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_LinphoneCallReleased,initial_michelle_stat.number_of_LinphoneCallReleased+1,3000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&laure->stat.number_of_LinphoneCallReleased,initial_laure_stat.number_of_LinphoneCallReleased+1,3000));
...
static void eject_from_4_participants_conference_legacy_layout(void) {
	eject_from_4_participants_conference_base(LinphoneConferenceLayoutLegacy);
}
...
static void add_participant_after_conference_started_base(bool_t pause_all_calls) {
	...
	if (pause_all_calls) {
		...
		BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_participants_added,michelle_stats.number_of_participants_added+1,3000));
		BC_ASSERT_TRUE(wait_for_list(lcs,&laure->stat.number_of_participants_added,laure_stats.number_of_participants_added+1,3000));
		BC_ASSERT_TRUE(wait_for_list(lcs,&pauline->stat.number_of_participants_added,pauline_stats.number_of_participants_added+1,3000));

		BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_participant_devices_added,michelle_stats.number_of_participant_devices_added+1,3000));
		BC_ASSERT_TRUE(wait_for_list(lcs,&laure->stat.number_of_participant_devices_added,laure_stats.number_of_participant_devices_added+1,3000));
		BC_ASSERT_TRUE(wait_for_list(lcs,&pauline->stat.number_of_participant_devices_added,pauline_stats.number_of_participant_devices_added+1,3000));

		BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_participant_device_joined,michelle_stats.number_of_participant_device_joined+1,3000));
		BC_ASSERT_TRUE(wait_for_list(lcs,&laure->stat.number_of_participant_device_joined,laure_stats.number_of_participant_device_joined+1,3000));
		BC_ASSERT_TRUE(wait_for_list(lcs,&pauline->stat.number_of_participant_device_joined,pauline_stats.number_of_participant_device_joined+1,3000));
	}
...
static void remove_participant_from_video_conference_legacy_layout(void) {
	remove_participant_from_video_conference_base(LinphoneConferenceLayoutLegacy);
}
...
static void conference_created_by_merging_video_calls_base(bool_t event_package_enabled, bool_t enable_video, const LinphoneConferenceLayout layout, bool_t enable_ice) {
	LinphoneCoreManager* marie = create_mgr_for_conference( "marie_rc", TRUE);
	linphone_core_enable_conference_server(marie->lc,TRUE);
	LinphoneCoreManager* pauline = create_mgr_for_conference( "pauline_tcp_rc", TRUE);
	LinphoneCoreManager* laure = create_mgr_for_conference( liblinphone_tester_ipv6_available() ? "laure_tcp_rc" : "laure_rc_udp", TRUE);
	...
	BC_ASSERT_TRUE(linphone_call_params_get_custom_sdp_media_attribute(params, LinphoneStreamTypeAudio, "rtcp-mux") != NULL);
	BC_ASSERT_TRUE(linphone_call_params_get_custom_sdp_media_attribute(params, LinphoneStreamTypeVideo, "rtcp-mux") != NULL);
	...
	BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_LinphoneCallStreamsRunning, 2 + ((enable_ice) ? 1 : 0), 10000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&pauline->stat.number_of_LinphoneCallStreamsRunning, 2 + ((enable_ice) ? 1 : 0), 10000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&laure->stat.number_of_LinphoneCallStreamsRunning, 2 + ((enable_ice) ? 1 : 0), 10000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&marie->stat.number_of_LinphoneCallStreamsRunning, 4 + ((enable_ice) ? 6 : 0), 10000));
	...
	if (enable_video) {
		...
		BC_ASSERT_TRUE(wait_for_list(lcs,&marie->stat.number_of_participant_device_media_capability_changed,marie_stats.number_of_participant_device_media_capability_changed+1,3000));
		if (event_package_enabled) {
			BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_participant_device_media_capability_changed,michelle_stats.number_of_participant_device_media_capability_changed+1,3000));
			BC_ASSERT_TRUE(wait_for_list(lcs,&laure->stat.number_of_participant_device_media_capability_changed,laure_stats.number_of_participant_device_media_capability_changed+1,3000));
			BC_ASSERT_TRUE(wait_for_list(lcs,&pauline->stat.number_of_participant_device_media_capability_changed,pauline_stats.number_of_participant_device_media_capability_changed+1,3000));
		}
	}
...
static void video_conference_created_by_merging_video_calls_without_conference_event_package(void) {
	conference_created_by_merging_video_calls_base(FALSE, TRUE, LinphoneConferenceLayoutGrid, FALSE);
}

static void video_conference_created_by_merging_video_calls_with_active_speaker_layout(void) {
	conference_created_by_merging_video_calls_base(TRUE, TRUE, LinphoneConferenceLayoutActiveSpeaker, FALSE);
}

static void video_conference_created_by_merging_video_calls_with_legacy_layout(void) {
	conference_created_by_merging_video_calls_base(TRUE, TRUE, LinphoneConferenceLayoutLegacy, FALSE);
}

static void video_conference_created_by_merging_video_calls_with_grid_layout(void) {
	conference_created_by_merging_video_calls_base(TRUE, TRUE, LinphoneConferenceLayoutGrid, FALSE);
}
...
static void ice_video_conference_one_participant_active_speaker_layout_without_local_participant(void) {
	ice_video_conference_one_participant(LinphoneConferenceLayoutActiveSpeaker,FALSE);
}

static void ice_video_conference_created_by_merging_video_calls_with_grid_layout(void) {
	conference_created_by_merging_video_calls_base(TRUE, TRUE, LinphoneConferenceLayoutGrid, TRUE);
}

static void audio_conference_created_by_merging_video_calls(void) {
	conference_created_by_merging_video_calls_base(TRUE, FALSE, LinphoneConferenceLayoutGrid, FALSE);
}

static void audio_calls_added_to_video_conference_base(const LinphoneConferenceLayout layout, bool_t enable_ice, bool_t enable_participant_video, bool_t participant_calls_conference) {
	LinphoneCoreManager* marie = create_mgr_for_conference( "marie_rc", TRUE);
	linphone_core_enable_conference_server(marie->lc,TRUE);
	LinphoneCoreManager* pauline = create_mgr_for_conference( "pauline_tcp_rc", TRUE);
	LinphoneCoreManager* laure = create_mgr_for_conference( liblinphone_tester_ipv6_available() ? "laure_tcp_rc" : "laure_rc_udp", TRUE);
	...
	LinphoneVideoActivationPolicy * participant_pol = linphone_factory_create_video_activation_policy(linphone_factory_get());
	...
	LinphoneVideoActivationPolicy * conference_pol = linphone_factory_create_video_activation_policy(linphone_factory_get());
	...
	for (bctbx_list_t *it = lcs; it; it = bctbx_list_next(it)) {
		...
		if (c == marie->lc) {
			...
			BC_ASSERT_TRUE(linphone_video_activation_policy_get_automatically_initiate(cpol) == enable_participant_video);
			linphone_video_activation_policy_unref(cpol);
		}
	...
	BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_LinphoneCallStreamsRunning, 2 + ((enable_ice) ? 2 : 0), 10000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&pauline->stat.number_of_LinphoneCallStreamsRunning, 2 + ((enable_ice) ? 2 : 0), 10000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&laure->stat.number_of_LinphoneCallStreamsRunning, 2 + ((enable_ice) ? 2 : 0), 10000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&marie->stat.number_of_LinphoneCallStreamsRunning, 6 + ((enable_ice) ? 6 : 0), 10000));
...
static void simple_participant_leaves_conference_base(bool_t remote_participant_leaves) {
	...
	if (remote_participant_leaves) {
		...
		if (pauline_conference) {
			...
			BC_ASSERT_TRUE(wait_for_list(lcs,&marie->stat.number_of_participant_device_left,marie_stats.number_of_participant_device_left+1,3000));
			BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_participant_device_left,michelle_stats.number_of_participant_device_left+1,3000));
			BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_participant_device_media_capability_changed,michelle_stats.number_of_participant_device_media_capability_changed+1,3000));
		...
		if (marie_conference) {
			...
			BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_participants_removed,michelle_stats.number_of_participants_removed+1,3000));
			BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_participant_devices_removed,michelle_stats.number_of_participant_devices_removed+1,3000));
			BC_ASSERT_EQUAL(linphone_conference_get_participant_count(marie_conference),2, int, "%d");
	...
	if (remote_participant_leaves) {
		...
		if (pauline_conference) {
			...
			BC_ASSERT_TRUE(wait_for_list(lcs,&marie->stat.number_of_participant_device_joined,marie_stats.number_of_participant_device_joined+1,3000));
			BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_participant_device_joined,michelle_stats.number_of_participant_device_joined+1,3000));
			BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_participant_device_media_capability_changed,michelle_stats.number_of_participant_device_media_capability_changed+1,3000));
...
static void participant_leaves_conference_base(bool_t remote_participant_leaves, bool_t add_participant, bool_t wait_for_updates) {
	...
	if (remote_participant_leaves) {
		...
		BC_ASSERT_TRUE(wait_for_list(lcs,&pauline->stat.number_of_participant_device_joined,marie_stats.number_of_participant_device_joined+1,3000));
...
static void all_temporarely_leave_conference_base(bool_t local_enters_first) {
	...
	BC_ASSERT_TRUE(wait_for_list(lcs,&laure->stat.number_of_participant_device_left,laure_stats.number_of_participant_device_left+4,3000));

	BC_ASSERT_TRUE(wait_for_list(lcs,&pauline->stat.number_of_participants_removed,pauline_stats.number_of_participants_removed+1,3000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&pauline->stat.number_of_participant_devices_removed,pauline_stats.number_of_participant_devices_removed+1,3000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&laure->stat.number_of_participants_removed,laure_stats.number_of_participants_removed+1,3000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&laure->stat.number_of_participant_devices_removed,laure_stats.number_of_participant_devices_removed+1,3000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_participants_removed,michelle_stats.number_of_participants_removed+1,3000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_participant_devices_removed,michelle_stats.number_of_participant_devices_removed+1,3000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&marie->stat.number_of_participants_removed,marie_stats.number_of_participants_removed+1,3000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&marie->stat.number_of_participant_devices_removed,marie_stats.number_of_participant_devices_removed+1,3000));
	...
	for (bctbx_list_t *it = participants; it; it = bctbx_list_next(it)) {
		...
		BC_ASSERT_TRUE(wait_for_list(lcs,&m->stat.number_of_participant_device_media_capability_changed,participants_initial_stats[counter].number_of_participant_device_media_capability_changed+2,3000));
		BC_ASSERT_TRUE(wait_for_list(lcs,&m->stat.number_of_participant_device_joined,participants_initial_stats[counter].number_of_participant_device_joined+participants_joined,3000));

		if (local_enters_first) {
			BC_ASSERT_TRUE(wait_for_list(lcs,&m->stat.number_of_participants_added,participants_initial_stats[counter].number_of_participants_added+1,3000));
			BC_ASSERT_TRUE(wait_for_list(lcs,&m->stat.number_of_participant_devices_added,participants_initial_stats[counter].number_of_participant_devices_added+1,3000));
		}
	...
	if (!local_enters_first) {
		...
		BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_participants_added,michelle_stats.number_of_participants_added+1,3000));
		BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_participant_devices_added,michelle_stats.number_of_participant_devices_added+1,3000));
		BC_ASSERT_TRUE(wait_for_list(lcs,&marie->stat.number_of_participants_added,marie_stats.number_of_participants_added+1,3000));
		BC_ASSERT_TRUE(wait_for_list(lcs,&marie->stat.number_of_participant_devices_added,marie_stats.number_of_participant_devices_added+1,3000));
...
static void participant_call_terminated_after_leaving_conference_base(bool_t local_terminates) {
	...
	BC_ASSERT_TRUE(wait_for_list(lcs,&chloe->stat.number_of_participant_device_media_capability_changed,chloe_stats.number_of_participant_device_media_capability_changed+1,3000));
	if (chloe_conference) {
...
static void local_participant_takes_call_after_conference_started_and_conference_ends(void) {
	...
	BC_ASSERT_TRUE(wait_for_list(lcs,&pauline->stat.number_of_participants_removed,pauline_initial_stats.number_of_participants_removed+1,3000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&pauline->stat.number_of_participant_devices_removed,pauline_initial_stats.number_of_participant_devices_removed+1,3000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&laure->stat.number_of_participants_removed,laure_initial_stats.number_of_participants_removed+1,3000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&laure->stat.number_of_participant_devices_removed,laure_initial_stats.number_of_participant_devices_removed+1,3000));
...
static void remote_participant_adds_video_during_conference(void) {
	...
	LinphoneCall * pauline_conf_call = linphone_core_get_call_by_remote_address2(pauline->lc, marie->identity);
	...
	BC_ASSERT_TRUE(wait_for_list(lcs, &pauline->stat.number_of_LinphoneCallUpdating, pauline_initial_stats.number_of_LinphoneCallUpdating + 1, 5000));
	BC_ASSERT_TRUE(wait_for_list(lcs, &pauline->stat.number_of_LinphoneCallStreamsRunning, pauline_initial_stats.number_of_LinphoneCallStreamsRunning + 1, 5000));
	// Laure and Michelle do not send reINVITEs because their video is off
	BC_ASSERT_TRUE(wait_for_list(lcs, &laure->stat.number_of_participant_device_media_capability_changed,laure_initial_stats.number_of_participant_device_media_capability_changed+1,3000));
	BC_ASSERT_TRUE(wait_for_list(lcs, &michelle->stat.number_of_participant_device_media_capability_changed,michelle_initial_stats.number_of_participant_device_media_capability_changed+1,3000));
	BC_ASSERT_TRUE(wait_for_list(lcs, &marie->stat.number_of_LinphoneCallUpdatedByRemote, marie_initial_stats.number_of_LinphoneCallUpdatedByRemote + 1, 5000));
	BC_ASSERT_TRUE(wait_for_list(lcs, &marie->stat.number_of_LinphoneCallStreamsRunning, marie_initial_stats.number_of_LinphoneCallStreamsRunning + 1, 5000));
	BC_ASSERT_TRUE(wait_for_list(lcs, &marie->stat.number_of_participant_device_media_capability_changed,marie_initial_stats.number_of_participant_device_media_capability_changed+1,3000));

	LinphoneCall * laure_conf_call = linphone_core_get_call_by_remote_address2(laure->lc, marie->identity);
	...
	BC_ASSERT_TRUE(wait_for_list(lcs, &laure->stat.number_of_LinphoneCallUpdating, laure_initial_stats.number_of_LinphoneCallUpdating + 1, 5000));
	BC_ASSERT_TRUE(wait_for_list(lcs, &laure->stat.number_of_LinphoneCallStreamsRunning, laure_initial_stats.number_of_LinphoneCallStreamsRunning + 1, 5000));
	BC_ASSERT_TRUE(wait_for_list(lcs, &pauline->stat.number_of_LinphoneCallUpdating, pauline_initial_stats.number_of_LinphoneCallUpdating + 1, 5000));
	BC_ASSERT_TRUE(wait_for_list(lcs, &pauline->stat.number_of_LinphoneCallStreamsRunning, pauline_initial_stats.number_of_LinphoneCallStreamsRunning + 1, 5000));
	BC_ASSERT_TRUE(wait_for_list(lcs, &pauline->stat.number_of_participant_device_media_capability_changed,pauline_initial_stats.number_of_participant_device_media_capability_changed+1,3000));
	// Michelle does not send reINVITEs because its video is off
	BC_ASSERT_TRUE(wait_for_list(lcs, &michelle->stat.number_of_participant_device_media_capability_changed,michelle_initial_stats.number_of_participant_device_media_capability_changed+1,3000));
	BC_ASSERT_TRUE(wait_for_list(lcs, &marie->stat.number_of_LinphoneCallUpdatedByRemote, marie_initial_stats.number_of_LinphoneCallUpdatedByRemote + 2, 5000));
	BC_ASSERT_TRUE(wait_for_list(lcs, &marie->stat.number_of_LinphoneCallStreamsRunning, marie_initial_stats.number_of_LinphoneCallStreamsRunning + 2, 5000));
	BC_ASSERT_TRUE(wait_for_list(lcs, &marie->stat.number_of_participant_device_media_capability_changed,marie_initial_stats.number_of_participant_device_media_capability_changed+1,3000));
...
static void participant_takes_call_after_conference_started_and_rejoins_conference(void) {
	...
	BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_participant_device_media_capability_changed,michelle_initial_stats.number_of_participant_device_media_capability_changed+1,3000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&pauline->stat.number_of_participant_device_media_capability_changed,pauline_initial_stats.number_of_participant_device_media_capability_changed+1,3000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_participant_device_left,michelle_initial_stats.number_of_participant_device_left+1,3000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&pauline->stat.number_of_participant_device_left,pauline_initial_stats.number_of_participant_device_left+1,3000));
...
static void participants_take_call_after_conference_started_and_rejoins_conference(void) {
	...
	BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_participant_device_media_capability_changed,michelle_initial_stats.number_of_participant_device_media_capability_changed+2,3000));
	BC_ASSERT_TRUE(wait_for_list(lcs,&michelle->stat.number_of_participant_device_left,michelle_initial_stats.number_of_participant_device_left+2,3000));
	...
	if (laure_called_by_chloe) {
		...
		BC_ASSERT_TRUE(wait_for_list(lcs,&chloe->stat.number_of_participant_device_media_capability_changed,chloe_initial_stats.number_of_participant_device_media_capability_changed+1,3000));
...
static void toggle_video_settings_during_conference_base(bool_t automatically_video_accept, bool_t defer_update) {
	...
	if (conference) {
		...
		terminate_conference(new_participants, marie, conference, NULL);
	}
	...
	if (conference) {
		...
		for (int i = 0; i < 4; i++) {
			...
			for (bctbx_list_t *it = new_participants; it; it = bctbx_list_next(it)) {
				...
				BC_ASSERT_TRUE(wait_for_list(lcs, &m->stat.number_of_participant_device_media_capability_changed, participants_initial_stats[counter].number_of_participant_device_media_capability_changed+2,3000));
				BC_ASSERT_FALSE(wait_for_list(lcs, &m->stat.number_of_participant_devices_added, participants_initial_stats[counter].number_of_participant_devices_added + 1,3000));
				BC_ASSERT_FALSE(wait_for_list(lcs, &m->stat.number_of_participant_device_joined, participants_initial_stats[counter].number_of_participant_device_joined + 1,3000));
			...
			BC_ASSERT_TRUE(wait_for_list(lcs, &marie->stat.number_of_LinphoneCallUpdatedByRemote, marie_stats.number_of_LinphoneCallUpdatedByRemote + 3, 5000));
			BC_ASSERT_TRUE(wait_for_list(lcs, &marie->stat.number_of_LinphoneCallStreamsRunning, marie_stats.number_of_LinphoneCallStreamsRunning + 3, 5000));
			BC_ASSERT_TRUE(wait_for_list(lcs, &marie->stat.number_of_participant_device_media_capability_changed, marie_stats.number_of_participant_device_media_capability_changed+3,3000));
			BC_ASSERT_FALSE(wait_for_list(lcs, &marie->stat.number_of_participant_devices_added, marie_stats.number_of_participant_devices_added+1,3000));
			BC_ASSERT_FALSE(wait_for_list(lcs, &marie->stat.number_of_participant_device_joined, marie_stats.number_of_participant_device_joined+1,3000));
...
void eject_from_3_participants_remote_conference(void) {
	...
	eject_from_3_participants_conference(marie, pauline, laure, (LinphoneCoreManager *)focus, LinphoneConferenceLayoutLegacy);
...
static void conference_mix_created_by_merging_video_calls_base (LinphoneConferenceLayout layout) {
	...
	// Check that video is still on
	BC_ASSERT_PTR_EQUAL(linphone_call_get_conference(marie_call_laure), conf);
	negotiated_call_params = linphone_call_get_current_params(marie_call_laure);
	BC_ASSERT_TRUE(linphone_call_params_video_enabled(negotiated_call_params) == TRUE);
	negotiated_call_params = linphone_call_get_current_params(laure_called_by_marie);
	BC_ASSERT_TRUE(linphone_call_params_video_enabled(negotiated_call_params) == TRUE);
	BC_ASSERT_PTR_EQUAL(linphone_call_get_conference(marie_call_pauline), conf);
	negotiated_call_params = linphone_call_get_current_params(marie_call_pauline);
	BC_ASSERT_TRUE(linphone_call_params_video_enabled(negotiated_call_params) == TRUE);
	negotiated_call_params = linphone_call_get_current_params(pauline_called_by_marie);
	BC_ASSERT_TRUE(linphone_call_params_video_enabled(negotiated_call_params) == TRUE);
...
static void video_conference_created_by_merging_video_calls_with_legacy_layout_and_local_participant_disabled(void) {
	conference_mix_created_by_merging_video_calls_base(LinphoneConferenceLayoutLegacy);
}
...
test_t audio_conference_tests[] = {
	...
	TEST_NO_TAG("Remove participant from video conference with legacy layout", remove_participant_from_video_conference_legacy_layout),
	TEST_NO_TAG("Remove participant from video conference with grid layout", remove_participant_from_video_conference_grid_layout),
...
test_t video_conference_tests[] = {
	...
	TEST_NO_TAG("Simple SRTP conference with active speaker layout and local participant disabled", simple_srtp_conference_with_active_speaker_layout_and_local_participant_disabled),
	...
	TEST_NO_TAG("Simple SRTP conference with grid layout and local participant disabled", simple_srtp_conference_with_grid_layout_and_local_participant_disabled),
	...
	TEST_NO_TAG("Simple conference with layout change of remote participant without conference params update", simple_conference_with_layout_change_remote_participant_without_conference_params_update),
	...
	TEST_NO_TAG("Audio calls initiated by host added to video conference", audio_calls_initiated_by_host_added_to_video_conference),
	TEST_NO_TAG("Audio calls with video rejected added to video conference", audio_calls_with_video_rejected_added_to_video_conference),
	TEST_NO_TAG("Add participant after layout change", add_participant_after_layout_change),
	TEST_NO_TAG("Add participant after layout change and local participant disabled", add_participant_after_layout_change_and_local_participant_disabled),
	...
	TEST_NO_TAG("Video conference by merging video calls with legacy layout", video_conference_created_by_merging_video_calls_with_legacy_layout),
	TEST_NO_TAG("Video conference by merging video calls with legacy layout and local participant disabled", video_conference_created_by_merging_video_calls_with_legacy_layout_and_local_participant_disabled),
	...
	TEST_ONE_TAG("One participant ICE video conference with active speaker layout without local participant", ice_video_conference_one_participant_active_speaker_layout_without_local_participant, "ICE"),
...
//	TEST_NO_TAG("Update conference parameter during conference", update_conf_params_during_conference),
	...
	TEST_NO_TAG("Simultaneous toggle of video settings during conference", simultaneous_toggle_video_settings_during_conference),

...
"a=rtpmap:101 telephone-event/8000\r\n";
	const char *application_stream_sdp =
...
"a=rtcp-fb:* ccm fir\r\n";
	...
	if (unknown_at_end){
		unsupported_invite = bctbx_strdup_printf("%s%s%s", unsupported_invite_base, video_stream_sdp, application_stream_sdp);
	}else{
		unsupported_invite = bctbx_strdup_printf("%s%s%s", unsupported_invite_base, application_stream_sdp, video_stream_sdp);
	}

	BC_ASSERT_TRUE(liblinphone_tester_send_data(unsupported_invite, strlen(unsupported_invite), "127.0.0.1", linphone_transports_get_udp_port(tp), SOCK_DGRAM) > 0);
	linphone_transports_unref(tp);
...
static test_t offeranswer_tests[] = {
	...
	TEST_NO_TAG("Simple call with different codec mappings and config-supplied sdp address", simple_call_with_different_codec_mappings_and_config_supplied_sdp_addresses),

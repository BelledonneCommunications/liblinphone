...
static void create_encrypted_conference_with_chat_sending_messages() {
	Focus focus("chloe_rc");
	{ // to make sure focus is destroyed after clients.
		...
		for (auto mgr : {focus.getCMgr(), marie.getCMgr(), pauline.getCMgr(), laure.getCMgr(), michelle.getCMgr(),
			...
			linphone_core_enable_send_message_after_notify(mgr->lc, FALSE);
			// Set a very high delay as the timer will be stopped anyway by the NOTIFY full state
			linphone_core_set_message_sending_delay(mgr->lc, 10000);
		...
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneCallIncomingReceived,
		                             focus_stat.number_of_LinphoneCallIncomingReceived + static_cast<int>(members.size()),
		                             liblinphone_tester_sip_timeout));
		int focus_no_streams_running = 2*static_cast<int>(members.size());
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneCallUpdatedByRemote,
		                             focus_stat.number_of_LinphoneCallUpdatedByRemote + static_cast<int>(members.size()),
		                             liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneCallStreamsRunning,
		                             focus_stat.number_of_LinphoneCallStreamsRunning + focus_no_streams_running,
		                             liblinphone_tester_sip_timeout));
		...
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneConferenceStateCreated,
		                             ...
		                             liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneSubscriptionIncomingReceived,
		                             focus_stat.number_of_LinphoneSubscriptionIncomingReceived + (static_cast<int>(members.size()) * nb_subscriptions),
		                             liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneSubscriptionActive,
		                             focus_stat.number_of_LinphoneSubscriptionActive + (static_cast<int>(members.size()) * nb_subscriptions),
		                             liblinphone_tester_sip_timeout));

		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_participants_added,
		                             focus_stat.number_of_participants_added + static_cast<int>(members.size()), liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_participant_devices_added,
		                             focus_stat.number_of_participant_devices_added + static_cast<int>(members.size()),
		                             liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_conference_participant_devices_present,
		                             focus_stat.number_of_conference_participant_devices_present + static_cast<int>(members.size()),
		                             liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_participant_devices_present,
		...
		const std::initializer_list<std::reference_wrapper<CoreManager>> coreMgrs{focus, marie, pauline, laure, michelle, berthe};
		const std::initializer_list<std::reference_wrapper<ClientConference>> cores2{marie, pauline, laure, michelle, berthe};
		for (auto mgr : members) {
			stats mgr_stat = mgr->stat;
			std::string msg_text = std::string(linphone_core_get_identity(mgr->lc)) + std::string(" wishes you a welcome to conference ") + Address::toCpp(confAddr)->toString();
			send_text_message_and_check(coreMgrs, marie.getCMgr(), msg_text, confAddr, cores2);
			BC_ASSERT_FALSE(wait_for_list(coresList, &mgr->stat.number_of_LinphoneMessagePendingDelivery, mgr_stat.number_of_LinphoneMessagePendingDelivery + 1, 2000));
		}
...
static test_t local_conference_end_to_end_encryption_scheduled_conference_tests[] = {
    ...
    TEST_ONE_TAG("Create simple encrypted conference terminated early",
                 ...
                 "End2EndConf"),
    TEST_ONE_TAG("Create encrypted conference with chat sending messages", LinphoneTest::create_encrypted_conference_with_chat_sending_messages,
                 "End2EndConf"),
};

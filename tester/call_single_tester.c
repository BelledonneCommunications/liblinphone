...
static void call_with_in_dialog_update(void) {
	...
	const LinphoneCallParams *marie_remote_params = linphone_call_get_remote_params(marie_call);
	const char *value = linphone_call_params_get_custom_sdp_attribute(marie_remote_params, "weather");
	BC_ASSERT_PTR_NOT_NULL(value);
	if (value) BC_ASSERT_STRING_EQUAL(value, "bad");

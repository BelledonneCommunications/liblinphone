...
static void group_chat_room_with_client_restart_base (bool encrypted) {
	...
	{//to make sure focus is destroyed after clients.
		...
		BC_ASSERT_TRUE(CoreManagerAssert({focus,marie,michelle,michelle2}).waitUntil(chrono::seconds(10),[&focus,&marie,&michelle,&michelle2] {
			const std::initializer_list<std::reference_wrapper<CoreManager>> cores{focus,marie,michelle,michelle2};
...
static bool have_common_audio_payload(LinphoneCoreManager * mgr1, LinphoneCoreManager * mgr2) {
	...
	for(;elem!=NULL;elem=elem->next){
		...
		if (linphone_core_payload_type_enabled(mgr1->lc, pt1) == TRUE) {
			LinphonePayloadType* pt2 = linphone_core_get_payload_type(mgr2->lc, pt1->mime_type, pt1->clock_rate, pt1->channels);
...
static LinphoneAddress * create_conference_on_server(Focus & focus, ClientConference & organizer, std::list<LinphoneCoreManager*> requested_participants, time_t start_time, time_t end_time, const char * subject, const char * description, bool_t send_ics) {
	...
	BC_ASSERT_TRUE(wait_for_list(coresList, &organizer.getStats().number_of_ConferenceSchedulerStateAllocationPending, organizer_stat.number_of_ConferenceSchedulerStateAllocationPending + 1, liblinphone_tester_sip_timeout));
	...
	if (dialout) {
		...
		if (focus_organizer_common_payload) {
			...
			BC_ASSERT_TRUE(wait_for_list(coresList, &organizer.getStats().number_of_ConferenceSchedulerStateError, organizer_stat.number_of_ConferenceSchedulerStateError + 1, liblinphone_tester_sip_timeout));
		...
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneCallError, focus_stat.number_of_LinphoneCallError + call_errors_cnt, 10000));
	...
	organizer_expected_participant_number = requested_participants.size() + ((!found_me && dialout && focus_organizer_common_payload) ? 1 : 0);
	participant_expected_participant_number = requested_participants.size() + ((!found_me && dialout && focus_organizer_common_payload) ? 1 : 0);
	check_conference_info(organizer.getCMgr(), conference_address, organizer.getCMgr(), organizer_expected_participant_number, start_time, ((start_time > 0) && (end_time > 0)) ? (int)(end_time - start_time)/60 : 0, subject, description, 0, LinphoneConferenceInfoStateNew);
		...
		if (!dialout || !previous_calls[mgr]) {
				...
				if (cr) {
					...
					if (msg) {
						...
						if (BC_ASSERT_PTR_NOT_NULL(conf_info_from_original_content)) {
							...
							BC_ASSERT_EQUAL(bctbx_list_size(ics_participants), participant_expected_participant_number, size_t, "%zu");

							const bctbx_list_t * ics_participants_db = linphone_conference_info_get_participants(conf_info_in_db);
							BC_ASSERT_EQUAL(bctbx_list_size(ics_participants_db), participant_expected_participant_number, size_t, "%zu");
			...
			if (!!send_ics || ((end_time <= 0) && (start_time <= 0))) {
				...
				if (itConferenceMgrs != actual_participants.end()) {
					...
					check_conference_info(mgr, conference_address, organizer.getCMgr(), participant_expected_participant_number, start_time, ((start_time > 0) && (end_time > 0)) ? (int)(end_time - start_time)/60 : 0, subject, (!!send_ics) ? description : NULL, 0, LinphoneConferenceInfoStateNew);
					if (!!send_ics) {
						for (auto & p : participants) {
	...
	conference_address_str = (conference_address) ? linphone_address_as_string(conference_address) : ms_strdup("<unknown>");
...
static void create_conference_with_codec_mismatch_base (bool_t organizer_codec_mismatch) {
	...
	{//to make sure focus is destroyed after clients.
		...
		for (auto mgr : {focus.getCMgr(), marie.getCMgr(), pauline.getCMgr(), laure.getCMgr(), michelle.getCMgr(), berthe.getCMgr()}) {
			...
			if ((organizer_codec_mismatch && (mgr == marie.getCMgr())) || (!organizer_codec_mismatch && (mgr == michelle.getCMgr()))) {
		...
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_participants_removed, focus_stat.number_of_participants_removed + static_cast<int>(members.size()), 10000));
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_participant_devices_removed, focus_stat.number_of_participant_devices_removed + static_cast<int>(members.size()), 10000));

		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneCallEnd, focus_stat.number_of_LinphoneCallEnd + static_cast<int>(members.size()), 10000));
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneCallReleased, focus_stat.number_of_LinphoneCallReleased + static_cast<int>(members.size()), 10000));
...
static void create_conference_dial_out_base (bool_t send_ics, LinphoneConferenceLayout layout, bool_t enable_video, bool_t enable_stun, bool_t enable_ice, LinphoneConferenceParticipantListType participant_list_type, bool_t accept, bool_t participant_codec_mismatch) {
	Focus focus("chloe_rc");
	{//to make sure focus is destroyed after clients.
		ClientConference marie("marie_rc", focus.getIdentity().asAddress());
		ClientConference pauline("pauline_rc", focus.getIdentity().asAddress());
		...
		BC_ASSERT_TRUE(wait_for_list(coresList, &marie.getStats().number_of_LinphoneConferenceStateCreated, marie_conferences, liblinphone_tester_sip_timeout));
		...
		if (participant_codec_mismatch) {
			...
			for (const auto & m : codec_mismatch_members) {
				...
				bctbx_list_t * focus_call_log = linphone_core_get_call_history_2(focus.getLc(), m->identity, focus.getCMgr()->identity);
				...
				bctbx_list_t * member_call_log = linphone_core_get_call_history_2(m->lc, focus.getCMgr()->identity, m->identity);
		...
		if (confAddr) {
			for (auto mgr : participants) {
				check_conference_info(mgr, confAddr, marie.getCMgr(), 5, 0, 0, initialSubject, (accept && send_ics) ? description : NULL, 0, LinphoneConferenceInfoStateNew);
		...
		if (accept) {
			...
			for (auto mgr : participants) {
				...
				check_conference_info(mgr, confAddr, marie.getCMgr(), participant_conference_info_participants, 0, 0, initialSubject, (send_ics) ? description : NULL, 0, LinphoneConferenceInfoStateNew);
				...
				if (pcall) {
					...
					if (BC_ASSERT_PTR_NOT_NULL(call_log_info)) {
						...
						BC_ASSERT_EQUAL(bctbx_list_size(info_participants), participant_conference_info_participants, size_t, "%zu");
			...
			if (enable_ice) {
				BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneCallUpdating, focus_stat.number_of_LinphoneCallUpdating + participant_no - 1, liblinphone_tester_sip_timeout));
			}
			...
			BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneSubscriptionIncomingReceived, focus_stat.number_of_LinphoneSubscriptionIncomingReceived + participant_no, liblinphone_tester_sip_timeout));
			BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneSubscriptionActive, focus_stat.number_of_LinphoneSubscriptionActive + participant_no, liblinphone_tester_sip_timeout));

			BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_participants_added, focus_stat.number_of_participants_added + participant_no, liblinphone_tester_sip_timeout));
			BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_participant_devices_added, focus_stat.number_of_participant_devices_added + participant_no, liblinphone_tester_sip_timeout));
			BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_participant_devices_joined, focus_stat.number_of_participant_devices_joined + participant_no, liblinphone_tester_sip_timeout));
			...
			BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneCallEnd, focus_stat.number_of_LinphoneCallEnd + members_no, liblinphone_tester_sip_timeout));
			BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneCallReleased, focus_stat.number_of_LinphoneCallReleased + members_no, liblinphone_tester_sip_timeout));
			BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneSubscriptionTerminated, focus_stat.number_of_LinphoneSubscriptionTerminated + members_no, liblinphone_tester_sip_timeout));
			BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_participants_removed, focus_stat.number_of_participants_removed + members_no, liblinphone_tester_sip_timeout));
			BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_participant_devices_removed, focus_stat.number_of_participant_devices_removed + members_no, liblinphone_tester_sip_timeout));
			...
			BC_ASSERT_TRUE(wait_for_list(coresList, &marie.getStats().number_of_participants_removed, 4, 2*liblinphone_tester_sip_timeout));
		...
		for (auto mgr : members) {
			...
			check_conference_info(mgr, confAddr, marie.getCMgr(), (mgr == marie.getCMgr()) ? 5 : participant_conference_info_participants, 0, 0, initialSubject, ((accept && send_ics) || (mgr == marie.getCMgr())) ? description : NULL, 0, LinphoneConferenceInfoStateNew);
		}
...
static void create_simple_conference_dial_out (void) {
	create_conference_dial_out_base(FALSE, LinphoneConferenceLayoutActiveSpeaker, TRUE, FALSE, FALSE, LinphoneConferenceParticipantListTypeClosed, TRUE, FALSE);
}

static void create_simple_conference_dial_out_and_ics (void) {
	create_conference_dial_out_base(TRUE, LinphoneConferenceLayoutGrid, TRUE, TRUE, TRUE, LinphoneConferenceParticipantListTypeOpen, TRUE, FALSE);
}

static void create_simple_conference_dial_out_with_calls_declined (void) {
	create_conference_dial_out_base(FALSE, LinphoneConferenceLayoutGrid, TRUE, TRUE, TRUE, LinphoneConferenceParticipantListTypeOpen, FALSE, FALSE);
}

static void create_simple_conference_dial_out_participant_codec_mismatch (void) {
	create_conference_dial_out_base(FALSE, LinphoneConferenceLayoutActiveSpeaker, TRUE, FALSE, FALSE, LinphoneConferenceParticipantListTypeClosed, TRUE, TRUE);
}
...
static void create_simple_conference_dial_out_organizer_codec_mismatch (void) {
	...
	{//to make sure focus is destroyed after clients.
		...
		LinphoneAddress * confAddr = create_conference_on_server(focus, marie, participants, -1, -1, initialSubject, description, TRUE);
		BC_ASSERT_PTR_NOT_NULL(confAddr);

		// Chat room creation to send ICS
		BC_ASSERT_FALSE(wait_for_list(coresList, &marie.getStats().number_of_LinphoneConferenceStateCreated, marie_stat.number_of_LinphoneConferenceStateCreated + 1, liblinphone_tester_sip_timeout));
		BC_ASSERT_FALSE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneCallIncomingReceived, focus_stat.number_of_LinphoneCallIncomingReceived + 1, liblinphone_tester_sip_timeout));
		BC_ASSERT_FALSE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneConferenceStateCreationFailed, focus_stat.number_of_LinphoneConferenceStateCreationFailed + 1, liblinphone_tester_sip_timeout));
...
static test_t local_conference_scheduled_conference_basic_tests[] = {
	...
	TEST_NO_TAG("Create conference with participant codec mismatch", LinphoneTest::create_conference_with_participant_codec_mismatch),
	TEST_NO_TAG("Create conference with organizer codec mismatch", LinphoneTest::create_conference_with_organizer_codec_mismatch)
};
...
static test_t local_conference_conference_edition_tests[] = {
	...
	TEST_NO_TAG("Conference edition with organizer codec mismatch", LinphoneTest::conference_edition_with_organizer_codec_mismatch),

...
void check_delete_focus_conference_info(std::initializer_list<std::reference_wrapper<CoreManager>> coreMgrs,
                                        ...
                                        time_t end_time) {
	if (end_time > 0) {
		long focus_cleanup_period = linphone_core_get_conference_cleanup_period(focus->lc);
		long max_cleanup_period = 20;
		bool cleanup_period_too_big = (focus_cleanup_period > max_cleanup_period);
		if ((focus_cleanup_period > 0) && !cleanup_period_too_big) {
			...
			CoreManagerAssert(coreMgrs).waitUntil(chrono::seconds(focus_cleanup_period), [] { return false; });
		}

		// Compute here how much time is left to the conference to automatically account for the cleanup period delay
		...
		for (const auto &mgr : conferenceMgrs) {
			...
			if (is_focus && cleanup_period_too_big) {
				ms_message("Do not check conference information on the focus core because the cleanup period %0ld is "
				           ...
				           focus_cleanup_period, max_cleanup_period);
				if (info) {
			...
			if (is_focus && (focus_cleanup_period > 0)) {
				ms_message("Wait for %ld more seconds before checking the existence of conference information in %s's "
				           ...
				           focus_cleanup_period, linphone_core_get_identity(mgr->lc), conferenceAddressString,
				           time_left);
				CoreManagerAssert(coreMgrs).waitUntil(chrono::seconds(focus_cleanup_period), [&mgr, &confAddr] {
		...
		if (time_left > 0) {
			...
			if (focus_cleanup_period > 0) {
				wait_time += focus_cleanup_period;
			}
...
void does_all_participants_have_matching_ekt(LinphoneCoreManager *focus,
	...
	if (focusConference) {
		...
		if (conference_params) {
			auto security_level = linphone_conference_params_get_security_level(conference_params);
			if (security_level == LinphoneConferenceSecurityLevelEndToEnd) {
	auto firstClientCConf = linphone_core_search_conference_2(members.begin()->first->lc, confAddr);
				BC_ASSERT_PTR_NOT_NULL(firstClientCConf);
	if (!firstClientCConf) {
		return;
	}
	auto firstClientConf = dynamic_cast<const LinphonePrivate::ClientConference *>(Conference::toCpp(firstClientCConf));
	...
	{ // to make sure focus is destroyed after clients.
		...
		for (auto mgr : members) {
			...
			BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_LinphoneSubscriptionActive, nb_subscriptions,
			                             liblinphone_tester_sip_timeout));
			BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_conference_full_state_received, 1,
			                             liblinphone_tester_sip_timeout));
		...
		BC_ASSERT_TRUE(
		    wait_for_list(coresList, &focus.getStats().number_of_LinphoneSubscriptionIncomingReceived,
		                  focus_stat.number_of_LinphoneSubscriptionIncomingReceived + (no_members * nb_subscriptions),
		                  liblinphone_tester_sip_timeout));
...
void create_conference_with_screen_sharing_base(time_t start_time,
	...
	{ // to make sure focus is destroyed after clients.
		...
		for (auto mgr : members) {
			...
			BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_LinphoneSubscriptionOutgoingProgress,
			                             1 * nb_subscriptions, liblinphone_tester_sip_timeout));
		...
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneSubscriptionIncomingReceived,
		                             ...
		                             liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneSubscriptionActive,
		                             focus_stat.number_of_LinphoneSubscriptionActive + (5 * nb_subscriptions),
		                             liblinphone_tester_sip_timeout));
		...
		if (berthe_call) {
			...
			BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneCallStreamsRunning,
			                             ...
			                             liblinphone_tester_sip_timeout));
			BC_ASSERT_TRUE(check_screen_sharing_call(focus.getCMgr(), berthe.getCMgr(), berthe_can_screen_share));
		}
		...
		if (laure_call) {
			...
			linphone_call_set_video_source(laure_call, descriptor);
			linphone_video_source_descriptor_unref(descriptor);
		...
		if (berthe_call) {
			...
			BC_ASSERT_EQUAL(linphone_call_params_screen_sharing_enabled(call_cparams), 0, int, "%0d");
			BC_ASSERT_TRUE(check_screen_sharing_call(focus.getCMgr(), berthe.getCMgr(), FALSE));
		}
...
void create_conference_with_screen_sharing_chat_base(time_t start_time,
	...
	{ // to make sure focus is destroyed after clients.
		...
		for (auto mgr : members) {
			...
			if (mgr == pauline.getCMgr()) {
				ms_message("%s is willing to join conference %s sharing its screen and turning its camera %s",
		...
		if (!enable_camera) {
			ms_message("%s enables its camera on the call to conference %s",
			           linphone_core_get_identity(pauline.getLc()), conference_address_str);
			LinphoneCall *pauline_call = linphone_core_get_call_by_remote_address2(pauline.getLc(), confAddr);
			BC_ASSERT_PTR_NOT_NULL(pauline_call);
			if (pauline_call) {
				focus_stat = focus.getStats();
				...
				BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneCallStreamsRunning,
				                             focus_stat.number_of_LinphoneCallStreamsRunning + 1,
				                             liblinphone_tester_sip_timeout));
				BC_ASSERT_TRUE(check_screen_sharing_call(focus.getCMgr(), pauline.getCMgr(), TRUE));
			}
		}
		...
		if (accept) {
			...
			if (security_level == LinphoneConferenceSecurityLevelEndToEnd) {
				BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphonePublishRefreshing,
				                             focus_stat.number_of_LinphonePublishRefreshing + participant_added,
				                             liblinphone_tester_sip_timeout));
				BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphonePublishOk,
				                             focus_stat.number_of_LinphonePublishOk + participant_added,
				                             liblinphone_tester_sip_timeout));
				// wait a bit longer to receive all EKT packets
				CoreManagerAssert({focus, marie, pauline, laure, michelle, berthe}).waitUntil(chrono::seconds(2), [] {
					return false;
	...
	{ // to make sure focus is destroyed after clients.
		...
		for (auto mgr : members) {
			...
			BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_LinphoneChatRoomStateCreated,
			                             ((mgr == marie.getCMgr()) ? 4 : 1), liblinphone_tester_sip_timeout));
			BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_LinphoneSubscriptionOutgoingProgress,
			                             nb_client_subscribes, liblinphone_tester_sip_timeout));
			BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_LinphoneSubscriptionActive,
			                             nb_client_subscribes, liblinphone_tester_sip_timeout));
			BC_ASSERT_TRUE(wait_for_list(coresList, &mgr->stat.number_of_NotifyFullStateReceived, 1,
			                             liblinphone_tester_sip_timeout));
		...
		BC_ASSERT_TRUE(
		    wait_for_list(coresList, &focus.getStats().number_of_LinphoneSubscriptionIncomingReceived,
		                  focus_stat.number_of_LinphoneSubscriptionIncomingReceived + 5 * nb_client_subscribes,
		                  liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneSubscriptionActive,
		...
		if (!!client_reenter_conference) {
			...
			if (pcall) {
				...
				BC_ASSERT_TRUE(
				    wait_for_list(coresList, &marie.getStats().number_of_LinphoneSubscriptionOutgoingProgress,
				                  marie_stat.number_of_LinphoneSubscriptionOutgoingProgress + nb_client_subscribes,
				                  liblinphone_tester_sip_timeout));
				BC_ASSERT_TRUE(wait_for_list(coresList, &marie.getStats().number_of_LinphoneSubscriptionActive,
				                             marie_stat.number_of_LinphoneSubscriptionActive + nb_client_subscribes,
				                             liblinphone_tester_sip_timeout));
		...
		for (auto mgr : {marie.getCMgr(), pauline.getCMgr()}) {
			...
			if (!!join_after_termination) {
				...
				if (pcall) {
					...
					if (error_info) {
						long focus_cleanup_period = linphone_core_get_conference_cleanup_period(focus.getLc());
						LinphoneReason reason =
						    (focus_cleanup_period <= 0) ? LinphoneReasonForbidden : LinphoneReasonGone;
	...
	{ // to make sure focus is destroyed after clients.
		...
		for (int attempt = 1; attempt <= 5; attempt++) {
			...
			if (end_joining_window > 0) {
				...
				if (time_left <= 0) {
					ms_message("Iteration[%0d]: conference %s is likely to expire before all participants join it",
			...
			for (auto mgr : members) {
				...
				BC_ASSERT_PTR_NOT_NULL(call);
				if (call) {
					ms_message("Iteration[%0d]: %s is terminating call to %s", attempt,
					           linphone_core_get_identity(mgr->lc), conference_address_str);
					linphone_call_terminate(call);
				}
			...
			// Wait a little bit
			ms_message("Iteration[%0d]: waiting that the conference information cleanup timer expires", attempt);
			CoreManagerAssert({focus, marie, pauline, laure, michelle, berthe})
			    .waitUntil(chrono::seconds(2 * focus_cleanup_period), [] { return false; });

			for (auto mgr : conferenceMgrs) {
				bool info_deleted = false;
				if (!!linphone_core_conference_server_enabled(mgr->lc) && (focus_cleanup_period > 0) &&
				    (end_joining_window > 0)) {
					time_t now = ms_time(NULL);
					time_t time_left = end_joining_window - now;
					if (time_left <= 0) {
						ms_message("Iteration[%0d]: conference information of conference %s was deleted on the server "
...
<p1:list>\n\
  ...
  <p1:entry uri=\"sip:pauline@sip.example.org\"/>\n\
		...
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneConferenceStateCreated,
		                             ...
		                             liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneSubscriptionIncomingReceived,
		                             focus_stat.number_of_LinphoneSubscriptionIncomingReceived + 4,
		                             liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneSubscriptionActive,
...
void create_conference_with_audio_only_participants_base(LinphoneConferenceSecurityLevel security_level) {
	...
	{ // to make sure focus is destroyed after clients.
		...
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneSubscriptionIncomingReceived,
		                             ...
		                             liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneSubscriptionActive,
		                             focus_stat.number_of_LinphoneSubscriptionActive + (3 * nb_subscriptions),
		                             liblinphone_tester_sip_timeout));
		...
		BC_ASSERT_TRUE(wait_for_list(coresList, &berthe.getCMgr()->stat.number_of_LinphoneChatRoomStateCreated, 1,
		                             liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(wait_for_list(coresList, &berthe.getCMgr()->stat.number_of_LinphoneSubscriptionOutgoingProgress,
		                             1, liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(wait_for_list(coresList, &berthe.getCMgr()->stat.number_of_LinphoneSubscriptionActive, 1,
		                             liblinphone_tester_sip_timeout));
		...
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneSubscriptionIncomingReceived,
		                             ...
		                             liblinphone_tester_sip_timeout));
		BC_ASSERT_TRUE(wait_for_list(coresList, &focus.getStats().number_of_LinphoneSubscriptionActive,
		                             focus_stat.number_of_LinphoneSubscriptionActive + nb_subscriptions,
		                             liblinphone_tester_sip_timeout));

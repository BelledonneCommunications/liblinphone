 ...
 */
LINPHONE_PUBLIC void linphone_core_cbs_set_conference_info_received(LinphoneCoreCbs *cbs, LinphoneCoreCbsConferenceInfoReceivedCb cb);
 ...
 * When log collection is enabled, the function will close the file with the current prefix in the old path and it will open the new file with current prefix in the new path.
 ...
 * When log collection is enabled, the function will close the old file and it will open the new one in the current path.
 * If you need to change the path and the file at the same time, then you should deactivate log collection with linphone_core_enable_log_collection() before doing modifications.
 ...
 * When 'enable' is set to FALSE, software echo cancellation is disabled, but hardware one, if available, remains activated.
 * When set to TRUE, software echo cancellation is activated in either of these two conditions:
 ...
 * @param core #LinphoneCore object @notnil
 * @return the current calibration value, -1 if it failed, 0 if not done or not needed, a positive value if a software echo canceller is required after running linphone_core_start_echo_canceller_calibration().
 * @ingroup media_parameters
*/
 ...
 **/
LINPHONE_PUBLIC const LinphonePushNotificationConfig *linphone_core_get_push_notification_config(const LinphoneCore *core);
 ...
 * @param core A #LinphoneCore object @notnil
 * @param enable A boolean value telling whether to enable or disable the deletion of chat rooms with no participants in it
 */

...
class Conference {
  public:
	  ...
	  public:
		Params(const LinphoneCore *core = NULL) {
			...
			if (core) {
				...
				if (policy->automatically_initiate)
					m_enableVideo = true;
			}
	...
	const Params &getCurrentParams() const {
	...
	AudioStream *getAudioStream() const {
		return m_localParticipantStream;
	}
	int muteMicrophone(bool val);
	bool microphoneIsMuted() const {
		return m_isMuted;
	}
	float getInputVolume() const;

	virtual int getSize() const {
		return m_participants.size() + (isIn() ? 1 : 0);
	}
	...
	LinphoneConferenceState getState() const {
		return m_state;
	}
...
Conference::Conference(LinphoneCore *core, const Conference::Params *params)
	: m_core(core), m_localParticipantStream(NULL), m_isMuted(false), m_currentParams(),
...
LocalConference::LocalConference(LinphoneCore *core, const Conference::Params *params)
	: Conference(core, params), m_conf(NULL), m_localEndpoint(NULL), m_recordEndpoint(NULL), m_localDummyProfile(NULL),
	  m_terminating(FALSE) {
...
int RemoteConference::removeParticipant(const LinphoneAddress *uri) {
	...
	default:
		ms_error("Cannot remove %s from conference: Bad conference state (%s)", linphone_address_as_string(uri),
...
void RemoteConference::callStateChangedCb(LinphoneCore *lc, LinphoneCall *call, LinphoneCallState cstate,
										  const char *message) {
...
void RemoteConference::transferStateChanged(LinphoneCore *lc, LinphoneCall *transfered,
											LinphoneCallState new_call_state) {
	LinphoneCoreVTable *vtable = linphone_core_get_current_vtable(lc);
	RemoteConference *conf = (RemoteConference *)linphone_core_v_table_get_user_data(vtable);
	list<LinphoneCall *>::iterator it =
		find(conf->m_transferingCalls.begin(), conf->m_transferingCalls.end(), transfered);

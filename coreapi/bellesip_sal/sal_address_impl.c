...
const char *sal_address_get_scheme(const SalAddress *addr) {
	...
	} else if (generic_uri) return belle_generic_uri_get_scheme(generic_uri);
	else return NULL;
}
...
SalTransport sal_address_get_transport(const SalAddress *addr) {
	...
	if (transport) return sal_transport_parse(transport);
	else return SalTransportUDP;
};
...
char *sal_address_as_string(const SalAddress *addr) {
	return belle_sip_object_to_string(addr);
}
...
char *sal_address_as_string_uri_only(const SalAddress *addr) {
	...
	char *tmp = belle_sip_object_to_string(uri);
...
void sal_address_set_header(SalAddress *addr, const char *header_name, const char *header_value) {
	belle_sip_uri_set_header(belle_sip_header_address_get_uri(BELLE_SIP_HEADER_ADDRESS(addr)), header_name,
	                         header_value);
}
...
bool_t sal_address_is_ipv6(const SalAddress *addr) {
	...
	if (uri) {
		...
		if (host && strchr(host, ':') != NULL) return TRUE;
	}
	return FALSE;
}

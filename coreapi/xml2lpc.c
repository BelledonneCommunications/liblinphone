...
static void dumpContent(xmlNode *node, xml2lpc_context *ctx) {
	if (node->children) xml2lpc_log(ctx, XML2LPC_DEBUG, "content: %s", node->children->content);
	else xml2lpc_log(ctx, XML2LPC_DEBUG, "content: ");
}
...
static int processEntry(xmlElement *element, const char *sectionName, xml2lpc_context *ctx) {
	...
	if (element->children) value = (const char *)element->children->content;
	else value = "";
	...
	if (name != NULL) {
		...
		xml2lpc_log(ctx, XML2LPC_WARNING, "ignored entry with no \"name\" attribute line:%d",
		            xmlGetLineNo((xmlNode *)element));
	}
	return 0;
}
...
static int processSection(xmlElement *element, xml2lpc_context *ctx) {
	...
	if (name != NULL) {
		...
		xml2lpc_log(ctx, XML2LPC_WARNING, "ignored section with no \"name\" attribute, line:%d",
		            xmlGetLineNo((xmlNode *)element));
	}
...
static int processConfig(xmlElement *element, xml2lpc_context *ctx) {
	...
	for (cur_node = element->children; cur_node; cur_node = cur_node->next) {
		dumpNode(cur_node, ctx);
		if (cur_node->type == XML_ELEMENT_NODE && strcmp((const char *)cur_node->name, "section") == 0) {
...
static int processDoc(xmlNode *node, xml2lpc_context *ctx) {
	...
	if (node->type == XML_ELEMENT_NODE && strcmp((const char *)node->name, "config") == 0) {
...
int xml2lpc_validate(xml2lpc_context *xmlCtx) {
	...
	if (ret > 0) {
		if (strlen(xmlCtx->warningBuffer) > 0) xml2lpc_log(xmlCtx, XML2LPC_WARNING, "%s", xmlCtx->warningBuffer);
		if (strlen(xmlCtx->errorBuffer) > 0) xml2lpc_log(xmlCtx, XML2LPC_ERROR, "%s", xmlCtx->errorBuffer);
	} else if (ret < 0) {

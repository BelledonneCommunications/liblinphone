 ...
 * @param[in,out]	associatedKeys	Structure containing the self and peer URI. After this call contains all key
 * material associated to the given URI. Must be then freed through lime_freeKeys function
 *
 ...
 */
LINPHONE_PUBLIC int
lime_getCachedRcvKeyByZid(void *cachedb, limeKey_t *associatedKey, const char *selfURI, const char *peerURI);
 ...
 * tag Authentication tag is set at the begining of the encrypted Message
 *
 * @return 0 on success, error code otherwise
 */
LINPHONE_PUBLIC int lime_encryptMessage(limeKey_t *key,
                                        const uint8_t *plainMessage,
 ...
 */
LINPHONE_PUBLIC int
lime_encryptFile(void **cryptoContext, unsigned char *key, size_t length, char *plain, char *cipher);

/**
 * @brief Decrypt a file retrieved from server, decryption is done in several call, first one will be done with
 * cryptoContext null, last one with length = 0
 *
 * @param[in,out]	cryptoContext		The context used to decrypt the file using AES-GCM. Is created at first call(if
 * null)
 * @param[in]		key					256 bits : 192 bits of key || 64 bits of Initial Vector
 ...
 */
LINPHONE_PUBLIC int
lime_decryptFile(void **cryptoContext, unsigned char *key, size_t length, char *plain, char *cipher);
 ...
 * tag
 * + 1 for null termination char Authentication tag is retrieved at the begining of the encrypted Message
 *
 * @return 0 on success, error code otherwise
 */

LINPHONE_PUBLIC int lime_decryptMessage(
    limeKey_t *key, uint8_t *encryptedMessage, uint32_t messageLength, uint8_t selfZID[12], uint8_t *plainMessage);
 ...
 *
 * @param[in,out]	cachedb			Pointer to the sqlite DB holding zrtp/lime cache, get the keys and selfZID from it,
 * updated by this function with derivated keys
 ...
 * terminated string). Must be freed by caller
 * @return 	0 on success, error code otherwise
 */
LINPHONE_PUBLIC int lime_createMultipartMessage(void *cachedb,
 ...
 * @param[in,out]	cachedb			Pointer to the sqlite DB holding zrtp/lime cache, get the keys and selfZID from it,
 * updated by this function with derivated keys
 * @param[in]		message			The multipart message, contain one or several part identified by destination ZID,
 * one shall match the self ZID retrieved from cache
 * @param[in]		selfURI			The source URI
 * @param[in]		peerURI			The destination URI, associated keys will be found in cache
 * @param[out]		output			The output buffer, allocated and set with the decrypted message(null terminated
 * string). Must be freed by caller
 * @param[out]		content_type	The content type of the decrypted message
 * @param[in]		validityTimeSpan	If not 0, update the <valid> tag associated to sender to now+validityTimeSpan
 * (in seconds)
 * @return 	0 on success, error code otherwise
 */
LINPHONE_PUBLIC int lime_decryptMultipartMessage(void *cachedb,
                                                 uint8_t *message,
                                                 const char *selfURI,
                                                 const char *peerURI,
                                                 uint8_t **output,
                                                 char **content_type,
                                                 uint64_t validityTimeSpan);
...
int lime_im_encryption_engine_process_incoming_message_cb(LinphoneImEncryptionEngine *engine,
                                                          LinphoneChatRoom *room,
                                                          LinphoneChatMessage *msg);

int lime_im_encryption_engine_process_outgoing_message_cb(LinphoneImEncryptionEngine *engine,
                                                          LinphoneChatRoom *room,
                                                          LinphoneChatMessage *msg);

int lime_im_encryption_engine_process_downloading_file_cb(LinphoneImEncryptionEngine *engine,
                                                          LinphoneChatMessage *msg,
                                                          size_t offset,
...
int lime_im_encryption_engine_process_uploading_file_cb(LinphoneImEncryptionEngine *engine,
                                                        LinphoneChatMessage *msg,
                                                        size_t offset,
                                                        const uint8_t *buffer,
                                                        size_t *size,
...
bool_t lime_im_encryption_engine_is_file_encryption_enabled_cb(LinphoneImEncryptionEngine *engine,
                                                               LinphoneChatRoom *room);

void lime_im_encryption_engine_generate_file_transfer_key_cb(LinphoneImEncryptionEngine *engine,
                                                             LinphoneChatRoom *room,
                                                             LinphoneChatMessage *msg);

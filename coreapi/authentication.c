...
static const LinphoneAuthInfo *find_auth_info(LinphoneCore *lc, const char *username, const char *realm, const char *domain, bool_t ignore_realm){
	...
	for (elem=lc->auth_info;elem!=NULL;elem=elem->next) {
		...
		if (username && strlen(linphone_auth_info_get_username(pinfo)) !=0 && strcmp(username, linphone_auth_info_get_username(pinfo))==0) 
		{
			if (realm && domain){
				if (strlen(linphone_auth_info_get_realm(pinfo))!=0 && realm_match(realm, linphone_auth_info_get_realm(pinfo))
					&& strlen(linphone_auth_info_get_domain(pinfo)) !=0 && strcmp(domain, linphone_auth_info_get_domain(pinfo))==0) {
					return pinfo;
				}
			} else if (realm) {
				if (strlen(linphone_auth_info_get_realm(pinfo)) !=0 && realm_match(realm, linphone_auth_info_get_realm(pinfo))) {
			...
			} else if (domain && strlen(linphone_auth_info_get_domain(pinfo)) !=0 && strcmp(domain,linphone_auth_info_get_domain(pinfo))==0 && (linphone_auth_info_get_ha1(pinfo)==NULL || ignore_realm)) {
				return pinfo;
...
const LinphoneAuthInfo *_linphone_core_find_tls_auth_info(LinphoneCore *lc) {
	...
	for (elem=lc->auth_info;elem!=NULL;elem=elem->next) {
		LinphoneAuthInfo *pinfo = (LinphoneAuthInfo*)elem->data;
		if (strlen(linphone_auth_info_get_tls_cert(pinfo)) !=0 && strlen(linphone_auth_info_get_tls_key(pinfo))!= 0) {
			return pinfo;
		} else if (strlen(linphone_auth_info_get_tls_cert_path(pinfo)) !=0 && strlen(linphone_auth_info_get_tls_key_path(pinfo))!=0) {
			return pinfo;
		}
	}
	return NULL;
...
void linphone_core_add_auth_info(LinphoneCore *lc, const LinphoneAuthInfo *info){
	...
	if (strlen(linphone_auth_info_get_tls_key(info)) == 0 && strlen(linphone_auth_info_get_tls_key_path(info)) != 0 && strlen(linphone_auth_info_get_ha1(info)) !=0 && strlen(linphone_auth_info_get_password(info)) !=0 ){
	...
	if (ai!=NULL && strlen(linphone_auth_info_get_domain(ai)) != 0 && strlen(linphone_auth_info_get_domain(info)) !=0 && strcmp(linphone_auth_info_get_domain(ai), linphone_auth_info_get_domain(info))==0){
	...
	for (const auto &op : pendingAuths) {
		...
		if (ai){
			...
			if (strlen(linphone_auth_info_get_tls_cert(ai)) !=0 && strlen(linphone_auth_info_get_tls_key(ai)) != 0) {
			...
			} else if ( strlen(linphone_auth_info_get_tls_cert_path(ai)) !=0 && strlen(linphone_auth_info_get_tls_key_path(ai)) !=0) {

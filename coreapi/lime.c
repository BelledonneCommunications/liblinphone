...
int lime_getCachedSndKeysByURI(void *cachedb, limeURIKeys_t *associatedKeys) {
	...
	/* query the DB: join ziduri, lime and zrtp tables : retrieve zuid(for easier key update in cache), peerZID, sndKey,
	 * sndSId, sndIndex, valid where self and peer ZIDs are matching constraint and pvs is raised */
	/* Note: retrieved potentially expired keys, just to be able to send a different status to caller(no keys found is
	 * not expired key found) */
	...
	if (associatedKeys->selfURI == NULL) {
		stmt = sqlite3_mprintf(
		    "SELECT zu.zuid, zu.zid as peerZID, l.sndkey, l.sndSId, l.sndIndex, l.valid FROM ziduri as zu INNER JOIN "
		    "zrtp as z ON z.zuid=zu.zuid INNER JOIN lime as l ON z.zuid=l.zuid WHERE zu.peeruri=? AND z.pvs=?;");
		...
		stmt = sqlite3_mprintf("SELECT zu.zuid, zu.zid as peerZID, l.sndkey, l.sndSId, l.sndIndex, l.valid FROM ziduri "
		                       "as zu INNER JOIN zrtp as z ON z.zuid=zu.zuid INNER JOIN lime as l ON z.zuid=l.zuid "
		                       "WHERE zu.selfuri=? AND zu.peeruri=? AND z.pvs=?;");
		ret = sqlite3_prepare_v2(db, stmt, -1, &sqlStmt, NULL);
		sqlite3_free(stmt);
		if (ret != SQLITE_OK) {
			return LIME_INVALID_CACHE;
	...
	while ((ret = sqlite3_step(sqlStmt)) == SQLITE_ROW) {
		...
		if (length == 4) { /* sndIndex : 4 bytes of a uint32_t, stored as a blob in big endian */
			...
			currentPeerKey->sessionIndex = ((uint32_t)(sessionId[0])) << 24 | ((uint32_t)(sessionId[1])) << 16 |
			                               ((uint32_t)(sessionId[2])) << 8 | ((uint32_t)(sessionId[3]));
		} else { /* something wrong with that one, skip it */
			continue;
		}
		...
		if (length == 8) { /* sndIndex : 8 bytes of a int64_t, stored as a blob in big endian */
			...
			validityTimeSpec.tv_sec = (int64_t)(((uint64_t)(validity[0])) << 56 | ((uint64_t)(validity[1])) << 48 |
			                                    ((uint64_t)(validity[2])) << 40 | ((uint64_t)(validity[3])) << 32 |
			                                    ((uint64_t)(validity[4])) << 24 | ((uint64_t)(validity[5])) << 16 |
			                                    ((uint64_t)(validity[6])) << 8 | ((uint64_t)(validity[7])));
		} else { /* something wrong with that one, skip it */
			continue;
		}
		...
		if (validityTimeSpec.tv_sec == 0 || bctbx_timespec_compare(&currentTimeSpec, &validityTimeSpec) < 0) {
			...
			associatedKeys->peerKeys = (limeKey_t **)bctbx_realloc(
			    associatedKeys->peerKeys, (associatedKeys->associatedZIDNumber) * sizeof(limeKey_t *));
...
int lime_getCachedRcvKeyByZid(void *cachedb, limeKey_t *associatedKey, const char *selfURI, const char *peerURI) {
	...
	/* retrieve zuid(for easier key update in cache), rcvKey, rcvSId, rcvIndex where self/peer uris and peer zid are
	 * matching constraint(unique row) and pvs is raised */
	stmt = sqlite3_mprintf(
	    "SELECT zu.zuid, l.rcvkey, l.rcvSId, l.rcvIndex FROM ziduri as zu INNER JOIN zrtp as z ON z.zuid=zu.zuid INNER "
	    "JOIN lime as l ON z.zuid=l.zuid WHERE zu.selfuri=? AND zu.peeruri=? AND zu.zid=? AND z.pvs=? LIMIT 1;");
	ret = sqlite3_prepare_v2(db, stmt, -1, &sqlStmt, NULL);
	sqlite3_free(stmt);
	if (ret != SQLITE_OK) {
	...
	if ((ret = sqlite3_step(sqlStmt)) == SQLITE_ROW) { /* we found a row */
		...
		if (length == 4) { /* rcvIndex */
			...
			associatedKey->sessionIndex = ((uint32_t)(sessionId[0])) << 24 | ((uint32_t)(sessionId[1])) << 16 |
			                              ((uint32_t)(sessionId[2])) << 8 | ((uint32_t)(sessionId[3]));
		} else { /* something wrong */
			sqlite3_finalize(sqlStmt);
...
int lime_setCachedKey(void *cachedb, limeKey_t *associatedKey, uint8_t role, uint64_t validityTimeSpan) {
	...
	size_t colLength[] = {32, 32, 4, 8}; /* data length: keys and session ID : 32 bytes, Index: 4 bytes(uint32_t),
	...
	/* update cache */
	return bzrtp_cache_write(cachedb, associatedKey->zuid, "lime",
	                         role == LIME_SENDER ? colNamesSender : colNamesReceiver, colValues, colLength, colNums);
}
 ...
 *
 * @param[in/out]	key		The structure containing the original key which will be overwritten, the sessionId and
 * SessionIndex
 *
...
static int lime_deriveKey(limeKey_t *key) {
	...
	/* Derivation is made derived Key = HMAC_SHA256(Key,
	 * 0x0000001||"MessageKey"||0x00||SessionId||SessionIndex||0x00000100)*/
...
int lime_encryptMessage(limeKey_t *key,
	...
	return bctbx_aes_gcm_encrypt_and_tag(
	    key->key, 24, plainMessage, messageLength, authenticatedData, 28, key->key + 24,
	    8,                      /* IV is at the end(last 64 bits) of the given key buffer */
...
int lime_decryptMessage(
	...
	retval = bctbx_aes_gcm_decrypt_and_auth(
	    key->key, 24, /* key is 192 bits long */
	    encryptedMessage + 16,
	    messageLength -
	        16, /* encrypted message first 16 bytes store the authentication tag, then is the actual message */
...
int lime_createMultipartMessage(void *cachedb,
	...
	for (i = 0; i < associatedKeys.associatedZIDNumber; i++) {
		...
		lime_encryptMessage(currentKey, (const uint8_t *)contentType, (uint32_t)strlen((char *)contentType), selfZid,
		...
		bctbx_base64_encode(
		    NULL, &b64Size, encryptedMessage,
		    encryptedMessageLength); /* b64Size is 0, so it is set to the requested output buffer size */
		encryptedMessageb64 = reinterpret_cast<unsigned char *>(
		    ms_malloc(b64Size + 1)); /* allocate a buffer of requested size +1 for NULL termination */
		bctbx_base64_encode(
		    encryptedMessageb64, &b64Size, encryptedMessage,
		    encryptedMessageLength);         /* b64Size is 0, so it is set to the requested output buffer size */
		...
		b64Size = 0;
		bctbx_base64_encode(
		    NULL, &b64Size, encryptedContentType,
		    encryptedContentTypeLength); /* b64Size is 0, so it is set to the requested output buffer size */
		encryptedContentTypeb64 = reinterpret_cast<unsigned char *>(
		    ms_malloc(b64Size + 1)); /* allocate a buffer of requested size +1 for NULL termination */
		bctbx_base64_encode(
		    encryptedContentTypeb64, &b64Size, encryptedContentType,
		    encryptedContentTypeLength);         /* b64Size is 0, so it is set to the requested output buffer size */
...
int lime_decryptMultipartMessage(void *cachedb,
                                 uint8_t *message,
                                 const char *selfURI,
                                 const char *peerURI,
                                 uint8_t **output,
	...
	/* retrieve selfZID from cache, and convert it to an Hexa buffer to easily match it against hex string containg in
	 * xml message as pzid */
	if (bzrtp_getSelfZID(cachedb, selfURI, selfZid, NULL) != 0) {
	...
	if (peerZidHex != NULL) {
		...
		if (retval != 0) {
			ms_error("[LIME] Couldn't get cache rcv key by ZID. Returns %04x. PeerZid %s peerURI %s selfURI %s", retval,
			         peerZidHex, peerURI, selfURI);
			...
			for (i = 1; i <= msg_object->nodesetval->nodeNr; i++) {
				...
				if ((currentZidHex != NULL) && (strcmp(currentZidHex, (char *)selfZidHex) == 0)) {
					...
					if (encryptedMessageb64 != NULL) {
						bctbx_base64_decode(NULL, &encryptedMessageLength, (const unsigned char *)encryptedMessageb64,
						                    strlen(encryptedMessageb64)); /* encryptedMessageLength is 0, so it will be
						                                                     set to the requested buffer length */
						encryptedMessage = (uint8_t *)ms_malloc(encryptedMessageLength);
						bctbx_base64_decode(encryptedMessage, &encryptedMessageLength,
						                    (const unsigned char *)encryptedMessageb64, strlen(encryptedMessageb64));
					...
					if (encryptedContentTypeb64 != NULL) {
						bctbx_base64_decode(
						    NULL, &encryptedContentTypeLength, (const unsigned char *)encryptedContentTypeb64,
						    strlen(encryptedContentTypeb64)); /* encryptedContentTypeLength is 0, so it will be set to
						                                         the requested buffer length */
						encryptedContentType = (uint8_t *)ms_malloc(encryptedContentTypeLength);
						bctbx_base64_decode(encryptedContentType, &encryptedContentTypeLength,
						                    (const unsigned char *)encryptedContentTypeb64,
						                    strlen(encryptedContentTypeb64));
	...
	if (usedSessionIndex < associatedKey.sessionIndex) {
		...
		ms_error("[LIME] Session index [%s] < associated key's session index [%s], should not happen !",
		         sessionIndexHex, associatedKeyIndexHex);
		ms_free(encryptedMessage);
		retval = LIME_UNABLE_TO_DECRYPT_MESSAGE;
		goto error;
	}
	...
	if ((usedSessionIndex - associatedKey.sessionIndex > MAX_DERIVATION_NUMBER)) {
		...
		ms_error("[LIME] Too many messages missed (%i), cache should be reset by ZRTP call",
	...
	if (associatedKey.sessionIndex != usedSessionIndex) {
		...
		bctbx_uint32_to_str(associatedKeyIndexHex, associatedKey.sessionIndex);
		ms_warning(
		    "LIME] unexpected session index [%s] received from [%s] (expected [%s]), [%i] messages will be discarded",
	...
	/* Decrypt the message */
	*output = (uint8_t *)ms_malloc(encryptedMessageLength - 16 +
	                               1); /* plain message is same length than encrypted one with 16 bytes less for the tag
	                                      + 1 to add the null termination char */
	...
	if (encryptedContentType != NULL) {
		*content_type = (char *)ms_malloc(encryptedContentTypeLength - 16 +
		                                  1); /* content-type is same length than encrypted one with 16 bytes less for
		                                         the tag + 1 to add the null termination char */
		retval = lime_decryptMessage(&associatedKey, encryptedContentType, (uint32_t)encryptedContentTypeLength,
		                             selfZid, *((uint8_t **)content_type));
		ms_free(encryptedContentType);
		if (retval != 0) {
		...
		switch (linphone_core_lime_enabled(linphone_chat_room_get_core(cr))) {
			...
			case LinphoneLimePreferred: {
				...
				if (zrtp_cache_db != NULL) {
					...
					char *peer = ms_strdup_printf("%s:%s@%s", linphone_address_get_scheme(peerAddr),
					                              linphone_address_get_username(peerAddr),
					                              linphone_address_get_domain(peerAddr));
					/* retrieve keys associated to the peer URI */
					associatedKeys.peerURI = bctbx_strdup(peer);
					associatedKeys.selfURI = NULL; /* TODO : there is no sender associated to chatroom so check for any
					                                  local URI available, shall we add sender to chatroom? */
					associatedKeys.associatedZIDNumber = 0;
					associatedKeys.peerKeys = NULL;
					/* with NULL is selfURI, just retrieve keys for any local uri found in cache, shall we use a
					dedicated function which would return the list of possible uris and store the selected one in the
					chatroom ? */
...
int lime_im_encryption_engine_process_incoming_message_cb(LinphoneImEncryptionEngine *engine,
	...
	if (linphone_chat_message_get_content_type(msg) &&
		...
		errcode = 0;
		int retval;
		void *zrtp_cache_db = NULL; /* use a void * instead of sqlite3 * to avoid problems and ifdef when SQLITE is not
		                               available(the get function shall return NULL in that case) */
		...
		peerUri = ms_strdup_printf("%s:%s@%s", linphone_address_get_scheme(fromAddr),
		                           linphone_address_get_username(fromAddr), linphone_address_get_domain(fromAddr));

		const LinphoneAddress *toAddr = linphone_chat_message_get_to_address(msg);
		selfUri = ms_strdup_printf("%s:%s@%s", linphone_address_get_scheme(toAddr),
		                           linphone_address_get_username(toAddr), linphone_address_get_domain(toAddr));

		retval = lime_decryptMultipartMessage(
		    zrtp_cache_db, (uint8_t *)linphone_chat_message_get_text(msg), selfUri, peerUri, &decrypted_body,
		...
		if (retval != 0) {
			...
			if (decrypted_content_type != NULL) {
				...
				if (strcmp("application/cipher.vnd.gsma.rcs-ft-http+xml",
				           linphone_chat_message_get_content_type(msg)) == 0) {
		...
		ms_message("Content type is unknown (%s), don't try to decrypt it",
...
int lime_im_encryption_engine_process_outgoing_message_cb(LinphoneImEncryptionEngine *engine,
	...
	if (linphone_core_lime_enabled(linphone_chat_room_get_core(room))) {
		if (linphone_chat_room_lime_available(room)) {
			void *zrtp_cache_db = NULL; /* use a void * instead of sqlite3 * to avoid problems and ifdef when SQLITE is
			                               not available(the get function shall return NULL in that case) */
			if (linphone_chat_message_get_content_type(msg)) {
			...
			if (zrtp_cache_db == NULL) {
				...
				char *peerUri =
				    ms_strdup_printf("%s:%s@%s", linphone_address_get_scheme(peerAddr),
				                     linphone_address_get_username(peerAddr), linphone_address_get_domain(peerAddr));
				const LinphoneAddress *fromAddr = linphone_chat_message_get_from_address(msg);
				char *selfUri =
				    ms_strdup_printf("%s:%s@%s", linphone_address_get_scheme(fromAddr),
				                     linphone_address_get_username(fromAddr), linphone_address_get_domain(fromAddr));

				retval = lime_createMultipartMessage(zrtp_cache_db, linphone_chat_message_get_content_type(msg),
				                                     (uint8_t *)linphone_chat_message_get_text(msg), selfUri, peerUri,
				                                     &crypted_body);
...
int lime_im_encryption_engine_process_downloading_file_cb(LinphoneImEncryptionEngine *engine,
	...
	if (!linphone_content_get_key(content)) return -1;

	if (!buffer || size == 0)
		return bctbx_aes_gcm_decryptFile(linphone_content_get_cryptoContext_address(content), NULL, 0, NULL, NULL);

	return bctbx_aes_gcm_decryptFile(linphone_content_get_cryptoContext_address(content),
	                                 (unsigned char *)linphone_content_get_key(content), size, (char *)decrypted_buffer,
	                                 (char *)buffer);
}
...
int lime_im_encryption_engine_process_uploading_file_cb(LinphoneImEncryptionEngine *engine,
	...
	return bctbx_aes_gcm_encryptFile(linphone_content_get_cryptoContext_address(content),
	                                 (unsigned char *)linphone_content_get_key(content), *size, (char *)buffer,
...
void lime_im_encryption_engine_generate_file_transfer_key_cb(LinphoneImEncryptionEngine *engine,
	...
	LinphoneContent *content = linphone_chat_message_get_file_transfer_information(msg);
	if (content)
		linphone_content_set_key(content, keyBuffer,
		                         FILE_TRANSFER_KEY_SIZE); /* key is duplicated in the content private structure */
}

#else /* HAVE_LIME */

bool_t lime_is_available() {
	return FALSE;
}
int lime_decryptMultipartMessage(BCTBX_UNUSED(void *cachedb),
                                 BCTBX_UNUSED(uint8_t *message),
                                 BCTBX_UNUSED(const char *selfURI),
                                 BCTBX_UNUSED(const char *peerURI),
                                 BCTBX_UNUSED(uint8_t **output),
                                 BCTBX_UNUSED(char **content_type),
                                 BCTBX_UNUSED(uint64_t validityTimeSpan)) {
	return LIME_NOT_ENABLED;
}
int lime_createMultipartMessage(BCTBX_UNUSED(void *cachedb),
                                BCTBX_UNUSED(const char *contentType),
                                BCTBX_UNUSED(uint8_t *message),
                                BCTBX_UNUSED(const char *selfURI),
                                BCTBX_UNUSED(const char *peerURI),
                                BCTBX_UNUSED(uint8_t **output)) {
...
void lime_freeKeys(BCTBX_UNUSED(limeURIKeys_t *associatedKeys)) {
}
int lime_getCachedSndKeysByURI(BCTBX_UNUSED(void *cachedb), BCTBX_UNUSED(limeURIKeys_t *associatedKeys)) {
	return LIME_NOT_ENABLED;
}
int lime_encryptMessage(BCTBX_UNUSED(limeKey_t *key),
                        BCTBX_UNUSED(const uint8_t *plainMessage),
                        BCTBX_UNUSED(uint32_t messageLength),
                        BCTBX_UNUSED(uint8_t selfZID[12]),
                        BCTBX_UNUSED(uint8_t *encryptedMessage)) {
	return LIME_NOT_ENABLED;
}
int lime_setCachedKey(BCTBX_UNUSED(void *cacheDb),
                      BCTBX_UNUSED(limeKey_t *associatedKey),
                      BCTBX_UNUSED(uint8_t role),
                      BCTBX_UNUSED(uint64_t validityTimeSpan)) {
	return LIME_NOT_ENABLED;
}
int lime_getCachedRcvKeyByZid(BCTBX_UNUSED(void *cacheDb),
                              BCTBX_UNUSED(limeKey_t *associatedKey),
                              BCTBX_UNUSED(const char *selfURI),
                              BCTBX_UNUSED(const char *peerURI)) {
	return LIME_NOT_ENABLED;
}
int lime_decryptMessage(BCTBX_UNUSED(limeKey_t *key),
                        BCTBX_UNUSED(uint8_t *encryptedMessage),
                        BCTBX_UNUSED(uint32_t messageLength),
                        BCTBX_UNUSED(uint8_t selfZID[12]),
                        BCTBX_UNUSED(uint8_t *plainMessage)) {
	return LIME_NOT_ENABLED;
}
bool_t linphone_chat_room_lime_available(BCTBX_UNUSED(LinphoneChatRoom *cr)) {
	return FALSE;
}
int lime_im_encryption_engine_process_incoming_message_cb(BCTBX_UNUSED(LinphoneImEncryptionEngine *engine),
                                                          BCTBX_UNUSED(LinphoneChatRoom *room),
                                                          BCTBX_UNUSED(LinphoneChatMessage *msg)) {
	return 500;
}
int lime_im_encryption_engine_process_outgoing_message_cb(BCTBX_UNUSED(LinphoneImEncryptionEngine *engine),
                                                          BCTBX_UNUSED(LinphoneChatRoom *room),
                                                          BCTBX_UNUSED(LinphoneChatMessage *msg)) {
	return 500;
}
int lime_im_encryption_engine_process_downloading_file_cb(BCTBX_UNUSED(LinphoneImEncryptionEngine *engine),
                                                          BCTBX_UNUSED(LinphoneChatMessage *msg),
                                                          BCTBX_UNUSED(size_t offset),
                                                          BCTBX_UNUSED(const uint8_t *buffer),
                                                          BCTBX_UNUSED(size_t size),
                                                          BCTBX_UNUSED(uint8_t *decrypted_buffer)) {
	return 500;
}
int lime_im_encryption_engine_process_uploading_file_cb(BCTBX_UNUSED(LinphoneImEncryptionEngine *engine),
                                                        BCTBX_UNUSED(LinphoneChatMessage *msg),
                                                        BCTBX_UNUSED(size_t offset),
                                                        BCTBX_UNUSED(const uint8_t *buffer),
                                                        BCTBX_UNUSED(size_t *size),
                                                        BCTBX_UNUSED(uint8_t *encrypted_buffer)) {
	return 500;
}
bool_t lime_im_encryption_engine_is_file_encryption_enabled_cb(BCTBX_UNUSED(LinphoneImEncryptionEngine *engine),
                                                               BCTBX_UNUSED(LinphoneChatRoom *room)) {
	return FALSE;
}
void lime_im_encryption_engine_generate_file_transfer_key_cb(BCTBX_UNUSED(LinphoneImEncryptionEngine *engine),
                                                             BCTBX_UNUSED(LinphoneChatRoom *room),
                                                             BCTBX_UNUSED(LinphoneChatMessage *msg)) {
}
...
const char *lime_error_code_to_string(int errorCode) {
	switch (errorCode) {
		case LIME_INVALID_CACHE:
			return "Invalid ZRTP cache";
		case LIME_UNABLE_TO_DERIVE_KEY:
			return "Unable to derive Key";
		case LIME_UNABLE_TO_ENCRYPT_MESSAGE:
			return "Unable to encrypt message";
		case LIME_UNABLE_TO_DECRYPT_MESSAGE:
			return "Unable to decrypt message";
		case LIME_NO_VALID_KEY_FOUND_FOR_PEER:
			return "No valid key found";
		case LIME_PEER_KEY_HAS_EXPIRED:
			return "Any key matching peer Uri has expired";
		case LIME_INVALID_ENCRYPTED_MESSAGE:
			return "Invalid encrypted message";
		case LIME_NOT_ENABLED:
			return "Lime not enabled at build";
	}
	return "Unknow error";
}

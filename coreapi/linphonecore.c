...
static void linphone_core_init(LinphoneCore * lc, LinphoneCoreCbs *cbs, LpConfig *config, void * userdata, void *system_context, bool_t automatically_start) {
	...
	if (lc->platform_helper == NULL) {
		...
		getPlatformHelpers(lc)->createBasicPlatformHelpers();
	}
#endif
	if (lc->platform_helper == NULL)
...
void linphone_core_unref(LinphoneCore *lc) {
	...
	if (lc->platform_helper) {
		bctbx_message("delete platformHelpers");
...
static void _linphone_core_stop_async_end(LinphoneCore *lc) {
	...
	if (lc->platform_helper) getPlatformHelpers(lc)->destroyBasicPlatformHelpers();
#else

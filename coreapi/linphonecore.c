...
static void bodyless_config_read(LinphoneCore *lc) {
	...
	while (bodyless_lists) {
		...
		if (!linphone_friend_list_subscriptions_enabled(friendList)) {
			ms_warning("Newly created bodyless friend list [%p](%s) subscriptions are disabled...", friendList, linphone_friend_list_get_display_name(friendList));
		}
...
static void misc_config_read (LinphoneCore *lc) {
	...
	if (contacts_vcard_list_uri) {
		if(lc->base_contacts_list_for_synchronization) linphone_friend_list_unref(lc->base_contacts_list_for_synchronization);
...
LinphoneAccountCreatorBackend linphone_core_get_account_creator_backend(const LinphoneCore *lc) {
	return (LinphoneAccountCreatorBackend)linphone_config_get_int(lc->config, "account_creator", "backend", (int)LinphoneAccountCreatorBackendXMLRPC);
}
...
static void _linphone_core_init_account_creator_service(LinphoneCore *lc) {
	...
	if (linphone_core_get_account_creator_backend(lc) == LinphoneAccountCreatorBackendXMLRPC) {
		...
		linphone_account_creator_service_set_is_alias_used_cb(service, linphone_account_creator_is_phone_number_used_flexiapi);
		...
		linphone_account_creator_service_set_recover_account_cb(service, linphone_account_creator_recover_phone_account_flexiapi);
		linphone_account_creator_service_set_login_linphone_account_cb(service, linphone_account_creator_login_linphone_account_flexiapi);
	}
	#endif
...
LinphoneProxyConfig * linphone_core_lookup_known_proxy_2(LinphoneCore *lc, const LinphoneAddress *uri, bool_t fallback_to_default){
...
const char *linphone_core_get_srtp_crypto_suites(LinphoneCore *core) {
	return linphone_config_get_string(core->config, "sip", "srtp_crypto_suites", "AEAD_AES_128_GCM, AES_CM_128_HMAC_SHA1_80, AEAD_AES_256_GCM, AES_256_CM_HMAC_SHA1_80");
}

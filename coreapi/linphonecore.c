...
void _linphone_core_uninit(LinphoneCore *lc)
{
	...
	if (lc->supported_encryptions) {
		bctbx_list_free_with_data(lc->supported_encryptions, (bctbx_list_free_func)bctbx_free);
		lc->supported_encryptions = NULL;
	}
...
void linphone_core_set_supported_media_encryptions(LinphoneCore *lc, const bctbx_list_t * enc_list) {
...
const bctbx_list_t * linphone_core_get_supported_media_encryptions(LinphoneCore *lc) {
	...
	if (!capability_negotiation_supported || (supported_encryptions == NULL)) {
		...
		if (default_encryption_list) {
			lc->supported_encryptions = bctbx_list_copy_with_data(default_encryption_list, (bctbx_list_copy_func)bctbx_strdup);
			bctbx_list_free_with_data(default_encryption_list, (bctbx_list_free_func)bctbx_free);
		}
	} else {
		lc->supported_encryptions = bctbx_list_copy_with_data(supported_encryptions, (bctbx_list_copy_func)bctbx_strdup);
	}

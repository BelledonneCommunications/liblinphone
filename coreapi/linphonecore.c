...
void linphone_core_send_conference_information(LinphoneCore *core, LinphoneConferenceInfo *conference_information, const char *text) {
	...
	while (participants != NULL) {
		bctbx_list_t *add_participant = bctbx_list_append(NULL, (LinphoneAddress *) bctbx_list_get_data(participants));
		LinphoneChatRoomParams *chat_room_params = linphone_core_create_default_chat_room_params(core);

		LinphoneChatRoom *cr = linphone_core_create_chat_room_6(core, chat_room_params, NULL, add_participant);

...
void linphone_core_set_media_resource_mode (LinphoneCore *lc, const char *mode) {
	if ((strcmp(mode, "shared") == 0) || (strcmp(mode, "unique") == 0)) {
		linphone_config_set_string(linphone_core_get_config(lc), "misc", "media_resources_mode", mode);
	} else {
		ms_error("Chosen mode %s is not among the list of available modes: unique shared - keeping current value %s", mode, linphone_core_get_media_resource_mode(lc));
	}
}

const char *linphone_core_get_media_resource_mode (const LinphoneCore *lc) {
	return linphone_config_get_string(linphone_core_get_config(lc), "misc", "media_resources_mode", "unique");
}

// This function states whether a locking of the sound resources is required based on the given call parameters
bool_t linphone_core_sound_resources_need_locking(LinphoneCore *lc, const LinphoneCallParams *params){
	return ((strcmp(linphone_core_get_media_resource_mode(lc), "unique") == 0) &&
...
int linphone_core_preempt_sound_resources(LinphoneCore *lc){
	...
	if((current_call != NULL) && (!linphone_core_is_incoming_invite_pending(lc))){
		...
		if (sal_media_description_has_dir(L_GET_PRIVATE(ms)->getResultDesc(), SalStreamSendOnly)) {
			ms_error("Trying to empty resources of a call whose SAL media direction is SendOnly - If you wish to do so, please set configuration parameter media_resources_mode to shared: linphone_core_set_media_resource_mode (lc, \"shared\")");
		} else {

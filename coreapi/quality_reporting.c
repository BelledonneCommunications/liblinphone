...
void linphone_reporting_on_rtcp_update(LinphoneCall *call, SalStreamType stats_type) {
	...
	if (stats_type == 0) {
		stats = Call::toCpp(call)->getStats(LinphoneStreamTypeAudio);
	} else if (stats_type == 1) {
		stats = Call::toCpp(call)->getStats(LinphoneStreamTypeVideo);
	} else {
		stats = Call::toCpp(call)->getStats(LinphoneStreamTypeText);
	}

...
static void append_to_buffer_valist(char **buff, size_t *buff_size, size_t *offset, const char *fmt, va_list args) {
	...
	if (ret == BELLE_SIP_BUFFER_OVERFLOW) {
		...
		ms_debug(
		    "QualityReporting: Buffer was too small to contain the whole report - increasing its size from %lu to %lu",
...
#define APPEND_IF_NOT_NULL_STR(buffer, size, offset, fmt, arg)                                                         \
	if (arg != NULL) append_to_buffer(buffer, size, offset, fmt, arg)
#define APPEND_IF_NUM_IN_RANGE(buffer, size, offset, fmt, arg, inf, sup)                                               \
	if (inf <= arg && arg <= sup) append_to_buffer(buffer, size, offset, fmt, arg)
#define APPEND_IF(buffer, size, offset, fmt, arg, cond)                                                                \
	if (cond) append_to_buffer(buffer, size, offset, fmt, arg)
#define IF_NUM_IN_RANGE(num, inf, sup, statement)                                                                      \
	if (inf <= num && num <= sup) statement
...
static uint8_t are_metrics_filled(const reporting_content_metrics_t *rm) {
	...
	if (rm->rtcp_sr_count + rm->rtcp_xr_count > 0) {
		IF_NUM_IN_RANGE(rm->delay.round_trip_delay / (rm->rtcp_sr_count + rm->rtcp_xr_count), 0, 65535,
		                ret |= METRICS_DELAY);
	}
...
static bool_t media_report_enabled(LinphoneCall *call, int stats_type) {
	if (!quality_reporting_enabled(call)) return FALSE;

	if (stats_type == LINPHONE_CALL_STATS_VIDEO) {
		if (!(Call::toCpp(call)->getLog()->getQualityReporting()->was_video_running ||
		      linphone_call_params_video_enabled(linphone_call_get_current_params(call))))
			return FALSE;
	}

	if (stats_type == LINPHONE_CALL_STATS_TEXT &&
	    !linphone_call_params_realtime_text_enabled(linphone_call_get_current_params(call)))
		return FALSE;

	return (Call::toCpp(call)->getLog()->getQualityReporting()->reports[stats_type] != NULL);
}

static void
append_metrics_to_buffer(char **buffer, size_t *size, size_t *offset, const reporting_content_metrics_t *rm) {
	...
	if (rm->timestamps.start > 0) timestamps_start_str = linphone_timestamp_to_rfc3339_string(rm->timestamps.start);
	if (rm->timestamps.stop > 0) timestamps_stop_str = linphone_timestamp_to_rfc3339_string(rm->timestamps.stop);
	...
	if ((available_metrics & METRICS_SESSION_DESCRIPTION) != 0) {
		...
		APPEND_IF(buffer, size, offset, " PT=%d", rm->session_description.payload_type,
		          rm->session_description.payload_type != -1);
		APPEND_IF_NOT_NULL_STR(buffer, size, offset, " PD=%s", rm->session_description.payload_desc);
		APPEND_IF(buffer, size, offset, " SR=%d", rm->session_description.sample_rate,
		          rm->session_description.sample_rate != -1);
		APPEND_IF(buffer, size, offset, " FD=%d", rm->session_description.frame_duration,
		          rm->session_description.frame_duration != -1);
		APPEND_IF_NOT_NULL_STR(buffer, size, offset, " FMTP=\"%s\"", rm->session_description.fmtp);
		APPEND_IF(buffer, size, offset, " PLC=%d", rm->session_description.packet_loss_concealment,
		          rm->session_description.packet_loss_concealment != -1);
	}
	...
	if ((available_metrics & METRICS_JITTER_BUFFER) != 0) {
		...
		if (rm->rtcp_xr_count) {
			APPEND_IF_NUM_IN_RANGE(buffer, size, offset, " JBN=%d", rm->jitter_buffer.nominal / rm->rtcp_xr_count, 0,
			                       65535);
			APPEND_IF_NUM_IN_RANGE(buffer, size, offset, " JBM=%d", rm->jitter_buffer.max / rm->rtcp_xr_count, 0,
			                       65535);
		}
		APPEND_IF_NUM_IN_RANGE(buffer, size, offset, " JBX=%d", rm->jitter_buffer.abs_max, 0, 65535);

		append_to_buffer(buffer, size, offset, "\r\nPacketLoss:");
		IF_NUM_IN_RANGE(rm->packet_loss.network_packet_loss_rate, 0, 255,
		                network_packet_loss_rate_str =
		                    float_to_one_decimal_string(rm->packet_loss.network_packet_loss_rate / 256));
		IF_NUM_IN_RANGE(rm->packet_loss.jitter_buffer_discard_rate, 0, 255,
		                jitter_buffer_discard_rate_str =
		                    float_to_one_decimal_string(rm->packet_loss.jitter_buffer_discard_rate / 256));
	...
	/*append_to_buffer(buffer, size, offset, "\r\nBurstGapLoss:");*/
	/*IF_NUM_IN_RANGE(rm.burst_gap_loss.gap_loss_density, 0, 10, gap_loss_density_str =
	 * float_to_one_decimal_string(rm.burst_gap_loss.gap_loss_density));*/
	...
	if ((available_metrics & METRICS_DELAY) != 0) {
		...
		if (rm->rtcp_xr_count + rm->rtcp_sr_count) {
			APPEND_IF_NUM_IN_RANGE(buffer, size, offset, " RTD=%d",
			                       rm->delay.round_trip_delay / (rm->rtcp_xr_count + rm->rtcp_sr_count), 0, 65535);
		}
	...
	if ((available_metrics & METRICS_QUALITY_ESTIMATES) != 0) {
		IF_NUM_IN_RANGE(rm->quality_estimates.moslq, 1, 5,
		                moslq_str = float_to_one_decimal_string(rm->quality_estimates.moslq));
		IF_NUM_IN_RANGE(rm->quality_estimates.moscq, 1, 5,
		                moscq_str = float_to_one_decimal_string(rm->quality_estimates.moscq));
...
static int send_report(LinphoneCall *call, reporting_session_report_t *report, const char *report_event) {
	...
	if (report->info.local_addr.ip == NULL || strlen(report->info.local_addr.ip) == 0 ||
		...
		ms_message("QualityReporting[%p]: Trying to submit a %s too early (call duration: %d sec) but %s IP could "
		           ...
		           call, report_event, linphone_call_get_duration(call),
		           (report->info.local_addr.ip == NULL || strlen(report->info.local_addr.ip) == 0) ? "local"
		                                                                                           : "remote");
		ret = 2;
		goto end;
	}
	...
	if (Call::toCpp(call)->getLog()->getQualityReporting()->on_report_sent != NULL) {
		SalStreamType type = report == Call::toCpp(call)->getLog()->getQualityReporting()->reports[0]   ? SalAudio
		                     : report == Call::toCpp(call)->getLog()->getQualityReporting()->reports[1] ? SalVideo
		                                                                                                : SalText;
	...
	ms_message("QualityReporting[%p]: Send '%s' with status %d", call, report_event, ret);

	return ret;
}

static const LinphonePrivate::SalStreamDescription *
get_media_stream_for_desc(const std::shared_ptr<LinphonePrivate::SalMediaDescription> &smd,
                          SalStreamType sal_stream_type) {
	if (smd != NULL) {
...
static void update_ip(LinphoneCall *call, int stats_type) {
	SalStreamType sal_stream_type = stats_type == LINPHONE_CALL_STATS_AUDIO   ? SalAudio
	                                : stats_type == LINPHONE_CALL_STATS_VIDEO ? SalVideo
	                                                                          : SalText;
	const LinphonePrivate::SalStreamDescription *local_desc =
	    get_media_stream_for_desc(Call::toCpp(call)->getOp()->getLocalMediaDescription(), sal_stream_type);
	const LinphonePrivate::SalStreamDescription *remote_desc =
	    get_media_stream_for_desc(Call::toCpp(call)->getOp()->getRemoteMediaDescription(), sal_stream_type);
	std::shared_ptr<CallLog> log = Call::toCpp(call)->getLog();
	LinphoneQualityReporting *log_reporting = log->getQualityReporting();
	...
	if (local_desc != NULL) {
		...
		if (local_desc->rtp_addr.empty() == false) {
			log_reporting->reports[stats_type]->info.local_addr.port = local_desc->rtp_port;
			STR_REASSIGN(log_reporting->reports[stats_type]->info.local_addr.ip,
			             ms_strdup(L_STRING_TO_C(local_desc->rtp_addr)));
		}
	}

	if (remote_desc != NULL) {
		/*port is always stored in stream description struct*/
		log_reporting->reports[stats_type]->info.remote_addr.port = remote_desc->rtp_port;

		/*for IP it can be not set if we are using a direct route*/
		if (remote_desc->rtp_addr.empty() == false) {
			STR_REASSIGN(log_reporting->reports[stats_type]->info.remote_addr.ip,
			             ms_strdup(L_STRING_TO_C(remote_desc->rtp_addr)));
		} else {
			STR_REASSIGN(log_reporting->reports[stats_type]->info.remote_addr.ip,
			             ms_strdup(L_STRING_TO_C(Call::toCpp(call)->getOp()->getRemoteMediaDescription()->addr)));
		}
	}
}
...
static void qos_analyzer_on_action_suggested(void *user_data, BCTBX_UNUSED(int datac), const char **datav) {
	...
	int up_bw[3] = {-1, -1, -1};
	int down_bw[3] = {-1, -1, -1};
	MediaStream *streams[3] = {linphone_call_get_stream(call, LinphoneStreamTypeAudio),
	                           linphone_call_get_stream(call, LinphoneStreamTypeVideo),
	                           linphone_call_get_stream(call, LinphoneStreamTypeText)};
	for (i = 0; i < 3; i++) {
		if (streams[i] != NULL) {
	...
	appendbuf = ms_strdup_printf("%s%llu;", report->qos_analyzer.timestamp ? report->qos_analyzer.timestamp : "",
	                             (unsigned long long)ms_time(0));
	STR_REASSIGN(report->qos_analyzer.timestamp, appendbuf);

	STR_REASSIGN(
	    report->qos_analyzer.input_leg,
	    ms_strdup_printf("%s aenc_ptime aenc_br a_dbw a_ubw venc_br v_dbw v_ubw tenc_br t_dbw t_ubw", datav[0]));
	appendbuf = ms_strdup_printf(
	    "%s%s %d %d %d %d %d %d %d %d %d %d;", report->qos_analyzer.input ? report->qos_analyzer.input : "", datav[1],
...
void linphone_reporting_update_media_info(LinphoneCall *call, int stats_type) {
	...
	if (!media_report_enabled(call, stats_type) || !Call::toCpp(call)->getOp()) return;

	std::string dialogId = Call::toCpp(call)->getOp()->getDialogId();

	STR_REASSIGN(report->info.call_id, ms_strdup(log->getCallId().c_str()));

	STR_REASSIGN(report->local_metrics.user_agent,
	             ms_strdup(linphone_core_get_user_agent(linphone_call_get_core(call))));
	...
	STR_REASSIGN(report->info.local_addr.group,
	             ms_strdup_printf("%s-%s-%s", dialogId.c_str(), "local",
	                              report->local_metrics.user_agent ? report->local_metrics.user_agent : ""));
	STR_REASSIGN(report->info.remote_addr.group,
	...
	report->local_metrics.timestamps.start = log->getStartTime();
	report->local_metrics.timestamps.stop = log->getStartTime() + linphone_call_get_duration(call);

	/*we use same timestamps for remote too*/
	report->remote_metrics.timestamps.start = log->getStartTime();
	report->remote_metrics.timestamps.stop = log->getStartTime() + linphone_call_get_duration(call);
	...
	if (stream != NULL) {
		...
		if (stream->qi != NULL) {
			report->local_metrics.quality_estimates.moslq =
			    ms_quality_indicator_get_average_lq_rating(stream->qi) >= 0
			        ? MAX(1, ms_quality_indicator_get_average_lq_rating(stream->qi))
			        : -1;
			report->local_metrics.quality_estimates.moscq =
			    ms_quality_indicator_get_average_rating(stream->qi) >= 0
			        ? MAX(1, ms_quality_indicator_get_average_rating(stream->qi))
			        : -1;
		}
	}
...
void linphone_reporting_on_rtcp_update(LinphoneCall *call, SalStreamType stats_type) {
	reporting_session_report_t *report = Call::toCpp(call)->getLog()->getQualityReporting()->reports[stats_type];
...
static int publish_report(LinphoneCall *call, const char *event_type) {
	...
	for (const auto &idx :
		...
		if (media_report_enabled(call, stream_index)) {
			...
			sndret = send_report(call, Call::toCpp(call)->getLog()->getQualityReporting()->reports[stream_index],
			                     event_type);
			if (sndret > 0) {
				ret += 10 + (idx + 1) * sndret;
			}
...
static bool_t
set_on_action_suggested_cb(MediaStream *stream, void (*on_action_suggested)(void *, int, const char **), void *u) {
	if (stream && stream->rc) {
		MSQosAnalyzer *analyzer = ms_bitrate_controller_get_qos_analyzer(stream->rc);
		if (analyzer) {
			ms_qos_analyzer_set_on_action_suggested(analyzer, on_action_suggested, u);
			return TRUE;
		}
	}
	return FALSE;
...
void linphone_reporting_call_state_updated(LinphoneCall *call) {
	...
	switch (state) {
		case LinphoneCallStreamsRunning: {
			MediaStream *streams[3] = {Call::toCpp(call)->getMediaStream(LinphoneStreamTypeAudio),
			                           Call::toCpp(call)->getMediaStream(LinphoneStreamTypeVideo),
			                           Call::toCpp(call)->getMediaStream(LinphoneStreamTypeText)};
			for (const auto &stream_index :
			     {LINPHONE_CALL_STATS_AUDIO, LINPHONE_CALL_STATS_VIDEO, LINPHONE_CALL_STATS_TEXT}) {
				bool_t enabled = media_report_enabled(call, stream_index);
				if (enabled && set_on_action_suggested_cb(streams[stream_index], qos_analyzer_on_action_suggested,
				                                          log->getQualityReporting()->reports[stream_index])) {
					log->getQualityReporting()->reports[stream_index]->call = call;
					STR_REASSIGN(log->getQualityReporting()->reports[stream_index]->qos_analyzer.name,
					             ms_strdup(ms_qos_analyzer_get_name(
					                 ms_bitrate_controller_get_qos_analyzer(streams[stream_index]->rc))));
				}
			}
		...
		case LinphoneCallEnd: {
			...
			if (log->getStatus() == LinphoneCallSuccess || log->getStatus() == LinphoneCallAborted) {
...
void linphone_reporting_set_on_report_send(LinphoneCall *call, LinphoneQualityReportingReportSendCb cb) {
	Call::toCpp(call)->getLog()->getQualityReporting()->on_report_sent = cb;
}

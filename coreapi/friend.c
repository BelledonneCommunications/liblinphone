...
BELLE_SIP_INSTANCIATE_VPTR(LinphoneFriendCbs, belle_sip_object_t,
						   NULL, // destroy
						   NULL, // clone
						   NULL, // Marshall
						   FALSE);
...
#define NOTIFY_IF_EXIST(cbName, functionName, ...)                                                                     \
	...
	for (bctbx_list_t *it = callbacksCopy; it; it = bctbx_list_next(it)) {                                             \
		linphone_friend_set_current_callbacks(linphone_friend, reinterpret_cast<LinphoneFriendCbs *>(bctbx_list_get_data(it)));   \

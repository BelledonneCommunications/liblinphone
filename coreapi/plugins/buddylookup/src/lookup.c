...
static SoupMessage *build_xmlrpc_request(
    const char *identity, const char *password, const char *key, const char *domain, const char *url, int max_results) {
	SoupMessage *msg;

	msg = soup_xmlrpc_request_new(url, "fp.searchUsers", G_TYPE_STRING, identity, G_TYPE_STRING,
	                              password ? password : "", G_TYPE_STRING, key, G_TYPE_INT, max_results, G_TYPE_INT, 0,
	                              G_TYPE_STRING, domain, G_TYPE_INVALID);
	if (!msg) {
...
static int lookup_buddy(SipSetupContext *ctx, BLReq *req) {
	...
	const char *url = linphone_config_get_string(config, "BuddyLookup", "url", NULL);
	...
	auth_info =
	    linphone_core_find_auth_info(lc, linphone_address_get_domain(from), linphone_address_get_username(from));
	if (auth_info) {
	...
	sm = build_xmlrpc_request(identity, auth_info ? auth_info->passwd : NULL, req->base.key,
	                          linphone_address_get_domain(from), url, req->base.max_results);
	ms_free(identity);
	req->msg = sm;
...
static int free_request(SipSetupContext *ctx, BuddyLookupRequest *req) {
	...
	if (blreq->session) g_object_unref(G_OBJECT(blreq->session));
...
static BuddyLookupFuncs bl_funcs = {
    .request_create = create_request, .request_submit = submit_request, .request_free = free_request};

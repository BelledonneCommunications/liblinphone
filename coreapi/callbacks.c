...
static bool_t fill_auth_info(LinphoneCore *lc, SalAuthInfo* sai) {
	...
	if (ai) {
		if (sai->mode == SalAuthModeHttpDigest) {
			 ...
			 */
			if (sai->algorithm && strlen(linphone_auth_info_get_algorithm(ai)) != 0) {
				if (strcasecmp(linphone_auth_info_get_algorithm(ai), sai->algorithm))
					return TRUE;
			} else if (
				(strcasecmp(linphone_auth_info_get_algorithm(ai), "MD5")) ||
				(sai->algorithm && strcasecmp(sai->algorithm, "MD5"))
			)
				return TRUE;

			sai->userid = ms_strdup(strlen(linphone_auth_info_get_userid(ai)) != 0 ? linphone_auth_info_get_userid(ai) : linphone_auth_info_get_username(ai));
			sai->password = strlen(linphone_auth_info_get_passwd(ai)) != 0 ? ms_strdup(linphone_auth_info_get_passwd(ai)) : NULL;
			sai->ha1 = strlen(linphone_auth_info_get_ha1(ai)) != 0 ? ms_strdup(linphone_auth_info_get_ha1(ai)) : NULL;
		...
		if (sai->realm && strlen(linphone_auth_info_get_realm(ai)) == 0){

...
BELLE_SIP_INSTANCIATE_VPTR(LinphoneEventCbs,
                           belle_sip_object_t,
                           NULL, // destroy
                           NULL, // clone
                           NULL, // marshal
...
static LinphoneEvent *linphone_event_new_base(LinphoneCore *lc,
	...
	if (strcmp(lev->name, "conference") == 0) lev->internal = TRUE;
...
static LinphoneEvent *linphone_event_new_with_op_base(
    LinphoneCore *lc, SalEventOp *op, LinphoneSubscriptionDir dir, const char *name, bool_t is_out_of_dialog) {
...
void linphone_event_set_state(LinphoneEvent *lev, LinphoneSubscriptionState state) {
	if (lev && lev->subscription_state != state) {
		ms_message("LinphoneEvent [%p] moving to subscription state %s", lev,
...
void linphone_event_set_publish_state(LinphoneEvent *lev, LinphonePublishState state) {
	if (lev->publish_state != state) {
		ms_message("LinphoneEvent [%p] moving from [%s] to publish state %s", lev,
		           linphone_publish_state_to_string(lev->publish_state), linphone_publish_state_to_string(state));
...
LinphoneStatus linphone_event_send_subscribe(LinphoneEvent *lev, const LinphoneContent *body) {
	...
	switch (lev->subscription_state) {
		...
		case LinphoneSubscriptionTerminated:
		case LinphoneSubscriptionOutgoingProgress:
			ms_error("linphone_event_send_subscribe(): cannot update subscription while in state [%s]",
			         linphone_subscription_state_to_string(lev->subscription_state));
			return -1;
			break;
		case LinphoneSubscriptionNone:
		case LinphoneSubscriptionActive:
...
LinphoneEvent *_linphone_core_create_publish(
	...
	linphone_configure_op_with_account(lc, lev->op, resource, NULL,
	                                   !!linphone_config_get_int(lc->config, "sip", "publish_msg_with_contact", 0),
	                                   account);
...
void linphone_event_terminate(LinphoneEvent *lev) {
	...
	if (lev->subscription_state == LinphoneSubscriptionError ||
	    lev->subscription_state == LinphoneSubscriptionTerminated) {
		return;
	}
...
const LinphoneAddress *linphone_event_get_from(const LinphoneEvent *lev) {
	if (lev->is_out_of_dialog_op && lev->dir == LinphoneSubscriptionOutgoing) return _linphone_event_cache_to(lev);
	return _linphone_event_cache_from(lev);
}

const LinphoneAddress *linphone_event_get_resource(const LinphoneEvent *lev) {
	if (lev->is_out_of_dialog_op && lev->dir == LinphoneSubscriptionOutgoing) return _linphone_event_cache_from(lev);
	return _linphone_event_cache_to(lev);
}
...
_linphone_event_marshall(belle_sip_object_t *obj, char *buff, size_t buff_size, size_t *offset) {
	LinphoneEvent *ev = (LinphoneEvent *)obj;
	belle_sip_error_code err = BELLE_SIP_OK;

	err = belle_sip_snprintf(buff, buff_size, offset, "%s of %s",
	                         ev->dir == LinphoneSubscriptionIncoming
	                             ? "Incoming Subscribe"
	                             : (ev->dir == LinphoneSubscriptionOutgoing ? "Outgoing subscribe" : "Publish"),
	                         ev->name);
...
void _linphone_event_notify_notify_response(LinphoneEvent *lev) {
	...
	for (bctbx_list_t *it = callbacksCopy; it; it = bctbx_list_next(it)) {
		...
		LinphoneEventCbsNotifyResponseCb callback =
		    linphone_event_cbs_get_notify_response(linphone_event_get_current_callbacks(lev));
		if (callback) {
			callback(lev);
		}
	}
...
BELLE_SIP_INSTANCIATE_VPTR(LinphoneEvent,
                           belle_sip_object_t,

...
void IosSharedCoreHelpers::putMsgInUserDefaults(LinphoneChatMessage *msg) {
	...
	if (mAppGroupId != TEST_GROUP_ID) { // For testing purpose
		...
		const LinphoneAddress *localAddr = linphone_chat_message_get_local_address(msg);
		const LinphoneAddress *peerAddr = linphone_chat_message_get_peer_address(msg); //todo paul : conf addr??
		NSString *fromUsername = [NSString stringWithUTF8String:linphone_address_as_string(fromAddr)];
		NSString *localUri = [NSString stringWithUTF8String:linphone_address_as_string(localAddr)];
		NSString *peerUri = [NSString stringWithUTF8String:linphone_address_as_string(peerAddr)];


		NSDictionary *newMsg = [NSDictionary dictionaryWithObjects:[NSArray arrayWithObjects:isText, textContent, subject, fromUsername, localUri, peerUri, nil]

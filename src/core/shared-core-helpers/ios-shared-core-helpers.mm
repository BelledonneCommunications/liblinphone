...
void on_msg_written_in_user_defaults(UNUSED(CFNotificationCenterRef center), void *observer, UNUSED(CFStringRef name), UNUSED(const void *object), UNUSED(CFDictionaryRef userInfo)) {
	lInfo() << "[push] " << __FUNCTION__;
	if (observer) {
...
static void on_push_notification_chat_room_joined(LinphoneChatRoom *cr, UNUSED(const LinphoneEventLog *evlog)){
...
void unlock_shared_core_if_needed(UNUSED(CFRunLoopTimerRef timer), void *info) {
...
void on_core_must_stop(UNUSED(CFNotificationCenterRef center), void *observer, UNUSED(CFStringRef name), UNUSED(const void *object), UNUSED(CFDictionaryRef userInfo)) {
   	lInfo() << "[SHARED] " << __FUNCTION__;
	if (observer) {
		IosSharedCoreHelpers *myself = (IosSharedCoreHelpers *) observer;

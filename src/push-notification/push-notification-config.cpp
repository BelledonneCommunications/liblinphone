...
PushNotificationConfig::PushNotificationConfig(string const &serializedConfig) : PushNotificationConfig() {
	...
	for (auto param : mPushParams) {
		param.second = pushParamsWrapper.getUriParamValue(param.first);
	}
}
...
string PushNotificationConfig::asString(bool withRemoteSpecificParams) const {
	...
	serializedConfig += mPushParams.at(PushConfigPridKey);
	serializedConfig += mPushParams.at(PushConfigProviderKey);
	serializedConfig += mPushParams.at(PushConfigParamKey);
	serializedConfig += mPushParams.at(PushConfigSilentKey);
	serializedConfig += mPushParams.at(PushConfigTimeoutKey);
	
	if (withRemoteSpecificParams) {
		serializedConfig += mPushParams.at(PushConfigMsgStrKey);
		serializedConfig += mPushParams.at(PushConfigCallStrKey);
		serializedConfig += mPushParams.at(PushConfigGroupChatStrKey);
		serializedConfig += mPushParams.at(PushConfigCallSoundKey);
		serializedConfig += mPushParams.at(PushConfigMsgSoundKey);
	}

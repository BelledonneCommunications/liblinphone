...
MainDbPrivate::findAudioVideoConference(const ConferenceId &conferenceId) const {
	L_Q();
...
long long MainDbPrivate::insertOrUpdateImportedBasicChatRoom(long long peerSipAddressId,
	...
	const int capabilities =
	    ChatRoom::CapabilitiesMask({ChatRoom::Capabilities::Basic, ChatRoom::Capabilities::Migratable});
...
long long MainDbPrivate::insertConferenceInfo(const std::shared_ptr<ConferenceInfo> &conferenceInfo,
	...
	if (conferenceInfoId >= 0) {
		...
		*dbSession.getBackendSession() << "UPDATE conference_info SET"
		    ...
		    soci::use(organizerSipAddressId), soci::use(startTime), soci::use(duration), soci::use(subject),
		    soci::use(description), soci::use(state), soci::use(sequence), soci::use(uid), soci::use(conferenceInfoId);
	} else {
		...
		*dbSession.getBackendSession() << "INSERT INTO conference_info ("
		                                  ...
		                                  ")",
		    soci::use(organizerSipAddressId), soci::use(uriSipAddressid), soci::use(startTime), soci::use(duration),
...
long long MainDbPrivate::insertOrUpdateConferenceCall(const std::shared_ptr<CallLog> &callLog,
	...
	if (conferenceCallId < 0) {
		...
		*dbSession.getBackendSession() << "INSERT INTO conference_call ("
		    ...
		    soci::use(startTime), soci::use(connectedTime), soci::use(status), soci::use(videoEnabled),
		    soci::use(quality), soci::use(callId), soci::use(refKey), soci::use(conferenceInfoId, confInfoInd);
		...
		*dbSession.getBackendSession()
		    ...
		    soci::use(duration), soci::use(connectedTime), soci::use(status), soci::use(videoEnabled),
		    soci::use(quality), soci::use(callId), soci::use(refKey), soci::use(conferenceInfoId, confInfoInd),
...
long long MainDbPrivate::selectSipAddressId(const std::shared_ptr<Address> &address) const {
	...
	// This is a hack, because all addresses don't print their parameters in the same order.
	const string sipAddress = address->toStringUriOnlyOrdered();
	return selectSipAddressId(sipAddress);
#else
	return -1;
#endif
}
...
long long MainDbPrivate::selectSipAddressId(const string &sipAddress) const {
	...
	soci::session *session = dbSession.getBackendSession();
	*session << Statements::get(Statements::SelectSipAddressId), soci::use(sipAddress), soci::into(sipAddressId);
...
#ifdef HAVE_DB_STORAGE
	long long peerSipAddressId = conferenceId.getPeerAddress() ? selectSipAddressId(conferenceId.getPeerAddress()) : -1;
	if (peerSipAddressId < 0) return -1;

	long long localSipAddressId =
	    conferenceId.getLocalAddress() ? selectSipAddressId(conferenceId.getLocalAddress()) : -1;
...
long long MainDbPrivate::insertConferenceChatMessageEvent(const shared_ptr<EventLog> &eventLog) {
	...
	*dbSession.getBackendSession()
	    << "INSERT INTO conference_chat_message_event ("
	       ...
	       ")",
	    soci::use(eventId), soci::use(fromSipAddressId), soci::use(toSipAddressId), soci::use(messageTime),
	    soci::use(state), soci::use(direction), soci::use(imdnMessageId), soci::use(isSecured),
	    soci::use(deliveryNotificationRequired), soci::use(displayNotificationRequired), soci::use(markedAsRead),
	    soci::use(forwardInfo), soci::use(callId), soci::use(replyMessageId), soci::use(replyToSipAddressId);
...
void MainDbPrivate::setChatMessageParticipantState(const shared_ptr<EventLog> &eventLog,
	...
	MainDbKeyPrivate *dEventKey = static_cast<MainDbKey &>(dEventLog->dbKey).getPrivate();
	const long long &eventId = dEventKey->storageId;
	const long long &participantSipAddressId = selectSipAddressId(participantAddress);
...
list<shared_ptr<ChatMessage>> MainDb::getEphemeralMessages() const {
	...
	string query =
	    ...
	    "security_alert, faulty_device, marked_as_read, forward_info, ephemeral_lifetime, expired_time, lifetime, "
	    "reply_message_id, reply_sender_address.value, chat_room_id"
	...
	return L_DB_TRANSACTION {
		...
		soci::rowset<soci::row> rows =
		    getBackend() == MainDb::Backend::Sqlite3
		        ? (d->dbSession.getBackendSession()->prepare << query, soci::use(Utils::getTimeTAsTm(0)),
...
list<ChatMessage::State> MainDb::getChatMessageParticipantStates(const shared_ptr<EventLog> &eventLog) const {
	...
	return L_DB_TRANSACTION {
		...
		unsigned int state;
		soci::statement statement = (d->dbSession.getBackendSession()->prepare
...
list<shared_ptr<ChatMessage>> MainDb::findChatMessagesFromCallId(const std::string &callId) const {
	...
	static const string query =
	    ...
	    "display_notification_required, security_alert, faulty_device, marked_as_read, forward_info, "
	    "ephemeral_lifetime, expired_time, lifetime, reply_message_id, reply_sender_address.value, chat_room_id"
	    " FROM conference_event_view"
	    " LEFT JOIN sip_address AS from_sip_address ON from_sip_address.id = from_sip_address_id"
	    " LEFT JOIN sip_address AS to_sip_address ON to_sip_address.id = to_sip_address_id"
	    " LEFT JOIN sip_address AS device_sip_address ON device_sip_address.id = device_sip_address_id"
		...
		for (const auto &row : rows) {
			...
			if (capabilities & ChatRoom::CapabilitiesMask(ChatRoom::Capabilities::Basic)) {
				...
				if (!linphone_core_conference_server_enabled(core->getCCore())) {
					...
					chatRoom->setState(hasBeenLeft ? ConferenceInterface::State::Terminated
...
#ifdef HAVE_DB_STORAGE
	const long long &previousConferenceSipAddressId = selectSipAddressId(previousConfId.getPeerAddress());
...
void MainDb::updateChatRoomLastUpdatedTime(const ConferenceId &conferenceId, time_t lastUpdatedTime) {
	...
	L_DB_TRANSACTION {
		...
		*d->dbSession.getBackendSession() << "UPDATE chat_room SET last_update_time = :lastUpdateTime"
		                                     " WHERE id = :chatRoomId",
		    soci::use(lastUpdateTimeTm), soci::use(dbChatRoomId);
...
void MainDb::migrateBasicToClientGroupChatRoom(const shared_ptr<AbstractChatRoom> &basicChatRoom,
	...
	L_DB_TRANSACTION {
		...
		*d->dbSession.getBackendSession() << "UPDATE chat_room"
		                                     ...
		                                     " WHERE id = :chatRoomId",
		    soci::use(capabilities), soci::use(peerSipAddressId), soci::use(localSipAddressId), soci::use(dbChatRoomId);
...
std::shared_ptr<Address> MainDb::findOneToOneConferenceChatRoomAddress(const std::shared_ptr<Address> &participantA,
	...
	return L_DB_TRANSACTION {
		...
		const long long &participantASipAddressId = d->selectSipAddressId(participantA);
		const long long &participantBSipAddressId = d->selectSipAddressId(participantB);
...
void MainDb::insertOneToOneConferenceChatRoom(const shared_ptr<AbstractChatRoom> &chatRoom, bool encrypted) {
	...
	L_DB_TRANSACTION {
		...
		const long long &participantASipAddressId = d->selectSipAddressId(participants.front()->getAddress());
		const long long &participantBSipAddressId = d->selectSipAddressId(participants.back()->getAddress());
...
void MainDb::enableChatRoomMigration(const ConferenceId &conferenceId, bool enable) {
	...
	L_DB_TRANSACTION {
		...
		int capabilities = 0;
		*session << "SELECT capabilities FROM chat_room WHERE id = :chatRoomId", soci::use(dbChatRoomId),
		    soci::into(capabilities);
		if (enable) capabilities |= int(ChatRoom::Capabilities::Migratable);
		else capabilities &= ~int(ChatRoom::Capabilities::Migratable);
		*session << "UPDATE chat_room SET capabilities = :capabilities WHERE id = :chatRoomId", soci::use(capabilities),
		    soci::use(dbChatRoomId);
...
void MainDb::deleteChatRoomParticipant(const std::shared_ptr<AbstractChatRoom> &chatRoom,
	...
	if (isInitialized()) {
		const long long &dbChatRoomId = d->selectChatRoomId(chatRoom->getConferenceId());
		const long long &participantSipAddressId = d->selectSipAddressId(participant);

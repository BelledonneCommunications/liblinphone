...
void MainDbPrivate::insertChatRoomParticipantDevice (long long participantId, long long participantDeviceSipAddressId, const std::string &deviceName) {
#ifdef HAVE_DB_STORAGE
	L_Q();
	if (q->isInitialized()) {
		soci::session *session = dbSession.getBackendSession();
...
void MainDbPrivate::insertChatRoomParticipantDevice (
	...
	if (q->isInitialized()) {
		soci::session *session = dbSession.getBackendSession();
		const long long &participantDeviceSipAddressId = insertSipAddress(device->getAddress().asString());

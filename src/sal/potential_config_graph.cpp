...
const belle_sip_list_t * PotentialCfgGraph::getSessionCapabilityAttributes(const belle_sdp_session_description_t* session_desc, const capability_type_t cap) {
 ...
 * Media
 */
void PotentialCfgGraph::processMediaDescription(const unsigned int & idx, const belle_sdp_media_description_t* media_desc) {
	// ACAP
	...
	if (acfgFound && pcfgFound) {
		lInfo() << "The provided sdp is not valid because it defines both attribute configurations and potential configurations - pcfg attrbutes are allowed in offers and acfg attributes in answers ";
	}
}

const belle_sip_list_t * PotentialCfgGraph::getMediaCapabilityAttributes(const belle_sdp_media_description_t* media_desc, const capability_type_t cap) {
	const std::string cap_name(capabilityToAttributeName(cap));
 ...
 */
PotentialCfgGraph::media_description_acap PotentialCfgGraph::getSessionDescriptionACapabilities (const belle_sdp_session_description_t* session_desc) {
...
PotentialCfgGraph::media_description_acap PotentialCfgGraph::createACapabilitiesList (const belle_sip_list_t * caps_attr, const capability_type_t cap) {
...
PotentialCfgGraph::media_description_acap PotentialCfgGraph::getMediaDescriptionACapabilities (const belle_sdp_media_description_t* media_desc) {
	const capability_type_t cap = capability_type_t::ATTRIBUTE;
 ...
 */
PotentialCfgGraph::media_description_base_cap PotentialCfgGraph::getSessionDescriptionTCapabilities (const belle_sdp_session_description_t* session_desc) {
...
PotentialCfgGraph::media_description_base_cap PotentialCfgGraph::createTCapabilitiesList (const belle_sip_list_t * caps_attr, const capability_type_t cap) {
...
PotentialCfgGraph::media_description_base_cap PotentialCfgGraph::getMediaDescriptionTCapabilities (const belle_sdp_media_description_t* media_desc) {
	const capability_type_t cap = capability_type_t::TRANSPORT_PROTOCOL;
	const belle_sip_list_t * caps_attr = getMediaCapabilityAttributes(media_desc, cap);
	auto capList = createTCapabilitiesList(caps_attr, cap);
	belle_sip_list_free_with_data(const_cast<belle_sip_list_t *>(caps_attr), (void (*)(void*))belle_sip_object_unref);
...
bool PotentialCfgGraph::processMediaCfg(const unsigned int & idx, const belle_sdp_media_description_t* media_desc, const config_type_t cfgType) {
	bool found = false;
	switch (cfgType) {
...
bool PotentialCfgGraph::processMediaAcfg(const unsigned int & idx, const belle_sdp_media_description_t* media_desc) {
	...
	for(belle_sip_list_t * attr = attrs;attr!=NULL;attr=attr->next){
		...
		if (attr_configs.acap.empty() && attr_configs.tcap.empty()) {
			lInfo() << "Unable to build a potential config for id " << id << " because lists of attribute and transport capabilities are empty";
...
PotentialCfgGraph::media_description_config::mapped_type PotentialCfgGraph::createPConfigFromAttribute(belle_sdp_pcfg_attribute_t* attribute, const PotentialCfgGraph::media_description_acap & mediaAcap, const PotentialCfgGraph::media_description_base_cap & mediaTcap) {
...
PotentialCfgGraph::media_description_config::mapped_type PotentialCfgGraph::createAConfigFromAttribute(belle_sdp_acfg_attribute_t* attribute, const PotentialCfgGraph::media_description_acap & mediaAcap, const PotentialCfgGraph::media_description_base_cap & mediaTcap) {
	const belle_sip_list_t* configList = belle_sdp_acfg_attribute_get_configs(attribute);
	return processConfig(configList, mediaAcap, mediaTcap);
}

PotentialCfgGraph::media_description_config::mapped_type PotentialCfgGraph::processConfig(const belle_sip_list_t* configList, const PotentialCfgGraph::media_description_acap & mediaAcap, const PotentialCfgGraph::media_description_base_cap & mediaTcap) const {
	...
	for(;list!=NULL;list=list->next){
		...
		if (cap == capability_type_t::ATTRIBUTE) {
			if (acapCfgList.empty()) {
				...
				lError() << "Ignoring second attribute configuration list " << cfg << " has been found in the configuration because already " << acapCfgList.size() << " elements have been collected";
			}
			...
			if (tcapCfgList.empty()) {
				...
				if (!parsedList.empty()) {
					...
					for (const auto & tcapCfg : parsedList) {
						...
						if (tcapCfgSize > 1) {
							lError() << "Found " << tcapCfgSize << " transport protocols for a configuration. Ignoring it.";
						} else {
				...
				lError() << "Ignoring second transport configuration list " << cfg << " has been found in the configuration because already " << tcapCfgList.size() << " elements have been collected";
			}
		}
	}
	...
	if (processingError || ((tcapCfgList.empty()) && (acapCfgList.empty()))) {
		lError() << "Unable to build a valid config because no valid list of capacities has been found - error during processing " << (processingError ? "detected" : "not detected") << " acap list is " << (acapCfgList.empty() ? "empty" : "not empty") << " tcap list is " << (tcapCfgList.empty() ? "empty" : "not empty");
	} else {
...
const PotentialCfgGraph::media_description_config & PotentialCfgGraph::getCfgForStream(const PotentialCfgGraph::session_description_config::key_type & idx) const {
	try {
...
const PotentialCfgGraph::media_description_unparsed_config & PotentialCfgGraph::getUnparsedCfgForStream(const session_description_unparsed_config::key_type & idx) const {
	try {
...
const PotentialCfgGraph::media_description_acap & PotentialCfgGraph::getMediaAcapForStream(const PotentialCfgGraph::session_description_acap::key_type & idx) const {
	try {
...
const PotentialCfgGraph::media_description_base_cap & PotentialCfgGraph::getMediaTcapForStream(const PotentialCfgGraph::session_description_base_cap::key_type & idx) const {
	try {
...
const PotentialCfgGraph::media_description_acap PotentialCfgGraph::getAllAcapForStream(const PotentialCfgGraph::session_description_acap::key_type & idx) const {
...
const PotentialCfgGraph::media_description_base_cap PotentialCfgGraph::getAllTcapForStream(const PotentialCfgGraph::session_description_base_cap::key_type & idx) const {
...
bool PotentialCfgGraph::addAcapToStream(const PotentialCfgGraph::session_description_acap::key_type & streamIdx, const unsigned int & capIdx, const std::string & capName, const std::string & capValue) {
...
bool PotentialCfgGraph::addTcapToStream(const PotentialCfgGraph::session_description_base_cap::key_type & streamIdx, const unsigned int & capIdx, const std::string & capValue) {
...
bool PotentialCfgGraph::canFindAcapWithIdx(const unsigned int & index) const {
	...
	if (foundInStreamAcap && foundInGlobalAcap) {
		lError() << "Graph may be corrupted because acap at index " << index << " has been found in both global and stream attribute capabilities";
	}
...
bool PotentialCfgGraph::canFindTcapWithIdx(const unsigned int & index) const {
	...
	if (foundInStreamTcap && foundInGlobalTcap) {
		lError() << "Graph may be corrupted because tcap at index " << index << " has been found in both global and stream attribute capabilities";
	}

	return (foundInStreamTcap || foundInGlobalTcap);
}

void PotentialCfgGraph::addCfg(const PotentialCfgGraph::session_description_config::key_type & streamIdx, const PotentialCfgGraph::media_description_config::key_type & cfgIdx, const std::list<std::map<unsigned int, bool>> & acapIdxs, std::list<unsigned int> & tcapIdx, const bool delete_media_attributes, const bool delete_session_attributes) {
...
void PotentialCfgGraph::addAcapListToCfg(const PotentialCfgGraph::session_description_config::key_type & streamIdx, const PotentialCfgGraph::media_description_config::key_type & cfgIdx, const std::map<unsigned int, bool> & acapIdx) {
...
void PotentialCfgGraph::addTcapListToCfg(const PotentialCfgGraph::session_description_config::key_type & streamIdx, const PotentialCfgGraph::media_description_config::key_type & cfgIdx, std::list<unsigned int> & tcapIdx) {
	if (!tcapIdx.empty()) {
		PotentialCfgGraph::media_description_config cfg;
		config_attribute cfgAttr;
		try {
...
std::list<config_capability<acapability>> PotentialCfgGraph::createAcapList(const PotentialCfgGraph::session_description_config::key_type & streamIdx, const std::map<unsigned int, bool> & acapIdx) const {
...
std::list<config_capability<capability>> PotentialCfgGraph::createTcapList(const PotentialCfgGraph::session_description_config::key_type & streamIdx, const std::list<unsigned int> & tcapIdx) const {
...
config_attribute PotentialCfgGraph::createCfgAttr(const PotentialCfgGraph::session_description_config::key_type & streamIdx, const std::list<std::map<unsigned int, bool>> & acapIdxs, std::list<unsigned int> & tcapIdx, const bool delete_media_attributes, const bool delete_session_attributes) const {
	config_attribute cfgAttr;

...
void SalSubscribeOp::subscribeProcessRequestEventCb (void *userCtx, const belle_sip_request_event_t *event) {
	...
	if (!op->mDialog) {
		if (method == "SUBSCRIBE") {
			auto newDialog = belle_sip_provider_create_dialog(op->mRoot->mProvider, BELLE_SIP_TRANSACTION(serverTransaction));
			if (!newDialog) {
...
int SalSubscribeOp::notify (const SalBodyHandler *bodyHandler) {
	...
	belle_sip_message_set_body_handler(BELLE_SIP_MESSAGE(request), BELLE_SIP_BODY_HANDLER(bodyHandler));
	/* It is preferable to send NOTIFY with a Contact header, because it may arrive before the initial 200 OK of the SUBSCRIBE.
	 * Thanks to the Contact, we are able to establish the dialog.
	 */

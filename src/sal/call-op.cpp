...
void SalCallOp::fillInvite (belle_sip_request_t *invite) {
	...
	if (mRoot->mSessionExpires != 0) {
		belle_sip_message_add_header(
			...
			BELLE_SIP_HEADER(belle_sip_header_session_expires_create(mRoot->mSessionExpires, BELLE_SIP_HEADER_SESSION_EXPIRES_UAC))
		);
			...
			if (method == "INVITE") {
				if (op->mState == State::Terminating) {
					...
					belle_sip_message_add_header(
						...
						BELLE_SIP_HEADER(belle_sip_header_session_expires_create(minSE, BELLE_SIP_HEADER_SESSION_EXPIRES_UAC))
					);
					belle_sip_message_add_header(
						BELLE_SIP_MESSAGE(newRequest),
					...
					if (method == "INVITE") {
						if ((code >= 200) && (code < 300)) {
							...
							if (op->mRoot->mSessionExpires != 0 ) {
								// TODO refresher=uac if mSessionExpires != 0 and no SE present
							}

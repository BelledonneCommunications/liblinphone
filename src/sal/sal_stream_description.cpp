...
bool SalConfigurationCmp::operator()(const PotentialCfgGraph::media_description_config::key_type& lhs, const PotentialCfgGraph::media_description_config::key_type& rhs) const { 
...
void SalStreamDescription::fillPotentialConfigurationsFromPotentialCfgGraph(const PotentialCfgGraph::media_description_config & sdpCfgs) {
	...
	for (const auto & SDPMediaDescriptionCfgPair : sdpCfgs) {
		...
		createPotentialConfigurationAtIdx(idx, protoMap, attrList, cfgAttributeList.delete_session_attributes, cfgAttributeList.delete_media_attributes);
	}
}
...
const SalStreamDescription::tcap_map_t::value_type & SalStreamDescription::encryptionToTcap(const SalStreamDescription::tcap_map_t & caps, const LinphoneMediaEncryption encEnum, const bool avpf) {
	const auto & it = std::find_if(caps.cbegin(), caps.cend(), [&avpf, &encEnum] (const auto & cap) {
		return (cap.second.compare(sal_media_proto_to_string(linphone_media_encryption_to_sal_media_proto(encEnum, ((avpf) ? true : false)))) == 0);
	});
	if (it != caps.end()) {
		return *it;
	}
...
void SalStreamDescription::addSupportedEncryptionFromSdp(const SalStreamDescription::tcap_map_t & protoMap, const std::list<SalStreamDescription::acap_map_t> & attrList) {
	...
	for (const auto & tAttr : protoMap) {
		...
		enc = sal_media_proto_to_linphone_media_encryption(sal_media_proto_from_string(protoStr.c_str()), (haveZrtpHash ? true : false));
...
void SalStreamDescription::createPotentialConfigurationAtIdx(const unsigned int & idx, const SalStreamDescription::tcap_map_t & tcaps, const std::list<SalStreamDescription::acap_map_t> & attrList, const bool delete_session_attributes, const bool delete_media_attributes) {
...
void SalStreamDescription::createActualCfg(const SalMediaDescription * salMediaDesc, const belle_sdp_session_description_t  *sdp, const belle_sdp_media_description_t *media_desc) {
	...
	LinphoneMediaEncryption enc = sal_media_proto_to_linphone_media_encryption(actualCfg.getProto(), (actualCfg.hasZrtpHash() ? true : false));
	supportedEncryption.push_front(enc);
...
belle_sdp_media_description_t * SalStreamDescription::toSdpMediaDescription(const SalMediaDescription * salMediaDesc, belle_sdp_session_description_t *session_desc) const {
	...
	if (actualCfg.mixer_to_client_extension_id != 0) {
		char *value = bctbx_strdup_printf("%i urn:ietf:params:rtp-hdrext:csrc-audio-level", actualCfg.mixer_to_client_extension_id);
	...
	if (actualCfg.proto != SalProtoUdpTlsRtpSavpf && actualCfg.proto != SalProtoUdpTlsRtpSavp && actualCfg.conference_ssrc) {
...
bool SalStreamDescription::hasConfigurationAtIndex(const PotentialCfgGraph::media_description_config::key_type & index) const {
	const auto & elCount = cfgs.count(index);
	return (elCount != 0);
}

const SalStreamConfiguration & SalStreamDescription::getConfigurationAtIndex(const PotentialCfgGraph::media_description_config::key_type & index) const {
	try {
...
void SalStreamDescription::addConfigurationAtIndex(const PotentialCfgGraph::media_description_config::key_type & idx, const SalStreamConfiguration & cfg) {
	cfgs[idx] = cfg;
}

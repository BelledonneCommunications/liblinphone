...
void Sal::processDialogTerminatedCb(BCTBX_UNUSED(void *sal), const belle_sip_dialog_terminated_event_t *event) {
	...
	else lError() << "Sal::processDialogTerminatedCb(): no op found for this dialog [" << dialog << "], ignoring";
}
...
void Sal::processIoErrorCb(BCTBX_UNUSED(void *userCtx), const belle_sip_io_error_event_t *event) {
	if (BELLE_SIP_OBJECT_IS_INSTANCE_OF(belle_sip_io_error_event_get_source(event), belle_sip_client_transaction_t)) {
		...
		auto op =
		    static_cast<SalOp *>(belle_sip_transaction_get_application_data(BELLE_SIP_TRANSACTION(client_transaction)));
		// Also reset auth count on IO error
		op->mAuthRequests = 0;
		if (op->mCallbacks && op->mCallbacks->process_io_error) op->mCallbacks->process_io_error(op, event);
	} else {
...
void Sal::processRequestEventCb(void *userCtx, const belle_sip_request_event_t *event) {
	...
	if (dialog) {
		...
		if (method == "INVITE") {
			...
			op->fillCallbacks();
		           ...
		           (evh = belle_sip_message_get_header(BELLE_SIP_MESSAGE(request), "Event")) != nullptr) {
			string eventHeaderValue = belle_sip_header_get_unparsed_value(evh);
			if (eventHeaderValue == "presence") op = new SalPresenceOp(sal);
			else op = new SalSubscribeOp(sal);
			op->fillCallbacks();
	...
	if (!op->mFromAddress) {
		...
		if (uri) {
			addressHeader = belle_sip_header_address_create(
			    belle_sip_header_address_get_displayname(BELLE_SIP_HEADER_ADDRESS(fromHeader)), uri);
		} else if (absoluteUri) {
			addressHeader = belle_sip_header_address_create2(
			    belle_sip_header_address_get_displayname(BELLE_SIP_HEADER_ADDRESS(fromHeader)), absoluteUri);
		} else {
	...
	if (!op->mToAddress) {
		...
		if (uri) {
			addressHeader = belle_sip_header_address_create(
			    belle_sip_header_address_get_displayname(BELLE_SIP_HEADER_ADDRESS(toHeader)), uri);
		} else if (absoluteUri) {
			addressHeader = belle_sip_header_address_create2(
			    belle_sip_header_address_get_displayname(BELLE_SIP_HEADER_ADDRESS(toHeader)), absoluteUri);
		} else {
	...
	if (op->mRemoteUserAgent.empty()) op->setRemoteUserAgent(BELLE_SIP_MESSAGE(request));

	if (op->mCallId.empty()) {
		op->mCallId = belle_sip_header_call_id_get_call_id(BELLE_SIP_HEADER_CALL_ID(
		    belle_sip_message_get_header_by_type(BELLE_SIP_MESSAGE(request), belle_sip_header_call_id_t)));
	}
	...
	if (op->mCallbacks && op->mCallbacks->process_request_event) op->mCallbacks->process_request_event(op, event);
	else lError() << "Sal::processRequestEventCb(): not implemented yet";
}
...
void Sal::processResponseEventCb(BCTBX_UNUSED(void *userCtx), const belle_sip_response_event_t *event) {
	...
	if (op->mCallId.empty()) {
		op->mCallId = belle_sip_header_call_id_get_call_id(BELLE_SIP_HEADER_CALL_ID(
		    belle_sip_message_get_header_by_type(BELLE_SIP_MESSAGE(response), belle_sip_header_call_id_t)));
	}
			...
			case 407:
				if ((op->mState == SalOp::State::Terminating) && (method != "BYE")) {
					...
					if (++op->mAuthRequests > 2) {
						lWarning() << "Auth info cannot be found for op [" << op->getFrom() << "/" << op->getTo()
						           << "] after 2 attempts, giving up";
						op->mRoot->mCallbacks.auth_failure(op, op->mAuthInfo);
						op->mRoot->removePendingAuth(op);
					} else {
						op->mPendingAuthTransaction =
						    BELLE_SIP_CLIENT_TRANSACTION(belle_sip_object_ref(clientTransaction));
				...
				break;
			case 403:
				if ((op->mAuthInfo) && (method == "REGISTER")) op->mRoot->mCallbacks.auth_failure(op, op->mAuthInfo);
				break;
			case 302:
			case 301:
				op->mAuthRequests = 0;
				if (op->mRoot->mCallbacks.process_redirect(op) == 0) return;
				break;
			case 491: /* request pending */
				op->handleRetry();
				break;
		}
		if ((responseCode >= 180) && (responseCode != 401) && (responseCode != 407) &&
		    (responseCode != 403)) // Not an auth request
			op->mAuthRequests = 0;
...
void Sal::processTransactionTerminatedCb(BCTBX_UNUSED(void *userCtx),
	...
	auto transaction =
	    clientTransaction ? BELLE_SIP_TRANSACTION(clientTransaction) : BELLE_SIP_TRANSACTION(serverTransaction);
...
void Sal::setCallbacks(const Callbacks *cbs) {
	...
	if (!mCallbacks.call_received) mCallbacks.call_received = (OnCallReceivedCb)unimplementedStub;
	if (!mCallbacks.call_ringing) mCallbacks.call_ringing = (OnCallRingingCb)unimplementedStub;
	if (!mCallbacks.call_accepted) mCallbacks.call_accepted = (OnCallAcceptedCb)unimplementedStub;
	if (!mCallbacks.call_failure) mCallbacks.call_failure = (OnCallFailureCb)unimplementedStub;
	if (!mCallbacks.call_terminated) mCallbacks.call_terminated = (OnCallTerminatedCb)unimplementedStub;
	if (!mCallbacks.call_released) mCallbacks.call_released = (OnCallReleasedCb)unimplementedStub;
	if (!mCallbacks.call_updating) mCallbacks.call_updating = (OnCallUpdatingCb)unimplementedStub;
	if (!mCallbacks.auth_failure) mCallbacks.auth_failure = (OnAuthFailureCb)unimplementedStub;
	if (!mCallbacks.register_success) mCallbacks.register_success = (OnRegisterSuccessCb)unimplementedStub;
	if (!mCallbacks.register_failure) mCallbacks.register_failure = (OnRegisterFailureCb)unimplementedStub;
	if (!mCallbacks.dtmf_received) mCallbacks.dtmf_received = (OnDtmfReceivedCb)unimplementedStub;
	if (!mCallbacks.notify) mCallbacks.notify = (OnNotifyCb)unimplementedStub;
	if (!mCallbacks.subscribe_received) mCallbacks.subscribe_received = (OnSubscribeReceivedCb)unimplementedStub;
	...
	if (!mCallbacks.notify_presence) mCallbacks.notify_presence = (OnNotifyPresenceCb)unimplementedStub;
	...
	if (!mCallbacks.message_received) mCallbacks.message_received = (OnMessageReceivedCb)unimplementedStub;
...
int Sal::addListenPort(SalAddress *addr, bool isTunneled) {
	...
	if (isTunneled) {
		...
		listeningPoint =
		    belle_sip_stack_create_listening_point(mStack, sal_address_get_domain(addr), sal_address_get_port(addr),
		                                           sal_transport_to_string(sal_address_get_transport(addr)));
	}

	if (!listeningPoint) return -1;
	...
	if (sal_address_get_transport(addr) == SalTransportTLS) setTlsProperties();
	return result;
}
...
int Sal::getListeningPort(SalTransport tr) {
	...
	if (listeningPoint) return belle_sip_listening_point_get_port(listeningPoint);
	return 0;
}
...
const string &Sal::getSupportedTags() const {
	if (mSupportedHeader) mSupported = belle_sip_header_get_unparsed_value(mSupportedHeader);
	else mSupported.clear();
	return mSupported;
}
...
const string &Sal::getUserAgent() const {
	char userAgent[256];
	belle_sip_header_user_agent_get_products_as_string(mUserAgentHeader, userAgent,
	                                                   (unsigned int)sizeof(userAgent) - 1);
...
void Sal::setKeepAlivePeriod(unsigned int value) {
	...
	for (auto it = belle_sip_provider_get_listening_points(mProvider); it; it = bctbx_list_next(it)) {
		...
		if (mUseTcpTlsKeepAlive || (strcasecmp(belle_sip_listening_point_get_transport(listeningPoint), "udp") == 0)) {
			belle_sip_listening_point_set_keep_alive(listeningPoint, (int)mKeepAlive);
		}
	}
}
...
void Sal::addContentTypeSupport(const string &contentType) {
	if (!contentType.empty() && !isContentTypeSupported(contentType)) mSupportedContentTypes.push_back(contentType);
}
...
void Sal::removeContentTypeSupport(const string &contentType) {
	...
	if (it != mSupportedContentTypes.end()) mSupportedContentTypes.erase(it);
}
...
string Sal::generateUuid() {
	...
	int written = snprintf(generatedUuid, sizeof(generatedUuid) - 1, "%8.8x-%4.4x-%4.4x-%2.2x%2.2x-",
	                       uuidStruct.timeLow, uuidStruct.timeMid, uuidStruct.timeHiAndVersion,
	                       uuidStruct.clockSeqHiAndReserved, uuidStruct.clockSeqLow);
	for (int i = 0; i < 6; i++)
		written += snprintf(generatedUuid + written, sizeof(generatedUuid) - (unsigned long)written, "%2.2x",
		                    uuidStruct.node[i]);
...
#if TARGET_OS_IPHONE
	belle_sip_stack_set_dns_engine(mStack,
	                               bctbx_list_size(servers) > 0
	                                   ? BELLE_SIP_DNS_DNS_C
	                                   : BELLE_SIP_DNS_APPLE_DNS_SERVICE); // Make sure we are not using Apple DNS
	                                                                       // Service when a custom DNS server is set
#endif
...
Sal::resolveA(const string &name, int port, int family, belle_sip_resolver_callback_t cb, void *data) {
	return belle_sip_stack_resolve_a(mStack, L_STRING_TO_C(name), port, family, cb, data);
}

belle_sip_resolver_context_t *Sal::resolve(const string &service,
...
belle_sip_source_t *
Sal::createTimer(const std::function<bool()> &something, unsigned int milliseconds, const string &name) {
	return belle_sip_main_loop_create_cpp_timeout_2(belle_sip_stack_get_main_loop(mStack), something,
	                                                (unsigned)milliseconds, name.c_str());
}

belle_sip_source_t *
Sal::createTimer(belle_sip_source_func_t func, void *data, unsigned int timeoutValueMs, const string &timerName) {
...
LINPHONE_PUBLIC void sal_call_set_sdp_handling(SalOp *op, SalOpSDPHandling handling) {
	...
	if (callOp) callOp->setSdpHandling(handling);
}

LINPHONE_PUBLIC const char *sal_call_get_local_tag(SalOp *op) {
	auto callOp = dynamic_cast<SalCallOp *>(op);
	if (!callOp) return nullptr;

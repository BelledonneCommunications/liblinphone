...
public:
	...
	void restartSessionTimersTimer(SalCallOp *op, int delta);
	bool refreshSession(const std::string &subject, bool noUserConsent);
	int update (const std::string &subject, bool noUserConsent);
	int update (const std::string &subject, bool noUserConsent, bool withSDP);

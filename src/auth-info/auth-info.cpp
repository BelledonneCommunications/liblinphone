...
AuthInfo::AuthInfo(const string &username, const string &userid, const string &passwd, const string &ha1, const string &realm, const string &domain, const string &algorithm) : mUsername(username), mUserid(userid), mPasswd(passwd), mRealm(realm), mDomain(domain){
    if(algorithm.empty()){
        mAlgorithm = "MD5";
    }
    else{
...
AuthInfo* AuthInfo::clone() const {
    return new AuthInfo(getUsername(), getUserid(), getPasswd(), getHa1(), getRealm(), getDomain());
}
...
const string& AuthInfo::getPasswd() const{
    return mPasswd;
}
...
void AuthInfo::setPasswd(const string &passwd){
    mPasswd = passwd;
}
...
void AuthInfo::setAlgorithm(const string &algorithm){
        ...
        if(algorithm.compare("MD5") && algorithm.compare("SHA-256")){
...
void AuthInfo::writeConfig(LpConfig *config, int pos){
    ...
    if (getHa1().empty() && !getRealm().empty() && !getPasswd().empty() && (!getUsername().empty() || !getUserid().empty()) && store_ha1_passwd) {
		/* Default algorithm is MD5 if it's NULL */

        if(getAlgorithm().compare("MD5")){
            myHa1 = reinterpret_cast<char *>(ms_malloc(33));
            sal_auth_compute_ha1(getUserid().empty() ? getUsername().c_str() : getUserid().c_str(), getRealm().c_str(), getPasswd().c_str(), myHa1);
            mHa1 = myHa1;
        }
            /* If algorithm is SHA-256, calcul ha1 by sha256*/
        else if(getAlgorithm().compare("SHA-256")){
            myHa1 = reinterpret_cast<char *>(ms_malloc(65));
            sal_auth_compute_ha1_for_algorithm(getUserid().empty() ? getUsername().c_str() : getUserid().c_str(), getRealm().c_str(), getPasswd().c_str(), myHa1, 65, getAlgorithm().c_str());
            mHa1 = myHa1;
        }
    }
	...
	if (store_ha1_passwd && !getHa1().empty()) {
	    ...
	    lp_config_set_string(config, key, "passwd", getPasswd().c_str());
		}
...
std::string AuthInfo::toString() const{
    ...
    ss << "TlsCert[" << mTlsCert << "];";
    ss << "TlsKey[" << mTlsKey << "];";
    ss << "TlsCertPath[" << mTlsCertPath << "];";
    ss << "TlsKeyPath[" << mTlsKeyPath << "];";

	...
	if (mOldParams == nullptr
	  ...
	  if (mCore && !mCore->push_notification_enabled) {
		  if (!mParams->isPushNotificationAvailable()) {
			  lError() << "Couldn't update push notifications parameters on account [" << this->toC() << "] because account params do not have available push notifications";
		  } else {
			  mParams->getPushNotificationConfig()->generatePushParams(mParams->mPushNotificationAllowed , mParams->mRemotePushNotificationAllowed);
		  }
	  }
		*/
	...
	if (host) {
		...
		if (mCore && mCore->push_notification_enabled) {
			if (!mParams->isPushNotificationAvailable()) {
				...
				mParams->mPushNotificationConfig->generatePushParams(mParams->mPushNotificationAllowed, mParams->mRemotePushNotificationAllowed);
		...
		if (!mParams->mContactUriParameters.empty()) {
			if (successfullyPreparedPushParameters) {
				...
				Address contactParamsWrapper(string("sip:dummy;" + mParams->mContactUriParameters));
				bool didRemoveParams = false;
				for (auto pushParam : mParams->mPushNotificationConfig->getPushParamsMap()) {
				...
				if (didRemoveParams) {
					...
					lWarning() << "Account [" << this << "] contact uri parameters changed from '" << mParams->mContactUriParameters<< "' to '" << newContactUriParams << "'";
					mParams->mContactUriParameters = newContactUriParams;
				}
		...
		if (successfullyPreparedPushParameters) {
			linphone_address_set_uri_param(result, PushConfigPridKey.c_str(), mParams->getPushNotificationConfig()->getPrid().c_str());
			linphone_address_set_uri_param(result, PushConfigProviderKey.c_str(), mParams->getPushNotificationConfig()->getProvider().c_str());
			linphone_address_set_uri_param(result, PushConfigParamKey.c_str(), mParams->getPushNotificationConfig()->getParam().c_str());
			
			auto &pushParams = mParams->getPushNotificationConfig()->getPushParamsMap();
			...
			if (mParams->mRemotePushNotificationAllowed) {
				linphone_address_set_uri_param(result, PushConfigMsgStrKey.c_str(), mParams->getPushNotificationConfig()->getMsgStr().c_str());
				linphone_address_set_uri_param(result, PushConfigCallStrKey.c_str(), mParams->getPushNotificationConfig()->getCallStr().c_str());
				linphone_address_set_uri_param(result, PushConfigGroupChatStrKey.c_str(), mParams->getPushNotificationConfig()->getGroupChatStr().c_str());
				linphone_address_set_uri_param(result, PushConfigCallSoundKey.c_str(), mParams->getPushNotificationConfig()->getCallSnd().c_str());
				linphone_address_set_uri_param(result, PushConfigMsgSoundKey.c_str(), mParams->getPushNotificationConfig()->getMsgSnd().c_str());
			}
			lInfo() << "Added push notification informations '" << mParams->getPushNotificationConfig()->asString(mParams->mRemotePushNotificationAllowed) << "' added to account [" << this << "]";
		}
	}

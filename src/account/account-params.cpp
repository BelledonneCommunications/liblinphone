...
AccountParams::AccountParams (LinphoneCore *lc)
{
	...
	mPushNotificationConfig = unique_ptr<PushNotificationConfig>(new PushNotificationConfig(string(lc ? linphone_config_get_default_string(lc->config, "proxy", "push_parameters", "") : "")));
	...
	if (lc && lc->push_config) {
		mPushNotificationConfig = unique_ptr<PushNotificationConfig>(PushNotificationConfig::toCpp(lc->push_config)->clone());
	} else {
		mPushNotificationConfig = unique_ptr<PushNotificationConfig>(new PushNotificationConfig());
	}
}
...
AccountParams::AccountParams (LinphoneCore *lc, int index) : AccountParams(lc) {
	...
	if (corePushEnabled) {
		mPushNotificationConfig = unique_ptr<PushNotificationConfig>(new PushNotificationConfig(linphone_config_get_string(config, key, "push_parameters", "")));
	} else {
		mPushNotificationConfig = unique_ptr<PushNotificationConfig>(new PushNotificationConfig(mContactUriParameters));
	}
...
AccountParams::AccountParams (const AccountParams &other) : HybridObject(other) {
	...
	mPushNotificationConfig = unique_ptr<PushNotificationConfig>(other.mPushNotificationConfig->clone());
}
...
void AccountParams::setPushNotificationConfig (PushNotificationConfig *pushNotificationConfig) {
	mPushNotificationConfig = unique_ptr<PushNotificationConfig>(pushNotificationConfig);
}
...
PushNotificationConfig* AccountParams::getPushNotificationConfig () const {
	return mPushNotificationConfig.get();
}

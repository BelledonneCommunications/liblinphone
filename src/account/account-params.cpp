...
AccountParams::AccountParams (LinphoneCore *lc)
{
	...
	if (lc && lc->push_config) {
		mPushNotificationConfig = make_unique<PushNotificationConfig>(PushNotificationConfig::toCpp(lc->push_config)->clone());
	} else {
		mPushNotificationConfig = make_unique<PushNotificationConfig>(string(lc ? linphone_config_get_default_string(lc->config, "proxy", "push_parameters", "") : ""));
	}
}
...
AccountParams::AccountParams (LinphoneCore *lc, int index) : AccountParams(lc) {
	...
	if (corePushEnabled) {
		mPushNotificationConfig = make_unique<PushNotificationConfig>(linphone_config_get_string(config, key, "push_parameters", ""));
	} else {
		mPushNotificationConfig = make_unique<PushNotificationConfig>(mContactUriParameters);
	}
...
AccountParams::AccountParams (const AccountParams &other) : HybridObject(other) {
	...
	mPushNotificationConfig = make_unique<PushNotificationConfig>(other.mPushNotificationConfig->clone());
}
...
void AccountParams::setPushNotificationConfig (PushNotificationConfig *pushNotificationConfig) {
	mPushNotificationConfig = make_unique<PushNotificationConfig>(pushNotificationConfig);
}

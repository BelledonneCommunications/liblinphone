...
AudioDevice* Call::getOutputAudioDevice() const {
	switch (getState()) {
		case CallSession::State::Paused:
		{
			/* in paused state, the AudioDevice may be used by the ToneManager, transciently, to play a waiting
			 * tone indication. */
		...
		case CallSession::State::Pausing:
		case CallSession::State::IncomingReceived:
			return getCore()->getPrivate()->getToneManager().getOutputDevice(getActiveSession());
		default:
		break;
	}

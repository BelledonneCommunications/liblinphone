...
void ServerGroupChatRoomPrivate::confirmCreation () {
	...
	if (remoteContactAddress.hasParam("+org.linphone.specs")) {
		...
		if (groupchat == protocols.end()){
			lError() << "Creator " << remoteContactAddress.asStringUriOnly() << " has no groupchat capability set: " << linphoneSpecs;
			q->setState(ConferenceInterface::State::CreationFailed);
			auto errorInfo = linphone_error_info_new();
			linphone_error_info_set(errorInfo, nullptr, LinphoneReasonNotAcceptable, 488, "\"groupchat\" capability has not been found in remote contact address", nullptr);
...
bool ServerGroupChatRoomPrivate::subscribeRegistrationForParticipants(const std::list<IdentityAddress> &identAddresses, bool newInvited){
	...
	if (gs != LinphoneGlobalOn) {
		lError() << __func__ << "The core is currently in state " << std::string(linphone_global_state_to_string(gs)) << " but registration subscriptions can only be set up when the core is in statez GlobalOn";
		return false;
	}
...
void ServerGroupChatRoomPrivate::removeCachedParticipant(const IdentityAddress &address) {
	L_Q();
	auto c = std::find_if(q->cachedParticipants.begin(), q->cachedParticipants.end(),
			      [&address](const auto &p) { return (address.asAddress().weakEqual(p->getAddress().asAddress())); });
...
void ServerGroupChatRoomPrivate::conclude(){
	...
	if (q->getParticipants().size() < 2){
		...
		// We may end up here if a client successfully created a chat room but the conference server thinks it should be allowed to be part of a conference.
		// A scenario where this branch is hit is the following.
		// The client creating the chatroom registered into the server without the groupchat capability. Nonetheless, the capability is added in the INVITE creating the chatroom.
		// Upon reception of the 302 Moved Temporarely, the client will dial the chatroom URI directly and the server will look for devices allowed to join the chatroom in method ServerGroupChatRoomPrivate::subscribeRegistrationForParticipants(). Since "groupchat" capability was not there, then the server doesn't allow to conclude the creation of the chatroom 
		// - 
		lError() << q << ": Declining session because it looks like the device creating the chatroom is not allowed to be part of this chatroom";
...
void ServerGroupChatRoomPrivate::onCallSessionStateChanged(const shared_ptr<CallSession> &session,
	...
	if (!device) {
		...
		if ((newState == CallSession::State::Released) && (session->getReason() == LinphoneReasonNotAcceptable) && (q->getState() == ConferenceInterface::State::CreationFailed)) {
			// Delete the chat room from the main DB as its termination process started and it cannot be retrieved in the future
...
void ServerGroupChatRoom::onGlobalStateChanged(LinphoneGlobalState state) {
	...
	if (state == LinphoneGlobalOn) {
		// Try to subscribe again to cached participants when the core goes to GlobalOn state because the first time around (when the chat room goes into state Created) the core might have still being initializing.
		// We must do be sure that the core is initialized because if one or more participants are in another domain that the chatroom. In such a scenarion, in fact, the server might be sending out SIP SUBSCRIBE and therefore the channel should not be destroyed by network changes that occur during startup
		lInfo() << "The core has reached the GlobalOn state, therefore try to subscribe participants of chatroom " << getConference()->getConferenceAddress().asAddress();
		list<IdentityAddress> participantAddresses;
		for (const auto &participant : cachedParticipants) {
			participantAddresses.emplace_back(participant->getAddress());
		}

	...
	if (event->response) {
		...
		if (code == 204) { // this is the reply to the first post to the server - an empty msg
			...
			if (body && strlen(body) > 0) {
				...
				string xml_body = dumpFileTransferContentAsXmlString(parsedXmlFileTransferContent, contentKey, contentKeySize, contentAuthTag, contentAuthTagSize, currentFileTransferContent->getFileName());
				delete parsedXmlFileTransferContent;

...
shared_ptr<ParticipantDevice> ParticipantPrivate::addDevice (const Address &address) {
	L_Q();
	shared_ptr<ParticipantDevice> device = findDevice(address);
	if (device)
		return device;
	device = make_shared<ParticipantDevice>(q, address);

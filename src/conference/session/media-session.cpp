...
void MediaSessionPrivate::fillConferenceParticipantVideoStream(SalStreamDescription &newStream, const std::shared_ptr<SalMediaDescription> & oldMd, const std::shared_ptr<SalMediaDescription> & md, const std::shared_ptr<ParticipantDevice> & dev, PayloadTypeHandler & pth) {
...
void MediaSessionPrivate::copyOldStreams(std::shared_ptr<SalMediaDescription> & md, const std::shared_ptr<SalMediaDescription> & oldMd, const std::shared_ptr<SalMediaDescription> & refMd, PayloadTypeHandler & pth) {
	L_Q();
	...
	if (refMd) {
		...
		for (const auto & s : refMd->streams) {
			const std::string layoutAttrValue = s.getContent();
			const std::string participantsAttrValue = s.getLabel();

			// If in a conference, either local or remote
			if (((conference && isInLocalConference) || remoteContactAddress.hasParam("isfocus")) && ((layoutAttrValue.empty() && !participantsAttrValue.empty()) || (!layoutAttrValue.empty() && (layoutAttrValue.compare("main") != 0)))) {
...
//				if (!participantsAttrValue.empty() || !layoutAttrValue.empty()) {
					...
					if (conference && isInLocalConference && !participantsAttrValue.empty()) {
						newStream.setLabel(participantsAttrValue);
					}
					if (!layoutAttrValue.empty()) {
						newStream.setContent(layoutAttrValue.c_str());
					}
...
/*
					...
					if (oldMd && (streamIdx < oldMd->streams.size())) {
					...
					if ((streamIdx < oldMd->streams.size()) && ((cfg.dir == SalStreamInactive) || (oldMd->streams[streamIdx].rtp_port != 0))) {
						// Copy previous rtp and rtcp ports if they were already assigned
...
void MediaSessionPrivate::makeLocalMediaDescription(bool localIsOfferer, const bool supportsCapabilityNegotiationAttributes, const bool offerNegotiatedMediaProtocolOnly, const bool forceCryptoKeyGeneration) {
	...
	if ((localIsOfferer && getParams()->audioEnabled()) || (oldAudioStream != Utils::getEmptyConstRefObject<SalStreamDescription>())){
		...
		const auto audioStreamIdx = refMd ? refMd->findIdxBestStream(SalAudio) : -1;
		SalStreamDescription & audioStream = addStreamToMd(md, audioStreamIdx);
	...
	const SalStreamDescription &oldVideoStream = refMd ? ((conference && (refMd->findStreamWithContent("main") != Utils::getEmptyConstRefObject<SalStreamDescription>())) ? refMd->findStreamWithContent("main") : refMd->findBestStream(SalVideo)) : Utils::getEmptyConstRefObject<SalStreamDescription>();
	...
	if ((localIsOfferer && addVideoStream) || (oldVideoStream != Utils::getEmptyConstRefObject<SalStreamDescription>())) {
		...
		if (conference && isInLocalConference) {
			...
			if (op && op->getRemoteContactAddress()) {
				...
				if (remoteContactAddress.hasParam("isfocus")) {
					const std::string layoutAttrValue = oldVideoStream.getContent();
					if (!layoutAttrValue.empty()) {
						videoStream.setContent(layoutAttrValue);
					}
				}
			}
		}
		...
		if (videoStream.getDirection() == SalStreamInactive) {
			videoStream.disable();
		}
	...
	if ((localIsOfferer && getParams()->realtimeTextEnabled()) || (oldTextStream != Utils::getEmptyConstRefObject<SalStreamDescription>())){
		...
		SalStreamDescription & textStream = addStreamToMd(md, textStreamIdx);
	...
	const auto videoStreamIndex = conference ? mdForMainStream->findIdxStreamWithContent(mainStreamAttrValue) : mdForMainStream->findIdxBestStream(SalVideo);

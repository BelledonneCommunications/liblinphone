...
void MediaSessionPrivate::stunAuthRequestedCb(void *userData, const char *realm, const char *nonce,
											  const char **username, const char **password, const char **ha1) {
...
void MediaSessionPrivate::accepted() {
	...
	case CallSession::State::Connected:
		if (q->getCore()->getCCore()->sip_conf.sdp_200_ack) {
			...
			if (!localIsOfferer) {
				// We were waiting for an incoming offer. Now prepare the local media description according to remote
				// offer.
			 ...
			 * enough time for the gathering step. Only local candidates will be answered in the ACK.
			 */
			if (linphone_nat_policy_ice_enabled(natPolicy)) {
				if (getStreamsGroup().prepare()) {
					lWarning()
						<< "Some gathering is needed for ICE, however since a defered sending of ACK is not supported"
	...
	if (md) {
		...
		case CallSession::State::UpdatedByRemote:
			if (state == CallSession::State::Updating && prevState == CallSession::State::Paused) {
				// If previous state was paused and we are in Updating, there is no reason to set it to anything else
				// than paused
				nextState = CallSession::State::Paused;
				nextStateMsg = "Call paused";
			} else {
				// The call always enters state PausedByRemote if all streams are rejected. This is done to support some
				// clients who accept to stop the streams by setting the RTP port to 0 If the call is part of a
		...
		case CallSession::State::Pausing:
			/* When we entered the pausing state, we always reach the paused state whatever the content of the remote
			 * SDP is. Our streams are all send-only (with music), soundcard and camera are never used. */
			nextState = CallSession::State::Paused;
			nextStateMsg = "Call paused";
			if (referPending)
		...
		// A negative value of the counter may lead to unexpected behaviour, hence terminate here the execution in order
		...
		if (nextState == CallSession::State::Idle) {
			...
			// If capability negotiation is enabled, a second invite must be sent if the selected configuration is not
			// the actual one. It normally occurs after moving to state StreamsRunning. However, if ICE negotiations are
			// not completed, then this action will be carried out together with the ICE re-INVITE
			if (localDesc->supportCapabilityNegotiation() && (nextState == CallSession::State::StreamsRunning) &&
				localIsOfferer && capabilityNegotiationReInviteEnabled) {
				// If no ICE session or checklist has completed, then send re-INVITE
				// The reINVITE to notify intermediaries that do not support capability negotiations (RFC5939) is sent
				// in the following scenarions:
				// - no ICE session is found in th stream group
				// - an ICE sesson is found and its checklist has already completed
				// - an ICE sesson is found and ICE reINVITE is not sent upon completition if the checklist (This case
				// is the default one for DTLS SRTP negotiation as it was observed that webRTC gateway did not correctly
				// support SIP ICE reINVITEs)
				if (!getStreamsGroup().getIceService().getSession() ||
					(getStreamsGroup().getIceService().getSession() &&
					 (!isUpdateSentWhenIceCompleted() || getStreamsGroup().getIceService().hasCompletedCheckList()))) {
					...
					if (potentialConfigurationChosen) {
						...
						lInfo() << "Using actual configuration after capability negotiation procedure, hence no need "
								   "to send a reINVITE";
					}
				} else {
			...
			// If the call was added to a conference after the last INVITE session was started, the reINVITE to enter
			// conference must be sent only if capability negotiation reINVITE was not sent
			if (!capabilityNegotiationReInviteSent) {
				...
				q->getCore()->doLater([this]() {
					/* This has to be done outside of the accepted callback, because after the callback the SIP ACK is
					 * going to be sent. Despite it is not forbidden by RFC3261, it is preferable for the sake of
					 * clarity that the ACK for the current transaction is sent before the new INVITE that will be sent
					 * by tryEnterConference(). Some implementations (eg FreeSwitch) reply "500 Overlapped request"
					 * otherwise ( which is in fact a misunderstanding of RFC3261).
					 */
					tryEnterConference();
				});
			}
...
void MediaSessionPrivate::setState(CallSession::State newState, const string &message) {
	...
	case CallSession::State::UpdatedByRemote:
		...
		if (rmd && getIceService().reinviteNeedsDeferedResponse(rmd)) {
			...
			lInfo() << "CallSession [" << q
					<< "]: ICE reinvite received, but one or more check-lists are not completed. Response will be sent "
					   "later, when ICE has completed";
		}
		break;
	default:
		break;
...
void MediaSessionPrivate::getLocalIp(const Address &remoteAddr) {
	...
	mediaLocalIp4 = linphone_config_get_string(linphone_core_get_config(q->getCore()->getCCore()), "rtp", "ipv4_sdp_address", nullptr);
	mediaLocalIp6 = linphone_config_get_string(linphone_core_get_config(q->getCore()->getCCore()), "rtp", "ipv6_sdp_address", nullptr);
...
int MediaSessionPrivate::portFromStreamIndex(int index) {
	if (index != -1) {
		...
		if (stream)
			return stream->getPortConfig().rtpPort;
	}
	return 0;
}
	...
	for (auto &sd : md->streams) {
		...
		case CallSession::State::Paused:
			if (sd.getDirection() == SalStreamRecvOnly) {
				...
				if ((sd.type == SalVideo) && linphone_config_get_int(linphone_core_get_config(q->getCore()->getCCore()),
																	 "sip", "inactive_video_on_pause", 0))
					sd.setDirection(SalStreamInactive);
			}
			break;
		default:
...
void MediaSessionPrivate::addStreamToBundle(const std::shared_ptr<SalMediaDescription> &md, SalStreamDescription &sd,
											SalStreamConfiguration &cfg, const std::string &mid) {
...
void MediaSessionPrivate::fillConferenceParticipantVideoStream(SalStreamDescription &newStream,
	...
	if (dev) {
		...
		if (!l.empty()) {
			...
			if (getParams()->rtpBundleEnabled())
				addStreamToBundle(md, newStream, cfg, "vs" + label);
...
void MediaSessionPrivate::fillLocalStreamDescription(SalStreamDescription &stream,
													 ...
													 const std::string name, const SalStreamType type,
													 const SalMediaProto proto, const SalStreamDir dir,
													 const std::list<OrtpPayloadType *> &codecs, const std::string mid,
...
SalStreamDescription &MediaSessionPrivate::addStreamToMd(std::shared_ptr<SalMediaDescription> md, int streamIdx,
														 const std::shared_ptr<SalMediaDescription> &oldMd) {
...
void MediaSessionPrivate::copyOldStreams(std::shared_ptr<SalMediaDescription> &md,
										 const std::shared_ptr<SalMediaDescription> &oldMd,
										 const std::shared_ptr<SalMediaDescription> &refMd, PayloadTypeHandler &pth,
										 const std::list<LinphoneMediaEncryption> &encs) {
	L_Q();
...
void MediaSessionPrivate::makeLocalMediaDescription(bool localIsOfferer,
	...
	const SalStreamDescription &oldVideoStream =
		(conference) ? ((oldGridLayoutMainVideoStream == Utils::getEmptyConstRefObject<SalStreamDescription>())
							? oldActiveSpeakerLayoutMainVideoStream
	...
	if (addVideoStream || (oldVideoStream != Utils::getEmptyConstRefObject<SalStreamDescription>())) {
		...
		const auto videoStreamIdx = refMd ? ((conference && refMd->findIdxStreamWithContent(mainStreamAttrValue))
												 ? refMd->findIdxStreamWithContent(mainStreamAttrValue)
												 : refMd->findIdxBestStream(SalVideo))
										  : -1;
	...
	if (audioStreamIndex != -1)
		getStreamsGroup().setStreamMain(static_cast<size_t>(audioStreamIndex));
	...
	if (videoStreamIndex != -1)
		getStreamsGroup().setStreamMain(static_cast<size_t>(videoStreamIndex));
	const auto textStreamIndex = mdForMainStream->findIdxBestStream(SalText);
	if (textStreamIndex != -1)
		getStreamsGroup().setStreamMain(static_cast<size_t>(textStreamIndex));
	...
	if (oldMd) {
		...
		if (getParams()->getPrivate()->getInternalCallUpdate()) {
			 ...
			 * However, the localDesc may change between first INVITE and ICE reINVITE, for example if the remote party
			 * has declined a video stream. We use the internalCallUpdate flag to prevent trigger an unnecessary media
			 * restart.
			 */
			localDescChanged = 0;
		}
...
void MediaSessionPrivate::onIceCompleted(BCTBX_UNUSED(IceService &service)) {
	...
	if (!getStreamsGroup().getIceService().hasCompletedCheckList())
		return;
...
MSWebCam *MediaSessionPrivate::getVideoDevice() const {
	...
	MS2VideoStream *vs = getStreamsGroup().lookupMainStreamInterface<MS2VideoStream>(SalVideo);
	if (vs)
		return vs->getVideoDevice(state);
#endif
	return nullptr;
}
...
void MediaSessionPrivate::updateCurrentParams() const {
	...
	if (md) {
		...
		const auto &videoStream =
			(streamIdx == -1) ? md->findBestStream(SalVideo) : md->getStreamIdx(static_cast<unsigned int>(streamIdx));
		if (videoStream != Utils::getEmptyConstRefObject<SalStreamDescription>()) {
...
LinphoneStatus MediaSessionPrivate::acceptUpdate(const CallSessionParams *csp, CallSession::State nextState,
...
void MediaSessionPrivate::stunAuthRequestedCb(const char *realm, BCTBX_UNUSED(const char *nonce), const char **username,
											  const char **password, const char **ha1) {
	L_Q();
...
MediaSession::MediaSession(const shared_ptr<Core> &core, std::shared_ptr<Participant> me,
						   const CallSessionParams *params, CallSessionListener *listener)
...
void MediaSession::configure(LinphoneCallDir direction, LinphoneProxyConfig *cfg, SalCallOp *op, const Address &from,
							 const Address &to) {
	L_D();
		...
		if (linphone_nat_policy_ice_enabled(d->natPolicy)) {
			...
			if (d->deferIncomingNotification) {
				auto incomingNotificationTask = [d]() {
					 ...
					 * offer/answer fails.*/
					if (d->state != State::Idle && d->state != State::PushIncomingReceived)
						return 0;
...
LinphoneStatus MediaSession::update(const MediaSessionParams *msp, const UpdateMethod method,
									const bool isCapabilityNegotiationUpdate, const string &subject) {
	L_D();
...
void MediaSession::requestNotifyNextVideoFrameDecoded() {
	...
	if (i)
		i->requestNotifyNextVideoFrameDecoded();
}
...
LinphoneStatus MediaSession::takePreviewSnapshot(const string &file) {
	...
	VideoControlInterface *i = d->getStreamsGroup().lookupMainStreamInterface<VideoControlInterface>(SalVideo);
	if (i)
		return i->takePreviewSnapshot(file);
	return -1;
}
...
LinphoneStatus MediaSession::takeVideoSnapshot(const string &file) {
	...
	VideoControlInterface *i = d->getStreamsGroup().lookupMainStreamInterface<VideoControlInterface>(SalVideo);
	if (i)
		return i->takeVideoSnapshot(file);
	return -1;
}
...
void MediaSession::enableEchoCancellation(bool value) {
	...
	AudioControlInterface *i = d->getStreamsGroup().lookupMainStreamInterface<AudioControlInterface>(SalAudio);
	if (i)
		i->enableEchoCancellation(value);
}
		...
		case LinphoneStreamTypeVideo: {
			...
			if ((videoStreamIdx >= 0) &&
				...
				const SalStreamDescription &videoStream =
					remoteMediaDesc->getStreamIdx(static_cast<unsigned int>(videoStreamIdx));

...
void ToneManager::notifyIncomingCall(const std::shared_ptr<CallSession> &session){
	...
	LinphoneCore *lc = getCore().getCCore();
	if ((currentCall != nullptr && currentCall->getActiveSession() != session) || linphone_core_is_in_conference(lc)){
...
void ToneManager::notifyState(const std::shared_ptr<CallSession> &callSession, CallSession::State state){
	...
	if (mSessionPaused){
		/* whatever happens on any call, we shall stop the paused state waiting tone, whose only purpose
		 * is to protect against unvolontary press of the pause button. */
		stopTone();
	...
	switch (state){
		...
		default:
			if (mSessionRinging){
				notifyIncomingCall(mSessionRinging);
			}
		break;
	}
}

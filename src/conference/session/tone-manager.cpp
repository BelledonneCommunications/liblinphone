...
LinphoneStatus ToneManager::playFile(const char *audiofile) {
	...
	if(bctbx_file_exist(audiofile) != 0){// This file doesn't exist. Check it from Platform resource.
		...
		bctbx_free(basename);
		lInfo() << "[ToneManager] " << __func__ << " requested play file " << std::string(audiofile) << " is not found. Using sound resource from platform helper " << ((fileString.empty()) ? std::string("<unknown>") : fileString);
		if(fileString != "")

...
void MS2Stream::startDtls(const OfferAnswerContext &params){
	...
	if (resultStreamDesc.getChosenConfiguration().dtls_role == SalDtlsRoleInvalid){
		...
		/* Give the peer certificate fingerprint to dtls context */
lInfo() << __func__ << " DEBUG DEBUG DTLS certificate remote " << params.getRemoteStreamDescription().getChosenConfiguration().dtls_fingerprint;
lInfo() << __func__ << " DEBUG DEBUG DTLS certificate local " << params.getLocalStreamDescription().getChosenConfiguration().dtls_fingerprint;

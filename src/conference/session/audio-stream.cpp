...
void MS2AudioStream::setInputDevice(AudioDevice *audioDevice) {
	if(!mStream) return;
	if(!audio_stream_set_input_ms_snd_card(mStream, (audioDevice?audioDevice->getSoundCard():NULL))){
...
void MS2AudioStream::setOutputDevice(AudioDevice *audioDevice) {
	if(!mStream) return;
	if(!audio_stream_set_output_ms_snd_card(mStream, (audioDevice?audioDevice->getSoundCard():NULL))){
		if(getState() == Running){// New device couldn't update the stream, request to stop it
			mStopStream = true;
		}
	}

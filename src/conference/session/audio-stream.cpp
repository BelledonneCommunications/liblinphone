		...
		for(int i = 0; (srtpSuites[i] != MS_CRYPTO_SUITE_INVALID) && (i < MS_MAX_ZRTP_CRYPTO_TYPES); i++) {
			switch (srtpSuites[i]) {
				...
				case MS_AEAD_AES_128_GCM:
					...
					params->ciphers[params->ciphersCount++] = MS_ZRTP_CIPHER_AES1;
					if (!gcm)
					params->authTags[params->authTagsCount++] = MS_ZRTP_AUTHTAG_GCM;
					gcm=true;
					aes1=true;
					break;
...
void MS2AudioStream::configureAudioStream(){
	...
	if (linphone_core_echo_cancellation_enabled(getCCore())) {
		...
		// If a positive delay has been measured by the echo canceller calibration, then we cannot rely on the hardware echo canceller (if any),
		// so we force the use of a software one.

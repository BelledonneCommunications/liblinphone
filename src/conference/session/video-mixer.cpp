...
void MS2VideoMixer::createLocalMember(bool isThumbnail) {
	...
	video_stream_set_fallback_to_dummy_codec(vs, linphone_config_get_bool(linphone_core_get_config(core), "video", "fallback_to_dummy_codec", TRUE));

	if (!mLocalDummyProfile)
	...
	if (isThumbnail) {
		...
		video_stream_set_native_preview_window_id(vs, linphone_core_get_native_preview_window_id(core));// Ensure to pass Window ID options (like not having to create a window)
		...
		if (linphone_core_get_video_display_filter(core)) {
			video_stream_set_display_filter_name(vs, linphone_core_get_video_display_filter(core));	// Use the filter defined in configuration
		} /*else keep default one*/
		video_stream_use_preview_video_window(vs, linphone_core_video_preview_enabled(core));	// Allow preview on conference
		video_stream_set_device_rotation(vs, mSession.getCCore()->device_rotation);
		video_stream_set_freeze_on_error(vs, !!linphone_config_get_int(linphone_core_get_config(mSession.getCCore()), "video", "freeze_on_error", 1));
		video_stream_use_video_preset(vs, linphone_config_get_string(linphone_core_get_config(mSession.getCCore()), "video", "preset", nullptr));
		...
		if (video_stream_start_from_io(vs, mLocalDummyProfile, "127.0.0.1", 65002, "127.0.0.1", 65003, sVP8PayloadTypeNumber, &io) != 0){
			lError() << "Could not start video stream.";
			video_stream_stop(vs);
			return;
		}
	...
	lInfo() << "[mix to all]: add local video endpoint label " << mLocalParticipantLabel << " is thumbnail " << isThumbnail;

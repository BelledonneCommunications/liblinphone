...
void CallSessionPrivate::onCallStateChanged (BCTBX_UNUSED(LinphoneCall *call), BCTBX_UNUSED(LinphoneCallState state), BCTBX_UNUSED(const std::string &message)) {
...
LinphoneStatus CallSessionPrivate::acceptUpdate (BCTBX_UNUSED(const CallSessionParams *csp), CallSession::State nextState, const string &stateInfo) {
...
void CallSession::iterate (time_t currentRealTime, bool oneSecondElapsed) {
	...
	if ((callTimeout > 0) && (connectedTime != 0)
		&& ((currentRealTime - connectedTime) > callTimeout)) {
		lInfo() << "Terminating call session " << this << " (local address " << getLocalAddress().asString() << " remote address " <<  (getRemoteAddress() ? getRemoteAddress()->asString() : "Unknown") << ") because the call timeout (" << callTimeout << "s) has been reached";
		terminate();
	}
}

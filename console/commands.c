...
static LPC_COMMAND commands[] = {
    ...
    {"calls", lpc_cmd_calls, "Show all the current calls with their id and status.", NULL},
    ...
    {"mute", lpc_cmd_mute_mic, "Mute microphone and suspend voice transmission.", NULL},
    ...
    {"playbackgain", lpc_cmd_playback_gain, "Adjust playback gain.", NULL},
    ...
    {"transfer", lpc_cmd_transfer, "Transfer a call to a specified destination.",
     ...
     "'transfer <call id1> --to-call <call id2>': transfers the call with 'id1' to the destination of call 'id2' "
     "(attended transfer)\n"},
    {"unmute", lpc_cmd_unmute_mic, "Unmute microphone and resume voice transmission.", NULL},
...
static LPC_COMMAND advanced_commands[] = {
    ...
    {"vfureq", lpc_cmd_vfureq, "Request the other side to send VFU for the current call", NULL},
#endif
    {"states", lpc_cmd_states,
    ...
    {"ringback", lpc_cmd_ringback, "Specifies a ringback tone to be played to remote end during incoming calls",
     "'ringback <path of mono .wav file>'\t: Specifies a ringback tone to be played to remote end during incoming "
     "calls\n"
 ...
 */
int linphonec_parse_command_line(LinphoneCore *lc, char *cl) {
	char *ptr = cl;
	char *args = NULL;
	LPC_COMMAND *cmd;
	...
	if (isdigit(*cl) || *cl == '#' || *cl == '*') {
		while (isdigit(*cl) || *cl == '#' || *cl == '*') {
			if (linphone_core_get_current_call(lc)) linphone_call_send_dtmf(linphone_core_get_current_call(lc), *cl);
 ...
 * top of the list.
 */
char *linphonec_command_generator(const char *text, int state) {
	static size_t len;
...
static int lpc_cmd_help(BCTBX_UNUSED(LinphoneCore *lc), char *arg) {
	int i = 0;
	LPC_COMMAND *cmd;
...
static int lpc_cmd_call(LinphoneCore *lc, char *args) {
	if (!args || !*args) {
		return 0;
	}
	{
		...
		if (opt1) {
			opt1[0] = '\0';
			while (--opt1 > args && opt1[0] == ' ')
				opt1[0] = '\0';
			linphone_call_params_enable_video(cp, FALSE);
		}
		if (opt2) {
			opt2[0] = '\0';
			while (--opt2 > args && opt2[0] == ' ')
				opt2[0] = '\0';
...
static int lpc_cmd_calls(LinphoneCore *lc, BCTBX_UNUSED(char *args)) {
...
static int lpc_cmd_chat(LinphoneCore *lc, char *args) {
	char *arg1 = args;
	char *arg2 = NULL;
	char *ptr = args;
	LinphoneChatRoom *cr;
...
static int lpc_cmd_transfer(LinphoneCore *lc, char *args) {
	if (args) {
		LinphoneCall *call;
		LinphoneCall *call2;
...
static int lpc_cmd_terminate(LinphoneCore *lc, char *args) {
...
static int lpc_cmd_redirect(LinphoneCore *lc, char *args) {
...
static int lpc_cmd_answer(LinphoneCore *lc, char *args) {
	if (!args) {
...
static int lpc_cmd_autoanswer(BCTBX_UNUSED(LinphoneCore *lc), char *args) {
	if (!args) {
...
static int lpc_cmd_quit(BCTBX_UNUSED(LinphoneCore *lc), BCTBX_UNUSED(char *args)) {
	linphonec_main_loop_exit();
	return 1;
}

static int lpc_cmd_nat(LinphoneCore *lc, char *args) {
	bool_t use;
	const char *nat;
	...
	linphonec_out("Nat address: %s%s\n", nat ? nat : "unspecified",
	              use ? "" : " (disabled - use 'firewall nat' to enable)");

	return 1;
}

static int lpc_cmd_stun(LinphoneCore *lc, char *args) {
	bool_t use;
	const char *stun;
	...
	linphonec_out("Stun server: %s%s\n", stun ? stun : "unspecified",
	              use ? "" : " (disabled - use 'firewall stun' to enable)");

	return 1;
}

static int lpc_cmd_firewall(LinphoneCore *lc, char *args) {
...
static int lpc_friend_name(char **args, char **name) {
...
static int lpc_cmd_friend(LinphoneCore *lc, char *args) {
	int friend_num;
 ...
 * Modified input
 */
static int lpc_cmd_proxy(LinphoneCore *lc, char *args) {
	char *arg1 = args;
	char *arg2 = NULL;
	char *ptr = args;
	int proxynum;
	...
	if (strcmp(arg1, "add") == 0) {
		...
		if (arg2 && *arg2) {
			proxynum = atoi(arg2);
			if (linphonec_proxy_use(lc, proxynum)) linphonec_out("Default proxy set to %d.\n", proxynum);
		} else {
...
static int lpc_cmd_call_logs(LinphoneCore *lc, BCTBX_UNUSED(char *args)) {
...
static int lpc_cmd_ipv6(LinphoneCore *lc, char *arg1) {
	if (!arg1) {
		return 0; /* syntax error */
	}
...
static int lpc_cmd_staticpic(LinphoneCore *lc, char *args) {
	char *arg1 = args;
	char *arg2 = NULL;
	char *ptr = args;
...
static void linphonec_proxy_add(LinphoneCore *lc) {
	...
	while (1) {
		...
		route_address = linphone_factory_create_address(linphone_factory_get(), clean);
		if (!route_address) {
		...
		linphone_account_params_set_routes_addresses(params, route_list);
...
static void linphonec_proxy_display(LinphoneAccount *account) {
	...
	linphonec_out("sip address: %s\nroute: %s\nidentity: %s\nregister: %s\nexpires: %i\nregistered: %s\n",
	              ...
	              linphone_account_params_get_expires(params),
	              linphone_account_get_state(account) == LinphoneRegistrationOk ? "yes" : "no");
	bctbx_free(identity);
	if (route) bctbx_free(route);
}
...
static int linphonec_friend_list(LinphoneCore *lc, char *pat) {
...
static int lpc_cmd_register(LinphoneCore *lc, char *args) {
	...
	LinphoneAccountParams *params;
	if (!account) {
		params = linphone_core_create_account_params(lc);
		account = linphone_core_create_account(lc, params);
	} else params = linphone_account_params_clone(linphone_account_get_params(account));
	...
	linphone_account_params_enable_register(params, TRUE);
	linphone_account_set_params(account, params);
	if (!elem) linphone_core_add_account(lc, account);
	linphone_core_set_default_account(lc, account);
	return 1;
}
...
static int lpc_cmd_ports(LinphoneCore *lc, char *args) {
	...
	if (!args) {
		linphonec_out("sip port = %i\naudio rtp port = %i\nvideo rtp port = %i\n", linphone_core_get_sip_port(lc),
		              linphone_core_get_audio_port(lc), linphone_core_get_video_port(lc));
		return 1;
	}
...
static int lpc_cmd_rtp_no_xmit_on_audio_mute(LinphoneCore *lc, char *args) {
	...
	if (args) {
		...
		if (linphone_core_get_current_call(lc) == NULL) linphone_core_set_rtp_no_xmit_on_audio_mute(lc, rtp_xmit_off);
		else linphonec_out("nortp-on-audio-mute: call in progress - cannot change state\n");
	}
...
static int _lpc_cmd_video_window(LinphoneCore *lc, char *args, bool_t is_preview) {
	...
	if (err >= 1) {
		if (strcmp(subcommand, "pos") == 0) {
			...
			if (is_preview) linphone_core_set_native_preview_window_id(lc, (void *)(long)a);
			else linphone_core_set_native_video_window_id(lc, (void *)(long)a);
...
static void lpc_display_call_states(LinphoneCore *lc) {
	...
	if (elem == NULL) {
		...
		for (; elem != NULL; elem = elem->next) {
			...
			linphonec_out("%-2i | %-35s | %-15s | %s\n", VOIDPTR_TO_INT(linphone_call_get_user_data(call)), tmp,
			              linphone_call_state_to_string(linphone_call_get_state(call)) + strlen("LinphoneCall"), flag);
			ms_free(tmp);
		}
	}
}
...
static int lpc_cmd_camera(LinphoneCore *lc, char *args) {
	...
	if (args) {
		if (strcmp(args, "on") == 0) activated = TRUE;
		else if (strcmp(args, "off") == 0) activated = FALSE;
		else return 0;
	}
	...
	if (call == NULL) {
		...
		if (linphonec_camera_enabled) {
			linphonec_out(
			    "Camera is enabled. Video stream will be setup immediately for outgoing and incoming calls.\n");
		} else {
			linphonec_out("Camera is disabled. Calls will be established with audio-only, with the possibility to "
			              "later add video using 'camera on'.\n");
		}
	} else {
...
static LPC_COMMAND *lpc_find_command(const char *name) {
	...
	for (i = 0; advanced_commands[i].name; ++i) {
		if (strcmp(name, advanced_commands[i].name) == 0) return &advanced_commands[i];
	}

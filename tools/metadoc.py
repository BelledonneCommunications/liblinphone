...
class ClassReference(Reference):
	...
	def resolve(self, api):
		try:
			...
			logging.warning('doc reference pointing on an unknown class object ({0})'.format(self.cname))
...
class Translator:
	...
	def translate_description(self, description, tagAsBrief=False):
		...
		for line in lines:
			...
			if len(line) == 0 and len(translatedDoc['lines']) > 0 and len(translatedDoc['lines'][-1]['line']) == 0:
				continue

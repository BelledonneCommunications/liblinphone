...
public interface LinphoneChatMessage {
  ...
  interface StateListener {
    void onLinphoneChatMessageStateChanged(LinphoneChatMessage msg,
                                           State state);
  }
  ...
  interface LinphoneChatMessageListener {
    ...
    /**
     * This function is called by the core upon an incoming File
     * transfer is started. This function may be call several time
     * for the same file in case of large file.
     * @param content incoming content information
     * @param buffer holding the received data. Empty buffer means
     * end of file.
     */
    void onLinphoneChatMessageFileTransferReceived(LinphoneChatMessage msg,
                                                   LinphoneContent content,
                                                   LinphoneBuffer buffer);

    /**
     * This function is called by the core when an outgoing file
     * transfer is started. This function is called until size is
     * set to 0.
     * @param content incoming content information
     * @param offset the offset in the file from where to get the
     * data to be sent
     * @param size the number of bytes expected by the framework
     * @param bufferToFill A LinphoneBuffer object holding the data
     * written by the application. An empty buffer means end of
     * file.
     */
    void onLinphoneChatMessageFileTransferSent(
        LinphoneChatMessage msg, LinphoneContent content, int offset, int size,
        LinphoneBuffer bufferToFill);

    /**
     * File transfer progress indication callback prototype.
     * @param content incoming content information
     * @param offset The number of bytes sent/received since the
     * beginning of the transfer.
     * @param total The total number of bytes to be sent/received.
     */
    void onLinphoneChatMessageFileTransferProgressChanged(
  ...
  public static class State {
    ...
    /**
     * Message succesffully delivered an acknoleged by remote end
     * point
     */
    ...
    /**
     * Message was received(and acknowledged) but cannot get file
     * from server
     */
    public final static State FileTransferError =
        new State(4, "FileTransferError");
    /**
     * File transfer has been completed successfully.
     */
    public final static State FileTransferDone =
        new State(5, "FileTransferDone");
   ...
   * @return true if the message has been sent, false if it has been
   * received
   */
  boolean isOutgoing();

  /**
   * THIS METHOD IS ONLY USED TO IMPORT OLD MESSAGES, DON'T USE IT FOR ANY
   * OTHER USAGE!
   */
  void store();
...
*/
  ...
  /**
   * Get the file_transfer_information (used by call backs to recover
   * informations during a rcs file transfer)
   * @return a pointer to the LinphoneContent structure or NULL if not
   * present.
   */
  ...
  /**
   * Set the path to the file to read from or write to during the file
   * transfer.
  ...
  /**
   * Fulfill a chat message char by char. Message linked to a Real Time
   * Text Call send char in realtime following RFC 4103/T.140
   ...
   * This is for optimizing the memory resources at runtime. Not calling
  *this does not result in a memory leak.
  **/
  void destroy();
}
